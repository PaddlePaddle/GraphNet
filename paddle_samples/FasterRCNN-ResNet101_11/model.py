...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2):
        ...
        # pd_op.reshape: (42780x4xf32) <- (42780x4xf32, 2xi64)
        ...
        # builtin.combine: ([42780x4xf32]) <- (42780x4xf32)
        combine_0 = [reshape_2]

        # pd_op.concat: (42780x4xf32) <- ([42780x4xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_0, full_1)

        # pd_op.transpose: (2x62x46x15xf32) <- (2x15x62x46xf32)
        transpose_0 = paddle._C_ops.transpose(data_0, [0, 2, 3, 1])

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_1 = [2, -1, 1]

        # pd_op.reshape: (2x42780x1xf32) <- (2x62x46x15xf32, 3xi64)
        ...
        # builtin.combine: ([2x42780x1xf32]) <- (2x42780x1xf32)
        combine_1 = [reshape_0]

        # pd_op.concat: (2x42780x1xf32) <- ([2x42780x1xf32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_1, full_0)

        # pd_op.transpose: (2x62x46x60xf32) <- (2x60x62x46xf32)
        transpose_1 = paddle._C_ops.transpose(data_1, [0, 2, 3, 1])

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [2, -1, 4]

        # pd_op.reshape: (2x42780x4xf32) <- (2x62x46x60xf32, 3xi64)
        reshape_1 = paddle._C_ops.reshape(transpose_1, full_int_array_2)

        # builtin.combine: ([2x42780x4xf32]) <- (2x42780x4xf32)
        combine_2 = [reshape_1]

        # pd_op.concat: (2x42780x4xf32) <- ([2x42780x4xf32], 1xi32)
