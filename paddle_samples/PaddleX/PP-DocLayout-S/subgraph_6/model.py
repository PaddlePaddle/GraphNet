...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.topk: (1x2x9xf32, 1x2x9xi64) <- (1x2x3136xf32, 1xi32)
        ...
        # pd_op.scale: (1x2x9xi64) <- (1x2x9xi64, 1xf32)
        ...
        # pd_op.one_hot: (1x2x9x3136xf32) <- (1x2x9xi64, 1xi32)
        ...
        # pd_op.sum: (1x2x3136xf32) <- (1x2x9x3136xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(one_hot_0, full_int_array_0, None, False)
        del one_hot_0

        # pd_op.multiply: (1x2x3136xf32) <- (1x2x3136xf32, 1x2x1xf32)
        multiply_0 = paddle._C_ops.multiply(sum_0, data_4)
        del sum_0

        # pd_op.topk: (1x2x9xf32, 1x2x9xi64) <- (1x2x784xf32, 1xi32)
        ...
        # pd_op.scale: (1x2x9xi64) <- (1x2x9xi64, 1xf32)
        scale_1 = paddle._C_ops.scale(topk_3, full_1, float("3136"), True)
        ...
        # pd_op.scale: (1x2x9xi64) <- (1x2x9xi64, 1xf32)
        scale_2 = paddle._C_ops.scale(topk_5, full_1, float("3920"), True)

        # pd_op.full: (1xi32) <- ()
        full_4 = paddle._C_ops.full(
            [1], float("196"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (1x2x9x196xf32) <- (1x2x9xi64, 1xi32)
        one_hot_2 = paddle._C_ops.one_hot(
            topk_5 % paddle.cast(full_4, topk_5.dtype), full_4
        )
        del full_4, topk_5

        # pd_op.sum: (1x2x196xf32) <- (1x2x9x196xf32, 1xi64)
        sum_2 = paddle._C_ops.sum(one_hot_2, full_int_array_0, None, False)
        del one_hot_2

        # pd_op.multiply: (1x2x196xf32) <- (1x2x196xf32, 1x2x1xf32)
        multiply_2 = paddle._C_ops.multiply(sum_2, data_4)
        del sum_2

        # pd_op.topk: (1x2x9xf32, 1x2x9xi64) <- (1x2x49xf32, 1xi32)
        ...
        # pd_op.scale: (1x2x9xi64) <- (1x2x9xi64, 1xf32)
        scale_3 = paddle._C_ops.scale(topk_7, full_1, float("4116"), True)
        del full_1
        ...
        # builtin.combine: ([1x2x3136xf32, 1x2x784xf32, 1x2x196xf32, 1x2x49xf32]) <- (1x2x3136xf32, 1x2x784xf32, 1x2x196xf32, 1x2x49xf32)
        combine_0 = [multiply_0, multiply_1, multiply_2, multiply_3]
        del multiply_0, multiply_1, multiply_2, multiply_3

        # pd_op.concat: (1x2x4165xf32) <- ([1x2x3136xf32, 1x2x784xf32, 1x2x196xf32, 1x2x49xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_0, full_6)
        del combine_0

        # builtin.combine: ([1x2x9xi64, 1x2x9xi64, 1x2x9xi64, 1x2x9xi64]) <- (1x2x9xi64, 1x2x9xi64, 1x2x9xi64, 1x2x9xi64)
        combine_1 = [scale_0, scale_1, scale_2, scale_3]
        del scale_0, scale_1, scale_2, scale_3

        # pd_op.concat: (1x2x36xi64) <- ([1x2x9xi64, 1x2x9xi64, 1x2x9xi64, 1x2x9xi64], 1xi32)
