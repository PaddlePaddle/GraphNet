...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, data_0, data_1, data_2, data_3):
        # pd_op.divide: (44436x2xf32) <- (44436x2xf32, 44436x1xf32)
        divide_0 = paddle._C_ops.divide(data_2, data_3)

        # pd_op.shape64: (3xi64) <- (1x44436x88xf32)
        ...
        full_5 = paddle._C_ops.full(
            [], float("22"), paddle.int64, paddle.core.CPUPlace()
        )

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_0 = [full_3, slice_1, full_4, full_5]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_0 = paddle._C_ops.stack(combine_0, 0)

        # pd_op.reshape: (-1x-1x4x22xf32) <- (1x44436x88xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(data_1, stack_0)

        # pd_op.softmax: (-1x-1x4x22xf32) <- (-1x-1x4x22xf32)
        softmax_0 = paddle._C_ops.softmax(reshape_0, -1)

        # pd_op.transpose: (-1x22x-1x4xf32) <- (-1x-1x4x22xf32)
        transpose_0 = paddle._C_ops.transpose(softmax_0, [0, 3, 1, 2])

        # pd_op.conv2d: (-1x1x-1x4xf32) <- (-1x22x-1x4xf32, 1x22x1x1xf32)
        ...
        # pd_op.add: (-1x44436x2xf32) <- (-1x-1x2xf32, 44436x2xf32)
        ...
        # builtin.combine: ([-1x44436x2xf32, -1x44436x2xf32]) <- (-1x44436x2xf32, -1x44436x2xf32)
        combine_1 = [add_0, add_1]

        # pd_op.concat: (-1x44436x4xf32) <- ([-1x44436x2xf32, -1x44436x2xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_1, full_2)

        # pd_op.share_data_: (1x44436x10xf32) <- (1x44436x10xf32)
        share_data__0 = data_0.detach()

        # pd_op.share_data_: (-1x44436x4xf32) <- (-1x44436x4xf32)
        share_data__1 = concat_0.detach()

        # pd_op.multiply: (-1x44436x4xf32) <- (-1x44436x4xf32, 44436x1xf32)
