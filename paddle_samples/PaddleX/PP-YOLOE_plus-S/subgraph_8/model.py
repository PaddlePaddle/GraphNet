...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.argmax: (8x-1xi64) <- (8x-1x-1xf32, 1xi64)
        ...
        # pd_op.add: (8x-1xi64) <- (8x-1xi64, 8x1xi64)
        ...
        # pd_op.reshape: (8x-1xi32) <- (-1xi32, 2xi64)
        ...
        # pd_op.full_like: (8x-1xi32) <- (8x-1xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_7, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_7

        # pd_op.where: (8x-1xi32) <- (8x-1xb, 8x-1xi32, 8x-1xi32)
        ...
        # pd_op.one_hot: (8x-1x5xf32) <- (8x-1xi32, 1xi32)
        ...
        # pd_op.index_select: (8x-1x4xf32) <- (8x-1x5xf32, 4xi64)
        ...
        # pd_op.unsqueeze: (8x1x-1x4xf32) <- (8x-1x4xf32, 1xi64)
        ...
        # pd_op.slice: (8x1x-1x2xf32) <- (8x1x-1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (8x-1x-1x2xf32) <- (8x-1x1x2xf32, 8x1x-1x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (8x-1x-1x2xf32) <- (8x-1x1x2xf32, 8x1x-1x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (8x-1x-1x2xf32) <- (8x-1x-1x2xf32, 8x-1x-1x2xf32)
        ...
        # pd_op.clip: (8x-1x-1x2xf32) <- (8x-1x-1x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_11, full_12)
        del subtract_0

        # pd_op.prod: (8x-1x-1xf32) <- (8x-1x-1x2xf32, 1xi64)
        ...
        # pd_op.subtract: (8x1x-1x2xf32) <- (8x1x-1x2xf32, 8x1x-1x2xf32)
        subtract_2 = paddle._C_ops.subtract(slice_3, slice_2)
        del slice_2, slice_3

        # pd_op.clip: (8x1x-1x2xf32) <- (8x1x-1x2xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(subtract_2, full_11, full_12)
        del full_11, full_12, subtract_2

        # pd_op.prod: (8x1x-1xf32) <- (8x1x-1x2xf32, 1xi64)
        prod_2 = paddle._C_ops.prod(clip_2, full_int_array_0, False, False)
        del clip_2

        # pd_op.add: (8x-1x-1xf32) <- (8x-1x1xf32, 8x1x-1xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)
        del prod_1, prod_2

        # pd_op.subtract: (8x-1x-1xf32) <- (8x-1x-1xf32, 8x-1x-1xf32)
        ...
        # pd_op.scale: (8x-1x-1xf32) <- (8x-1x-1xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_3, full_13, float("1e-09"), True)
        del full_13, subtract_3

        # pd_op.divide: (8x-1x-1xf32) <- (8x-1x-1xf32, 8x-1x-1xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_0)
        del prod_0, scale_0

        # pd_op.multiply: (8x-1x-1xf32) <- (8x-1x-1xf32, 8x-1x-1xf32)
