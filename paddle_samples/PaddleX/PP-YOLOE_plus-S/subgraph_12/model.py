...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (8x7581xi64) <- (8x1x7581xf32, 1xi64)
        ...
        # pd_op.add: (8x7581xi64) <- (8x7581xi64, 8x1xi64)
        ...
        # pd_op.flatten: (60648xi64) <- (8x7581xi64)
        ...
        # pd_op.gather: (60648xi32) <- (8xi32, 60648xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_5)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [8, 7581]

        # pd_op.reshape: (8x7581xi32) <- (60648xi32, 2xi64)
        ...
        # pd_op.greater_than: (8x7581xb) <- (8x7581xf32, xf32)
        ...
        # pd_op.full_like: (8x7581xi32) <- (8x7581xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_7, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_7

        # pd_op.where: (8x7581xi32) <- (8x7581xb, 8x7581xi32, 8x7581xi32)
        ...
        # pd_op.gather: (60648x4xf32) <- (8x4xf32, 60648xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_5)
        del flatten_1, full_5, reshape_2

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [8, 7581, 4]

        # pd_op.reshape: (8x7581x4xf32) <- (60648x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (8x7581x5xf32) <- (8x7581xi32, 1xi32)
        ...
        # pd_op.index_select: (8x7581x4xf32) <- (8x7581x5xf32, 4xi64)
        ...
        # pd_op.unsqueeze: (8x1x7581x4xf32) <- (8x7581x4xf32, 1xi64)
        ...
        # pd_op.slice: (8x1x7581x2xf32) <- (8x1x7581x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (8x1x7581x2xf32) <- (8x1x1x2xf32, 8x1x7581x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (8x1x7581x2xf32) <- (8x1x1x2xf32, 8x1x7581x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (8x1x7581x2xf32) <- (8x1x7581x2xf32, 8x1x7581x2xf32)
        ...
        # pd_op.clip: (8x1x7581x2xf32) <- (8x1x7581x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_10, full_11)
        del subtract_0

        # pd_op.prod: (8x1x7581xf32) <- (8x1x7581x2xf32, 1xi64)
        ...
        # pd_op.add: (8x1x7581xf32) <- (8x1x1xf32, 8x1x7581xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)
        del prod_1, prod_2

        # pd_op.subtract: (8x1x7581xf32) <- (8x1x7581xf32, 8x1x7581xf32)
        subtract_3 = paddle._C_ops.subtract(add_1, prod_0)
        del add_1

        # pd_op.scale: (8x1x7581xf32) <- (8x1x7581xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_4, float("1e-09"), True)
        del full_4, subtract_3

        # pd_op.divide: (8x1x7581xf32) <- (8x1x7581xf32, 8x1x7581xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_1)
        del prod_0, scale_1

        # pd_op.multiply: (8x1x7581xf32) <- (8x1x7581xf32, 8x1x7581xf32)
