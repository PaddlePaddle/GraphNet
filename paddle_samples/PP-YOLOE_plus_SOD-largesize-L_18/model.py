...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2):
        ...
        # pd_op.unsqueeze: (1x1x56784x2xf32) <- (56784x2xf32, 2xi64)
        ...
        # pd_op.split_with_num: ([1x1x56784x1xf32, 1x1x56784x1xf32]) <- (1x1x56784x2xf32, 1xi32)
        split_with_num_0 = paddle._C_ops.split_with_num(unsqueeze_0, 2, full_0)

        # builtin.split: (1x1x56784x1xf32, 1x1x56784x1xf32) <- ([1x1x56784x1xf32, 1x1x56784x1xf32])
        (
            split_0,
            split_1,
        ) = split_with_num_0
        ...
        # pd_op.unsqueeze: (1x46x1x4xf32) <- (1x46x4xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(data_1, full_int_array_1)

        # pd_op.split_with_num: ([1x46x1x1xf32, 1x46x1x1xf32, 1x46x1x1xf32, 1x46x1x1xf32]) <- (1x46x1x4xf32, 1xi32)
        split_with_num_1 = paddle._C_ops.split_with_num(unsqueeze_1, 4, full_0)

        # builtin.split: (1x46x1x1xf32, 1x46x1x1xf32, 1x46x1x1xf32, 1x46x1x1xf32) <- ([1x46x1x1xf32, 1x46x1x1xf32, 1x46x1x1xf32, 1x46x1x1xf32])
        (
            split_2,
            split_3,
            split_4,
        ...
        # pd_op.subtract: (1x46x56784x1xf32) <- (1x1x56784x1xf32, 1x46x1x1xf32)
        subtract_0 = paddle._C_ops.subtract(split_0, split_2)

        # pd_op.subtract: (1x46x56784x1xf32) <- (1x1x56784x1xf32, 1x46x1x1xf32)
        subtract_1 = paddle._C_ops.subtract(split_1, split_3)

        # pd_op.subtract: (1x46x56784x1xf32) <- (1x46x1x1xf32, 1x1x56784x1xf32)
        ...
        # builtin.combine: ([1x46x56784x1xf32, 1x46x56784x1xf32, 1x46x56784x1xf32, 1x46x56784x1xf32]) <- (1x46x56784x1xf32, 1x46x56784x1xf32, 1x46x56784x1xf32, 1x46x56784x1xf32)
        combine_0 = [subtract_0, subtract_1, subtract_2, subtract_3]

        # pd_op.concat: (1x46x56784x4xf32) <- ([1x46x56784x1xf32, 1x46x56784x1xf32, 1x46x56784x1xf32, 1x46x56784x1xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_0, full_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [-1]

        # pd_op.min: (1x46x56784xf32) <- (1x46x56784x4xf32, 1xi64)
        ...
        # pd_op.greater_than: (1x46x56784xb) <- (1x46x56784xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(min_0, full_2)

        # pd_op.unsqueeze: (1x1x56784x1xf32) <- (56784x1xf32, 2xi64)
        ...
        # pd_op.scale: (1x46x1x1xf32) <- (1x46x1x1xf32, 1xf32)
        ...
        # pd_op.subtract: (1x46x56784x1xf32) <- (1x1x56784x1xf32, 1x46x56784x1xf32)
        ...
        # pd_op.add: (1x46x56784x1xf32) <- (1x46x1x1xf32, 1x1x56784x1xf32)
        add_2 = paddle._C_ops.add(scale_0, unsqueeze_2)

        # pd_op.subtract: (1x46x56784x1xf32) <- (1x46x56784x1xf32, 1x1x56784x1xf32)
        ...
        # pd_op.cast: (1x46x56784xf32) <- (1x46x56784xb)
