...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.add: (8x500x256xf32) <- (8x500x256xf32, 8x500x256xf32)
        add_0 = paddle._C_ops.add(data_9, data_8)

        # pd_op.layer_norm: (8x500x256xf32, 8x500xf32, 8x500xf32) <- (8x500x256xf32, 256xf32, 256xf32)
        ...
        combine_0 = [full_5, slice_0, full_5, full_6]
        ...
        # pd_op.matmul: (8x500x192xf32) <- (8x500x256xf32, 256x192xf32)
        matmul_1 = paddle._C_ops.matmul(add_1, parameter_13, False, False)

        # pd_op.add: (8x500x192xf32) <- (8x500x192xf32, 192xf32)
        add_3 = paddle._C_ops.add(matmul_1, parameter_12)

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_3 = [8, 500, 8, 3, 4, 2]

        # pd_op.reshape: (8x500x8x3x4x2xf32) <- (8x500x192xf32, 6xi64)
        reshape_5 = paddle._C_ops.reshape(add_3, full_int_array_3)

        # pd_op.matmul: (8x500x96xf32) <- (8x500x256xf32, 256x96xf32)
        matmul_2 = paddle._C_ops.matmul(add_1, parameter_11, False, False)

        # pd_op.add: (8x500x96xf32) <- (8x500x96xf32, 96xf32)
        add_4 = paddle._C_ops.add(matmul_2, parameter_10)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_4 = [8, 500, 8, 12]

        # pd_op.reshape: (8x500x8x12xf32) <- (8x500x96xf32, 4xi64)
        reshape_6 = paddle._C_ops.reshape(add_4, full_int_array_4)

        # pd_op.softmax: (8x500x8x12xf32) <- (8x500x8x12xf32)
        softmax_0 = paddle._C_ops.softmax(reshape_6, -1)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_5 = [8, 500, 8, 3, 4]

        # pd_op.reshape: (8x500x8x3x4xf32) <- (8x500x8x12xf32, 5xi64)
        ...
        # pd_op.slice: (8x500x1x2xf32) <- (8x500x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.unsqueeze: (8x500x1x1x1x2xf32) <- (8x500x1x2xf32, 2xi64)
        ...
        # pd_op.scale: (8x500x8x3x4x2xf32) <- (8x500x8x3x4x2xf32, 1xf32)
        ...
        # pd_op.multiply: (8x500x8x3x4x2xf32) <- (8x500x8x3x4x2xf32, 8x500x1x1x1x2xf32)
        ...
        # pd_op.add: (8x500x8x3x4x2xf32) <- (8x500x1x1x1x2xf32, 8x500x8x3x4x2xf32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_7 = [full_8, full_6, slice_5, slice_6]
        ...
        # pd_op.slice: (8x500x8x4x2xf32) <- (8x500x8x3x4x2xf32, 1xi64, 1xi64)
        slice_13 = paddle._C_ops.slice(
            scale_2, [3], full_int_array_6, full_int_array_1, [1], [3]
        )

        # pd_op.transpose: (8x8x500x4x2xf32) <- (8x500x8x4x2xf32)
        transpose_1 = paddle._C_ops.transpose(slice_13, [0, 2, 1, 3, 4])

        # pd_op.flatten: (64x500x4x2xf32) <- (8x8x500x4x2xf32)
        flatten_0 = paddle._C_ops.flatten(transpose_1, 0, 1)

        # pd_op.grid_sample: (64x32x500x4xf32) <- (64x32x-1x-1xf32, 64x500x4x2xf32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_8 = [full_8, full_6, slice_8, slice_9]
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_9 = [full_8, full_6, slice_11, slice_12]
        ...
        # pd_op.transpose: (8x8x500x3x4xf32) <- (8x500x8x3x4xf32)
        transpose_6 = paddle._C_ops.transpose(reshape_7, [0, 2, 1, 3, 4])

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_10 = [64, 1, 500, 12]

        # pd_op.reshape: (64x1x500x12xf32) <- (8x8x500x3x4xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(transpose_6, full_int_array_10)

        # builtin.combine: ([64x32x500x4xf32, 64x32x500x4xf32, 64x32x500x4xf32]) <- (64x32x500x4xf32, 64x32x500x4xf32, 64x32x500x4xf32)
        combine_10 = [grid_sample_0, grid_sample_1, grid_sample_2]

        # pd_op.stack: (64x32x500x3x4xf32) <- ([64x32x500x4xf32, 64x32x500x4xf32, 64x32x500x4xf32])
        stack_0 = paddle._C_ops.stack(combine_10, -2)

        # pd_op.flatten: (64x32x500x12xf32) <- (64x32x500x3x4xf32)
        flatten_3 = paddle._C_ops.flatten(stack_0, 3, 4)

        # pd_op.multiply: (64x32x500x12xf32) <- (64x32x500x12xf32, 64x1x500x12xf32)
        multiply_0 = paddle._C_ops.multiply(flatten_3, reshape_3)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [-1]

        # pd_op.sum: (64x32x500xf32) <- (64x32x500x12xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(multiply_0, full_int_array_0, None, False)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_11 = [8, 256, 500]

        # pd_op.reshape: (8x256x500xf32) <- (64x32x500xf32, 3xi64)
        reshape_8 = paddle._C_ops.reshape(sum_0, full_int_array_11)

        # pd_op.transpose: (8x500x256xf32) <- (8x256x500xf32)
        transpose_7 = paddle._C_ops.transpose(reshape_8, [0, 2, 1])

        # pd_op.matmul: (8x500x256xf32) <- (8x500x256xf32, 256x256xf32)
        matmul_3 = paddle._C_ops.matmul(transpose_7, parameter_9, False, False)

        # pd_op.add: (8x500x256xf32) <- (8x500x256xf32, 256xf32)
        ...
        # pd_op.matmul: (8x500x1024xf32) <- (8x500x256xf32, 256x1024xf32)
        matmul_4 = paddle._C_ops.matmul(layer_norm_3, parameter_5, False, False)

        # pd_op.add: (8x500x1024xf32) <- (8x500x1024xf32, 1024xf32)
        add_10 = paddle._C_ops.add(matmul_4, parameter_4)

        # pd_op.relu: (8x500x1024xf32) <- (8x500x1024xf32)
        relu_0 = paddle._C_ops.relu(add_10)

        # pd_op.matmul: (8x500x256xf32) <- (8x500x1024xf32, 1024x256xf32)
