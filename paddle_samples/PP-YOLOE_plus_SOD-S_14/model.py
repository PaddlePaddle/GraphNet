...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        full_int_array_1 = [1, 45, 1]

        # pd_op.tile: (2x45x6804xb) <- (2x1x6804xb, 3xi64)
        ...
        # pd_op.argmax: (2x6804xi64) <- (2x45x6804xf32, 1xi64)
        argmax_0 = paddle._C_ops.argmax(multiply_1, full_1, False, False, paddle.int64)

        # pd_op.full: (1xi32) <- ()
        full_2 = paddle._C_ops.full(
            [1], float("45"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (2x6804x45xf32) <- (2x6804xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )

        # pd_op.transpose: (2x45x6804xf32) <- (2x6804x45xf32)
        ...
        full_3 = paddle._C_ops.full(
            [1], float("45"), paddle.float32, paddle.core.CPUPlace()
        )
        ...
        # pd_op.flatten: (90xi32) <- (2x45x1xi32)
        ...
        # pd_op.gather: (13608xi32) <- (90xi32, 13608xi64, 1xi32)
        ...
        # pd_op.reshape: (90x4xf32) <- (2x45x4xf32, 2xi64)
        reshape_2 = paddle._C_ops.reshape(data_5, full_int_array_4)

        # pd_op.gather: (13608x4xf32) <- (90x4xf32, 13608xi64, 1xi32)
        ...
        # pd_op.max: (2x45x1xf32) <- (2x45x6804xf32, 1xi64)
        ...
        # pd_op.scale: (2x45x1xf32) <- (2x45x1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(max_0, full_9, float("1e-09"), True)

        # pd_op.divide: (2x45x6804xf32) <- (2x45x6804xf32, 2x45x1xf32)
        ...
        # pd_op.max: (2x6804xf32) <- (2x45x6804xf32, 1xi64)
