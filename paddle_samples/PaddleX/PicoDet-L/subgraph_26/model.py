...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2):
        # pd_op.cast: (55080x4xf32) <- (55080x4xf32)
        ...
        # pd_op.greater_than: (55080x4xb) <- (55080x4xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(cast_0, full_0)

        # pd_op.cast: (55080x4xf32) <- (55080x4xb)
        cast_1 = paddle._C_ops.cast(greater_than_0, paddle.float32)
        del greater_than_0

        # pd_op.multiply: (55080x4xf32) <- (55080x4xf32, 55080x4xf32)
        multiply_0 = paddle._C_ops.multiply(cast_0, cast_1)
        del cast_1

        # pd_op.subtract: (55080x4xf32) <- (55080x4xf32, 55080x4xf32)
        subtract_0 = paddle._C_ops.subtract(data_0, cast_0)

        # pd_op.abs: (55080x4xf32) <- (55080x4xf32)
        abs_0 = paddle._C_ops.abs(subtract_0)

        # pd_op.pow: (55080x4xf32) <- (55080x4xf32)
        ...
        # pd_op.scale: (55080x4xf32) <- (55080x4xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(pow_0, full_1, float("0"), True)
        del pow_0

        # pd_op.less_equal: (55080x4xb) <- (55080x4xf32, xf32)
        ...
        # pd_op.add: (55080x4xf32) <- (55080x4xf32, 55080x4xf32)
        add_0 = paddle._C_ops.add(multiply_0, multiply_1)

        # pd_op.bce_loss: (55080x4xf32) <- (55080x4xf32, 55080x4xf32)
        ...
        # pd_op.sum: (55080xf32) <- (55080x4xf32, 1xi64)
        ...
        # pd_op.scale: (55080xf32) <- (55080xf32, 1xf32)
        ...
        # pd_op.sum: (xf32) <- (55080xf32, 0xi64)
