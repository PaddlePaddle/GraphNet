...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # builtin.combine: ([441x4xf32, 1764x4xf32, 7056x4xf32]) <- (441x4xf32, 1764x4xf32, 7056x4xf32)
        combine_9 = [reshape_0, reshape_2, reshape_4]

        # pd_op.concat: (9261x4xf32) <- ([441x4xf32, 1764x4xf32, 7056x4xf32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_9, full_12)
        del combine_9

        # builtin.combine: ([441x2xf32, 1764x2xf32, 7056x2xf32]) <- (441x2xf32, 1764x2xf32, 7056x2xf32)
        combine_10 = [reshape_1, reshape_3, reshape_5]
        del reshape_1, reshape_3, reshape_5

        # pd_op.concat: (9261x2xf32) <- ([441x2xf32, 1764x2xf32, 7056x2xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_10, full_12)
        del combine_10

        # builtin.combine: ([441x1xf32, 1764x1xf32, 7056x1xf32]) <- (441x1xf32, 1764x1xf32, 7056x1xf32)
        combine_11 = [full_5, full_8, full_11]
        del full_11, full_5, full_8

        # pd_op.concat: (9261x1xf32) <- ([441x1xf32, 1764x1xf32, 7056x1xf32], 1xi32)
        ...
        # pd_op.pool2d: (4x960x1x1xf32) <- (4x960x21x21xf32, 2xi64)
        ...
        # pd_op.sigmoid: (4x960x1x1xf32) <- (4x960x1x1xf32)
        ...
        # pd_op.add: (4x960x21x21xf32) <- (4x960x21x21xf32, 4x960x21x21xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (4x4x21x21xf32) <- (4x960x21x21xf32, 4x960x3x3xf32)
        ...
        # pd_op.add: (4x4x21x21xf32) <- (4x4x21x21xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (4x68x21x21xf32) <- (4x960x21x21xf32, 68x960x3x3xf32)
        ...
        # pd_op.add: (4x68x21x21xf32) <- (4x68x21x21xf32, 1x68x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_5, reshape_9)

        # pd_op.sigmoid: (4x4x21x21xf32) <- (4x4x21x21xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_2)
        del add_2

        # pd_op.flatten: (4x4x441xf32) <- (4x4x21x21xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (4x441x4xf32) <- (4x4x441xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (4x68x441xf32) <- (4x68x21x21xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (4x441x68xf32) <- (4x68x441xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (4x480x1x1xf32) <- (4x480x42x42xf32, 2xi64)
        ...
        # pd_op.sigmoid: (4x480x1x1xf32) <- (4x480x1x1xf32)
        ...
        # pd_op.add: (4x480x42x42xf32) <- (4x480x42x42xf32, 4x480x42x42xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (4x4x42x42xf32) <- (4x480x42x42xf32, 4x480x3x3xf32)
        ...
        # pd_op.add: (4x4x42x42xf32) <- (4x4x42x42xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (4x68x42x42xf32) <- (4x480x42x42xf32, 68x480x3x3xf32)
        ...
        # pd_op.add: (4x68x42x42xf32) <- (4x68x42x42xf32, 1x68x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (4x4x42x42xf32) <- (4x4x42x42xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (4x4x1764xf32) <- (4x4x42x42xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (4x1764x4xf32) <- (4x4x1764xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (4x68x1764xf32) <- (4x68x42x42xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (4x1764x68xf32) <- (4x68x1764xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (4x240x1x1xf32) <- (4x240x84x84xf32, 2xi64)
        ...
        # pd_op.sigmoid: (4x240x1x1xf32) <- (4x240x1x1xf32)
        ...
        # pd_op.add: (4x240x84x84xf32) <- (4x240x84x84xf32, 4x240x84x84xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (4x4x84x84xf32) <- (4x240x84x84xf32, 4x240x3x3xf32)
        ...
        # pd_op.add: (4x4x84x84xf32) <- (4x4x84x84xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (4x68x84x84xf32) <- (4x240x84x84xf32, 68x240x3x3xf32)
        ...
        # pd_op.add: (4x68x84x84xf32) <- (4x68x84x84xf32, 1x68x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (4x4x84x84xf32) <- (4x4x84x84xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (4x4x7056xf32) <- (4x4x84x84xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (4x7056x4xf32) <- (4x4x7056xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (4x68x7056xf32) <- (4x68x84x84xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (4x7056x68xf32) <- (4x68x7056xf32)
        ...
        # builtin.combine: ([4x441x4xf32, 4x1764x4xf32, 4x7056x4xf32]) <- (4x441x4xf32, 4x1764x4xf32, 4x7056x4xf32)
        combine_12 = [transpose_0, transpose_2, transpose_4]

        # pd_op.concat: (4x9261x4xf32) <- ([4x441x4xf32, 4x1764x4xf32, 4x7056x4xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_12, full_13)
        del combine_12

        # builtin.combine: ([4x441x68xf32, 4x1764x68xf32, 4x7056x68xf32]) <- (4x441x68xf32, 4x1764x68xf32, 4x7056x68xf32)
        combine_13 = [transpose_1, transpose_3, transpose_5]

        # pd_op.concat: (4x9261x68xf32) <- ([4x441x68xf32, 4x1764x68xf32, 4x7056x68xf32], 1xi32)
