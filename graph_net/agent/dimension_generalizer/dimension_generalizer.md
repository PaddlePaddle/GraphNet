...
12,538 个原始模型
  │
  ├─ 不可泛化（6,776 个，54.0%）
  │    ├─ 6,306 个全静态 ── 所有维度与权重 shape 绑定（纯 MLP、固定分辨率 CNN、小子图等）
  │    ├─ 892 个探测错误且无 constraints ── forward 在非标准维度下卡死
  │    └─ 838 个假阳性 ── 架构参数（head_dim 等）被误判为动态维度
  │
  ├─ 低优先级（857 个，6.8%）
  │    └─ 超时 ── 探测耗时过长被 kill，部分已进 pipeline
  │
  └─ 成功泛化（4,934 个，39.4%）
       │
       │  合计 5,762 个进入 pipeline（4,899 首轮 + 863 补充轮）
       │
       └─ 4,934 个成功生成变体 ─→ 38,030 个验证通过变体 ✓

合计：12,538 + 38,030 = 50,568 个模型（~403% 转化率）
```
