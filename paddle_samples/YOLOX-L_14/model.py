...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.slice: (658xf32) <- (658x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.subtract: (658xf32) <- (658xf32, 658xf32)
        ...
        # pd_op.multiply: (658xf32) <- (658xf32, 658xf32)
        ...
        # pd_op.slice: (658x2xf32) <- (658x4xf32, 1xi64, 1xi64)
        slice_8 = paddle._C_ops.slice(
            data_0, [1], full_int_array_2, full_int_array_0, [1], []
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [658, 1, 2]

        # pd_op.reshape: (658x1x2xf32) <- (658x2xf32, 3xi64)
        ...
        # pd_op.maximum: (658x2x2xf32) <- (658x1x2xf32, 2x2xf32)
        ...
        # pd_op.minimum: (658x2x2xf32) <- (658x1x2xf32, 2x2xf32)
        minimum_0 = paddle._C_ops.minimum(reshape_1, slice_11)

        # pd_op.subtract: (658x2x2xf32) <- (658x2x2xf32, 658x2x2xf32)
        ...
        # pd_op.clip: (658x2x2xf32) <- (658x2x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_4, full_0, full_1)

        # pd_op.slice: (658x2xf32) <- (658x2x2xf32, 1xi64, 1xi64)
        ...
        # pd_op.multiply: (658x2xf32) <- (658x2xf32, 658x2xf32)
        multiply_2 = paddle._C_ops.multiply(slice_12, slice_13)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_7 = [658, 1]

        # pd_op.reshape: (658x1xf32) <- (658xf32, 2xi64)
        ...
        # pd_op.add: (658x2xf32) <- (658x1xf32, 1x2xf32)
        add_0 = paddle._C_ops.add(reshape_2, reshape_3)

        # pd_op.subtract: (658x2xf32) <- (658x2xf32, 658x2xf32)
        ...
        # pd_op.maximum: (658x2xf32) <- (658x2xf32, 1xf32)
        maximum_1 = paddle._C_ops.maximum(subtract_5, assign_value__0)

        # pd_op.divide: (658x2xf32) <- (658x2xf32, 658x2xf32)
