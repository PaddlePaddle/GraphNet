...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.unsqueeze: (1x1x24276xf32) <- (1x24276xf32, 1xi64)
        ...
        # pd_op.argmax: (1x24276xi64) <- (1x38x24276xf32, 1xi64)
        argmax_0 = paddle._C_ops.argmax(multiply_1, full_1, False, False, paddle.int64)
        del multiply_1
        ...
        # pd_op.one_hot: (1x24276x38xf32) <- (1x24276xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )
        del argmax_0, full_2

        # pd_op.transpose: (1x38x24276xf32) <- (1x24276x38xf32)
        transpose_0 = paddle._C_ops.transpose(one_hot_0, [0, 2, 1])
        del one_hot_0

        # pd_op.where: (1x38x24276xf32) <- (1x38x24276xb, 1x38x24276xf32, 1x38x24276xf32)
        ...
        # pd_op.sum: (1x24276xf32) <- (1x38x24276xf32, 1xi64)
        ...
        full_3 = paddle._C_ops.full(
            [1], float("38"), paddle.float32, paddle.core.CPUPlace()
        )
        ...
        # pd_op.add: (1x24276xi64) <- (1x24276xi64, 1x1xi64)
        add_0 = paddle._C_ops.add(argmax_1, cast_0)
        del argmax_1, cast_0

        # pd_op.flatten: (38xi32) <- (1x38x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_4, 0, 2)
        del data_4

        # pd_op.flatten: (24276xi64) <- (1x24276xi64)
        ...
        # pd_op.gather: (24276xi32) <- (38xi32, 24276xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_3 = [1, 24276]

        # pd_op.reshape: (1x24276xi32) <- (24276xi32, 2xi64)
        ...
        # pd_op.greater_than: (1x24276xb) <- (1x24276xf32, xf32)
        ...
        # pd_op.full_like: (1x24276xi32) <- (1x24276xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_6

        # pd_op.where: (1x24276xi32) <- (1x24276xb, 1x24276xi32, 1x24276xi32)
        ...
        # pd_op.one_hot: (1x24276x11xf32) <- (1x24276xi32, 1xi32)
        ...
        full_8 = paddle._C_ops.full(
            [10], float("0"), paddle.int64, paddle.framework._current_expected_place()
        )

        # pd_op.assign_value_: (10xi64) <- (10xi64)
        ...
        # pd_op.index_select: (1x24276x10xf32) <- (1x24276x11xf32, 10xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_1, assign_value__0, -1)
        del assign_value__0, one_hot_1

        # pd_op.multiply: (1x38x24276xf32) <- (1x38x24276xf32, 1x38x24276xf32)
        multiply_2 = paddle._C_ops.multiply(data_6, where_0)
        del data_6

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_6 = [-1]

        # pd_op.max: (1x38x1xf32) <- (1x38x24276xf32, 1xi64)
        max_0 = paddle._C_ops.max(multiply_2, full_int_array_6, True)

        # pd_op.multiply: (1x38x24276xf32) <- (1x38x24276xf32, 1x38x24276xf32)
        multiply_3 = paddle._C_ops.multiply(data_1, where_0)
        del data_1, where_0

        # pd_op.max: (1x38x1xf32) <- (1x38x24276xf32, 1xi64)
        max_1 = paddle._C_ops.max(multiply_3, full_int_array_6, True)
        del multiply_3
        ...
        # pd_op.scale: (1x38x1xf32) <- (1x38x1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(max_0, full_9, float("1e-09"), True)
        del full_9, max_0

        # pd_op.divide: (1x38x24276xf32) <- (1x38x24276xf32, 1x38x1xf32)
        divide_0 = paddle._C_ops.divide(multiply_2, scale_1)
        del multiply_2, scale_1

        # pd_op.multiply: (1x38x24276xf32) <- (1x38x24276xf32, 1x38x1xf32)
        multiply_4 = paddle._C_ops.multiply(divide_0, max_1)
        del divide_0, max_1

        # pd_op.max: (1x24276xf32) <- (1x38x24276xf32, 1xi64)
        max_2 = paddle._C_ops.max(multiply_4, full_int_array_2, False)
        del full_int_array_2, multiply_4

        # pd_op.unsqueeze: (1x24276x1xf32) <- (1x24276xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(max_2, full_int_array_6)
        del full_int_array_6, max_2

        # pd_op.multiply: (1x24276x10xf32) <- (1x24276x10xf32, 1x24276x1xf32)
