...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (8x96x64x64xf32) <- (8x96x64x64xf32, 96x96x3x3xf32)
        ...
        # pd_op.conv2d: (8x4x64x64xf32) <- (8x96x64x64xf32, 4x96x1x1xf32)
        ...
        # pd_op.add: (8x4x64x64xf32) <- (8x4x64x64xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x5x64x64xf32) <- (8x96x64x64xf32, 5x96x1x1xf32)
        ...
        # pd_op.add: (8x5x64x64xf32) <- (8x5x64x64xf32, 1x5x1x1xf32)
        add_3 = paddle._C_ops.add(conv2d_6, reshape_1)

        # pd_op.sigmoid: (8x4x64x64xf32) <- (8x4x64x64xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_2)

        # pd_op.flatten: (8x4x4096xf32) <- (8x4x64x64xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (8x4096x4xf32) <- (8x4x4096xf32)
        ...
        # pd_op.split: ([8x4x64x64xf32, 8x1x64x64xf32]) <- (8x5x64x64xf32, 2xi64, 1xi32)
        split_4 = paddle._C_ops.split(add_3, full_int_array_1, full_0)

        # builtin.split: (8x4x64x64xf32, 8x1x64x64xf32) <- ([8x4x64x64xf32, 8x1x64x64xf32])
        (
            split_0,
            split_5,
        ) = split_4
        ...
        # pd_op.sigmoid: (8x1x64x64xf32) <- (8x1x64x64xf32)
        sigmoid_6 = paddle._C_ops.sigmoid(split_5)

        # pd_op.flatten: (8x1x4096xf32) <- (8x1x64x64xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_6, 2, 3)

        # pd_op.transpose: (8x4096x1xf32) <- (8x1x4096xf32)
        ...
        # pd_op.conv2d: (8x4x32x32xf32) <- (8x96x32x32xf32, 4x96x1x1xf32)
        ...
        # pd_op.add: (8x4x32x32xf32) <- (8x4x32x32xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x5x32x32xf32) <- (8x96x32x32xf32, 5x96x1x1xf32)
        ...
        # pd_op.add: (8x5x32x32xf32) <- (8x5x32x32xf32, 1x5x1x1xf32)
        add_5 = paddle._C_ops.add(conv2d_13, reshape_3)

        # pd_op.sigmoid: (8x4x32x32xf32) <- (8x4x32x32xf32)
        sigmoid_12 = paddle._C_ops.sigmoid(add_4)

        # pd_op.flatten: (8x4x1024xf32) <- (8x4x32x32xf32)
        flatten_3 = paddle._C_ops.flatten(sigmoid_12, 2, 3)

        # pd_op.transpose: (8x1024x4xf32) <- (8x4x1024xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])

        # pd_op.split: ([8x4x32x32xf32, 8x1x32x32xf32]) <- (8x5x32x32xf32, 2xi64, 1xi32)
        split_6 = paddle._C_ops.split(add_5, full_int_array_1, full_0)

        # builtin.split: (8x4x32x32xf32, 8x1x32x32xf32) <- ([8x4x32x32xf32, 8x1x32x32xf32])
        (
            split_1,
            split_7,
        ) = split_6
        ...
        # pd_op.sigmoid: (8x1x32x32xf32) <- (8x1x32x32xf32)
        sigmoid_13 = paddle._C_ops.sigmoid(split_7)

        # pd_op.flatten: (8x1x1024xf32) <- (8x1x32x32xf32)
        flatten_5 = paddle._C_ops.flatten(sigmoid_13, 2, 3)

        # pd_op.transpose: (8x1024x1xf32) <- (8x1x1024xf32)
        transpose_5 = paddle._C_ops.transpose(flatten_5, [0, 2, 1])

        # pd_op.conv2d: (8x96x16x16xf32) <- (8x384x16x16xf32, 96x384x1x1xf32)
        conv2d_14 = paddle._C_ops.conv2d(
            data_2, parameter_28, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (8x96x16x16xf32, 96xf32, 96xf32, 96xf32, 96xf32, -1xui8) <- (8x96x16x16xf32, 96xf32, 96xf32, 96xf32, 96xf32)
        (
            batch_norm__60,
            batch_norm__61,
            batch_norm__62,
        ...
        # pd_op.sigmoid: (8x96x16x16xf32) <- (8x96x16x16xf32)
        sigmoid_14 = paddle._C_ops.sigmoid(batch_norm__60)

        # pd_op.multiply: (8x96x16x16xf32) <- (8x96x16x16xf32, 8x96x16x16xf32)
        multiply_10 = paddle._C_ops.multiply(batch_norm__60, sigmoid_14)

        # pd_op.conv2d: (8x96x16x16xf32) <- (8x96x16x16xf32, 96x96x3x3xf32)
        ...
        # pd_op.conv2d: (8x4x16x16xf32) <- (8x96x16x16xf32, 4x96x1x1xf32)
        ...
        # pd_op.add: (8x4x16x16xf32) <- (8x4x16x16xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (8x5x16x16xf32) <- (8x96x16x16xf32, 5x96x1x1xf32)
        ...
        # pd_op.add: (8x5x16x16xf32) <- (8x5x16x16xf32, 1x5x1x1xf32)
        add_7 = paddle._C_ops.add(conv2d_20, reshape_5)

        # pd_op.sigmoid: (8x4x16x16xf32) <- (8x4x16x16xf32)
        sigmoid_19 = paddle._C_ops.sigmoid(add_6)

        # pd_op.flatten: (8x4x256xf32) <- (8x4x16x16xf32)
        flatten_6 = paddle._C_ops.flatten(sigmoid_19, 2, 3)

        # pd_op.transpose: (8x256x4xf32) <- (8x4x256xf32)
        transpose_6 = paddle._C_ops.transpose(flatten_6, [0, 2, 1])

        # pd_op.split: ([8x4x16x16xf32, 8x1x16x16xf32]) <- (8x5x16x16xf32, 2xi64, 1xi32)
        split_8 = paddle._C_ops.split(add_7, full_int_array_1, full_0)

        # builtin.split: (8x4x16x16xf32, 8x1x16x16xf32) <- ([8x4x16x16xf32, 8x1x16x16xf32])
        (
            split_2,
            split_9,
        ) = split_8
        ...
        # pd_op.sigmoid: (8x1x16x16xf32) <- (8x1x16x16xf32)
        sigmoid_20 = paddle._C_ops.sigmoid(split_9)

        # pd_op.flatten: (8x1x256xf32) <- (8x1x16x16xf32)
        flatten_8 = paddle._C_ops.flatten(sigmoid_20, 2, 3)

        # pd_op.transpose: (8x256x1xf32) <- (8x1x256xf32)
        transpose_8 = paddle._C_ops.transpose(flatten_8, [0, 2, 1])

        # builtin.combine: ([8x4096x4xf32, 8x1024x4xf32, 8x256x4xf32]) <- (8x4096x4xf32, 8x1024x4xf32, 8x256x4xf32)
        combine_0 = [transpose_0, transpose_3, transpose_6]

        # pd_op.concat: (8x5376x4xf32) <- ([8x4096x4xf32, 8x1024x4xf32, 8x256x4xf32], 1xi32)
        ...
        # builtin.combine: ([8x4096x1xf32, 8x1024x1xf32, 8x256x1xf32]) <- (8x4096x1xf32, 8x1024x1xf32, 8x256x1xf32)
        combine_2 = [transpose_2, transpose_5, transpose_8]

        # pd_op.concat: (8x5376x1xf32) <- ([8x4096x1xf32, 8x1024x1xf32, 8x256x1xf32], 1xi32)
        ...
        # pd_op.split_with_num: ([8x5376x2xf32, 8x5376x2xf32]) <- (8x5376x4xf32, 1xi32)
        split_with_num_0 = paddle._C_ops.split_with_num(concat_4, 2, full_1)

        # builtin.split: (8x5376x2xf32, 8x5376x2xf32) <- ([8x5376x2xf32, 8x5376x2xf32])
        (
            split_3,
            split_16,
        ) = split_with_num_0
        ...
        # pd_op.add: (8x5376x2xf32) <- (8x5376x2xf32, 5376x2xf32)
        add_0 = paddle._C_ops.add(split_3, divide_0)

        # pd_op.exp: (8x5376x2xf32) <- (8x5376x2xf32)
        ...
        # pd_op.scale: (8x5376x2xf32) <- (8x5376x2xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(exp_0, full_2, float("0"), True)

        # pd_op.subtract: (8x5376x2xf32) <- (8x5376x2xf32, 8x5376x2xf32)
        subtract_0 = paddle._C_ops.subtract(add_0, scale_0)

        # pd_op.add: (8x5376x2xf32) <- (8x5376x2xf32, 8x5376x2xf32)
        ...
        # builtin.combine: ([8x5376x2xf32, 8x5376x2xf32]) <- (8x5376x2xf32, 8x5376x2xf32)
        combine_11 = [subtract_0, add_1]

        # pd_op.concat: (8x5376x4xf32) <- ([8x5376x2xf32, 8x5376x2xf32], 1xi32)
