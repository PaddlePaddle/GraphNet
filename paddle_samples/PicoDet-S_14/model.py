...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.unsqueeze: (48x1x4xf32) <- (48x4xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(reshape_2, full_int_array_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [0]

        # pd_op.unsqueeze: (1x1726x4xf32) <- (1726x4xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(data_0, full_int_array_2)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_3 = [2]

        # pd_op.slice: (48x1x2xf32) <- (48x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.slice: (1x1726x2xf32) <- (1x1726x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (48x1726x2xf32) <- (48x1x2xf32, 1x1726x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (48x1726x2xf32) <- (48x1x2xf32, 1x1726x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (48x1726x2xf32) <- (48x1726x2xf32, 48x1726x2xf32)
        ...
        # pd_op.scale: (48x1726xf32) <- (48x1726xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_3, full_2, float("1e-10"), True)

        # pd_op.divide: (48x1726xf32) <- (48x1726xf32, 48x1726xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_0)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_6 = [16, -1, 1726]

        # pd_op.reshape: (16x3x1726xf32) <- (48x1726xf32, 3xi64)
        reshape_1 = paddle._C_ops.reshape(divide_0, full_int_array_6)

        # pd_op.slice: (48xf32) <- (48x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.add: (48xf32) <- (48xf32, 48xf32)
        ...
        # pd_op.stack: (48x2xf32) <- ([48xf32, 48xf32])
        stack_1 = paddle._C_ops.stack(combine_0, -1)

        # pd_op.unsqueeze: (48x1x2xf32) <- (48x2xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(stack_1, full_int_array_1)

        # pd_op.slice: (1726xf32) <- (1726x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.add: (1726xf32) <- (1726xf32, 1726xf32)
        add_3 = paddle._C_ops.add(slice_8, slice_9)

        # pd_op.scale: (1726xf32) <- (1726xf32, 1xf32)
        ...
        # builtin.combine: ([1726xf32, 1726xf32]) <- (1726xf32, 1726xf32)
        combine_1 = [scale_3, scale_4]

        # pd_op.stack: (1726x2xf32) <- ([1726xf32, 1726xf32])
        stack_0 = paddle._C_ops.stack(combine_1, -1)

        # pd_op.unsqueeze: (1x1726x2xf32) <- (1726x2xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(stack_0, full_int_array_2)

        # pd_op.subtract: (48x1726x2xf32) <- (48x1x2xf32, 1x1726x2xf32)
        subtract_4 = paddle._C_ops.subtract(unsqueeze_2, unsqueeze_3)

        # pd_op.p_norm: (48x1726xf32) <- (48x1726x2xf32)
