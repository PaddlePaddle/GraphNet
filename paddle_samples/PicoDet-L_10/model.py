...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6, data_7):
        ...
        # pd_op.max: (82280x1xf32) <- (82280x4xf32, 1xi64)
        ...
        # pd_op.clip: (85xf32) <- (85xf32, 1xf32, 1xf32)
        ...
        # builtin.combine: ([85xf32, 85xf32, 85xf32, 85xf32]) <- (85xf32, 85xf32, 85xf32, 85xf32)
        combine_0 = [clip_2, clip_3, clip_4, clip_5]

        # pd_op.stack: (85x4xf32) <- ([85xf32, 85xf32, 85xf32, 85xf32])
        ...
        # pd_op.reshape: (340xf32) <- (85x4xf32, 1xi64)
        ...
        # pd_op.split_with_num: ([85x1xf32, 85x1xf32, 85x1xf32, 85x1xf32]) <- (85x4xf32, 1xi32)
        split_with_num_0 = paddle._C_ops.split_with_num(gather_3, 4, full_0)

        # builtin.split: (85x1xf32, 85x1xf32, 85x1xf32, 85x1xf32) <- ([85x1xf32, 85x1xf32, 85x1xf32, 85x1xf32])
        (
            split_0,
            split_1,
            split_2,
        ...
        # pd_op.clip: (85x1xf32) <- (85x1xf32, 1xf32, 1xf32)
        ...
        # pd_op.multiply: (85x1xf32) <- (85x1xf32, 85x1xf32)
        ...
        # pd_op.add: (85x1xf32) <- (85x1xf32, 85x1xf32)
        ...
        # pd_op.scale: (85x1xf32) <- (85x1xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_16, full_2, float("1e-10"), True)

        # pd_op.divide: (85x1xf32) <- (85x1xf32, 85x1xf32)
        ...
        # pd_op.unsqueeze: (340x1xi64) <- (340xi64, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(cast_2, full_int_array_7)

        # pd_op.cross_entropy_with_softmax: (340x8xf32, 340x1xf32) <- (340x8xf32, 340x1xi64)
        ...
        # pd_op.add: (340xf32) <- (340xf32, 340xf32)
        ...
        # pd_op.sum: (xf32) <- (82280x4xf32, 0xi64)
        sum_1 = paddle._C_ops.sum(data_0, full_int_array_0, None, False)

        # pd_op.cast: (82280x4xf32) <- (82280x4xf32)
        ...
        # pd_op.greater_than: (82280x4xb) <- (82280x4xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(cast_0, full_11)

        # pd_op.cast: (82280x4xf32) <- (82280x4xb)
        cast_5 = paddle._C_ops.cast(greater_than_0, paddle.float32)

        # pd_op.multiply: (82280x4xf32) <- (82280x4xf32, 82280x4xf32)
        multiply_7 = paddle._C_ops.multiply(cast_0, cast_5)

        # pd_op.subtract: (82280x4xf32) <- (82280x4xf32, 82280x4xf32)
        subtract_9 = paddle._C_ops.subtract(data_1, cast_0)

        # pd_op.abs: (82280x4xf32) <- (82280x4xf32)
        abs_0 = paddle._C_ops.abs(subtract_9)

        # pd_op.pow: (82280x4xf32) <- (82280x4xf32)
        ...
        # pd_op.scale: (82280x4xf32) <- (82280x4xf32, 1xf32)
        scale_5 = paddle._C_ops.scale(pow_0, full_7, float("0"), True)

        # pd_op.less_equal: (82280x4xb) <- (82280x4xf32, xf32)
