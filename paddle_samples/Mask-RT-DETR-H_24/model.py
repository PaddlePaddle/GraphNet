...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.unsqueeze: (1x64x64x2xf32) <- (64x64x2xf32, 1xi64)
        ...
        # pd_op.scale: (1x64x64x2xf32) <- (1x64x64x2xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(unsqueeze_0, full_4, float("0.5"), True)

        # pd_op.divide: (1x64x64x2xf32) <- (1x64x64x2xf32, 2xf32)
        divide_0 = paddle._C_ops.divide(scale_0, cast_1)

        # pd_op.full_like: (1x64x64x2xf32) <- (1x64x64x2xf32, 1xf32)
        ...
        # builtin.combine: ([1x64x64x2xf32, 1x64x64x2xf32]) <- (1x64x64x2xf32, 1x64x64x2xf32)
        combine_2 = [divide_0, scale_2]

        # pd_op.concat: (1x64x64x4xf32) <- ([1x64x64x2xf32, 1x64x64x2xf32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_2, full_6)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_1 = [-1, 4096, 4]

        # pd_op.reshape: (1x4096x4xf32) <- (1x64x64x4xf32, 3xi64)
        ...
        # pd_op.unsqueeze: (1x32x32x2xf32) <- (32x32x2xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(stack_2, full_int_array_0)

        # pd_op.scale: (1x32x32x2xf32) <- (1x32x32x2xf32, 1xf32)
        scale_3 = paddle._C_ops.scale(unsqueeze_1, full_4, float("0.5"), True)

        # pd_op.divide: (1x32x32x2xf32) <- (1x32x32x2xf32, 2xf32)
        divide_1 = paddle._C_ops.divide(scale_3, cast_2)

        # pd_op.full_like: (1x32x32x2xf32) <- (1x32x32x2xf32, 1xf32)
        ...
        # builtin.combine: ([1x32x32x2xf32, 1x32x32x2xf32]) <- (1x32x32x2xf32, 1x32x32x2xf32)
        combine_5 = [divide_1, scale_5]

        # pd_op.concat: (1x32x32x4xf32) <- ([1x32x32x2xf32, 1x32x32x2xf32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_5, full_6)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [-1, 1024, 4]

        # pd_op.reshape: (1x1024x4xf32) <- (1x32x32x4xf32, 3xi64)
        ...
        # pd_op.unsqueeze: (1x16x16x2xf32) <- (16x16x2xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(stack_3, full_int_array_0)

        # pd_op.scale: (1x16x16x2xf32) <- (1x16x16x2xf32, 1xf32)
        scale_6 = paddle._C_ops.scale(unsqueeze_2, full_4, float("0.5"), True)

        # pd_op.divide: (1x16x16x2xf32) <- (1x16x16x2xf32, 2xf32)
        divide_2 = paddle._C_ops.divide(scale_6, cast_3)

        # pd_op.full_like: (1x16x16x2xf32) <- (1x16x16x2xf32, 1xf32)
        ...
        # builtin.combine: ([1x16x16x2xf32, 1x16x16x2xf32]) <- (1x16x16x2xf32, 1x16x16x2xf32)
        combine_8 = [divide_2, scale_8]

        # pd_op.concat: (1x16x16x4xf32) <- ([1x16x16x2xf32, 1x16x16x2xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_8, full_6)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [-1, 256, 4]

        # pd_op.reshape: (1x256x4xf32) <- (1x16x16x4xf32, 3xi64)
        ...
        # builtin.combine: ([1x4096x4xf32, 1x1024x4xf32, 1x256x4xf32]) <- (1x4096x4xf32, 1x1024x4xf32, 1x256x4xf32)
        combine_9 = [reshape_2, reshape_3, reshape_4]

        # pd_op.concat: (1x5376x4xf32) <- ([1x4096x4xf32, 1x1024x4xf32, 1x256x4xf32], 1xi32)
        ...
        # pd_op.greater_than: (1x5376x4xb) <- (1x5376x4xf32, xf32)
        ...
        # pd_op.less_than: (1x5376x4xb) <- (1x5376x4xf32, xf32)
        less_than_0 = paddle._C_ops.less_than(concat_4, full_15)

        # pd_op.multiply: (1x5376x4xb) <- (1x5376x4xb, 1x5376x4xb)
        multiply_0 = paddle._C_ops.multiply(greater_than_0, less_than_0)

        # pd_op.all: (1x5376x1xb) <- (1x5376x4xb)
        ...
        # pd_op.scale: (1x5376x4xf32) <- (1x5376x4xf32, 1xf32)
        scale_9 = paddle._C_ops.scale(concat_4, full_16, float("1"), True)

        # pd_op.divide: (1x5376x4xf32) <- (1x5376x4xf32, 1x5376x4xf32)
        divide_3 = paddle._C_ops.divide(concat_4, scale_9)

        # pd_op.log: (1x5376x4xf32) <- (1x5376x4xf32)
        ...
        # pd_op.full_like: (1x5376x4xf32) <- (1x5376x4xf32, 1xf32)
        ...
        # pd_op.full_like: (1x5376x256xf32) <- (1x5376x256xf32, 1xf32)
        ...
        # pd_op.add: (1x5376x256xf32) <- (1x5376x256xf32, xf32)
        add_15 = paddle._C_ops.add(full_like_6, full_like_7)

        # pd_op.add: (1x5376x256xf32) <- (1x5376x256xf32, 1x5376x1xf32)
        add_0 = paddle._C_ops.add(add_15, cast_7)

        # pd_op.add: (1x5376x256xf32) <- (1x5376x256xf32, 1x5376x256xf32)
        add_1 = paddle._C_ops.add(data_0, add_0)

        # pd_op.add: (1x5376x256xf32) <- (xf32, 1x5376x256xf32)
        add_2 = paddle._C_ops.add(assign_value__0, add_0)

        # pd_op.add: (1x5376x256xf32) <- (1x5376x1xf32, 1x5376x256xf32)
        add_16 = paddle._C_ops.add(cast_8, add_0)

        # pd_op.cast: (1x5376x256xb) <- (1x5376x256xf32)
        cast_0 = paddle._C_ops.cast(add_16, paddle.bool)

        # pd_op.where: (1x5376x256xf32) <- (1x5376x256xb, 1x5376x256xf32, 1x5376x256xf32)
        where_1 = paddle._C_ops.where(cast_0, add_1, add_2)

        # pd_op.matmul: (1x5376x256xf32) <- (1x5376x256xf32, 256x256xf32)
        matmul_0 = paddle._C_ops.matmul(where_1, parameter_19, False, False)

        # pd_op.add: (1x5376x256xf32) <- (1x5376x256xf32, 256xf32)
        add_3 = paddle._C_ops.add(matmul_0, parameter_18)

        # pd_op.layer_norm: (1x5376x256xf32, 1x5376xf32, 1x5376xf32) <- (1x5376x256xf32, 256xf32, 256xf32)
        ...
        # pd_op.matmul: (1x5376x2xf32) <- (1x5376x256xf32, 256x2xf32)
        matmul_12 = paddle._C_ops.matmul(layer_norm_0, parameter_15, False, False)

        # pd_op.add: (1x5376x2xf32) <- (1x5376x2xf32, 2xf32)
        ...
        # pd_op.relu: (1x5376x256xf32) <- (1x5376x256xf32)
        ...
        # pd_op.matmul: (1x5376x4xf32) <- (1x5376x256xf32, 256x4xf32)
        matmul_3 = paddle._C_ops.matmul(relu_1, parameter_9, False, False)

        # pd_op.add: (1x5376x4xf32) <- (1x5376x4xf32, 4xf32)
        ...
        # pd_op.max: (1x5376xf32) <- (1x5376x2xf32, 1xi64)
        ...
        # pd_op.topk: (1x300xf32, 1x300xi64) <- (1x5376xf32, 1xi32)
        ...
        # pd_op.gather_nd: (1x300x256xf32) <- (1x5376x256xf32, 1x300x2xi64)
        gather_nd_0 = paddle._C_ops.gather_nd(layer_norm_0, stack_0)

        # pd_op.gather_nd: (1x300x4xf32) <- (1x5376x4xf32, 1x300x2xi64)
        ...
        # pd_op.flatten: (1x128x16384xf32) <- (1x128x128x128xf32)
        flatten_0 = paddle._C_ops.flatten(data_1, 2, 3)

        # pd_op.assign: (1x128x16384xf32) <- (1x128x16384xf32)
        assign_1 = flatten_0

        # pd_op.bmm: (1x300x16384xf32) <- (1x300x128xf32, 1x128x16384xf32)
        bmm_0 = paddle._C_ops.bmm(add_6, flatten_0)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_6 = [1, 300, 128, 128]

        # pd_op.reshape: (1x300x128x128xf32) <- (1x300x16384xf32, 4xi64)
        ...
        # pd_op.greater_than: (1x300x128x128xb) <- (1x300x128x128xf32, xf32)
        ...
        # pd_op.cast: (1x300x128x128xf32) <- (1x300x128x128xb)
        cast_9 = paddle._C_ops.cast(greater_than_1, paddle.float32)

        # pd_op.multiply: (1x300x128x128xf32) <- (128x128xf32, 1x300x128x128xf32)
        multiply_1 = paddle._C_ops.multiply(split_7, cast_9)

        # pd_op.flatten: (1x300x16384xf32) <- (1x300x128x128xf32)
        flatten_1 = paddle._C_ops.flatten(multiply_1, 2, 3)

        # pd_op.max: (1x300xf32) <- (1x300x16384xf32, 1xi64)
        ...
        # pd_op.full_like: (1x300x128x128xf32) <- (1x300x128x128xf32, 1xf32)
        ...
        # pd_op.full_like: (1x300x128x128xb) <- (1x300x128x128xb, 1xf32)
        ...
        # pd_op.any: (1x300xb) <- (1x300x128x128xb)
        ...
        assign_value__7 = paddle._C_ops.assign_value_(
            ...
            [4],
            paddle.int64,
            [float("128"), float("128"), float("128"), float("128")],
            paddle.framework._current_expected_place(),
        )
        ...
        # pd_op.bmm: (1x400x16384xf32) <- (1x400x128xf32, 1x128x16384xf32)
        bmm_1 = paddle._C_ops.bmm(add_7, flatten_0)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_8 = [1, 400, 128, 128]

        # pd_op.reshape: (1x400x128x128xf32) <- (1x400x16384xf32, 4xi64)
