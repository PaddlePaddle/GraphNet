...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.unsqueeze: (2x1x12096xf32) <- (2x12096xf32, 1xi64)
        ...
        # pd_op.greater_than: (2x1x12096xb) <- (2x1x12096xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(unsqueeze_0, full_0)
        del full_0, unsqueeze_0

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_1 = [1, 11, 1]

        # pd_op.tile: (2x11x12096xb) <- (2x1x12096xb, 3xi64)
        ...
        # pd_op.argmax: (2x12096xi64) <- (2x11x12096xf32, 1xi64)
        ...
        # pd_op.one_hot: (2x12096x11xf32) <- (2x12096xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )
        del argmax_0, full_2

        # pd_op.transpose: (2x11x12096xf32) <- (2x12096x11xf32)
        transpose_0 = paddle._C_ops.transpose(one_hot_0, [0, 2, 1])
        del one_hot_0

        # pd_op.where: (2x11x12096xf32) <- (2x11x12096xb, 2x11x12096xf32, 2x11x12096xf32)
        ...
        # pd_op.sum: (2x12096xf32) <- (2x11x12096xf32, 1xi64)
        ...
        # pd_op.add: (2x12096xi64) <- (2x12096xi64, 2x1xi64)
        add_0 = paddle._C_ops.add(argmax_1, cast_0)
        del argmax_1, cast_0

        # pd_op.flatten: (22xi32) <- (2x11x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_4, 0, 2)
        del data_4

        # pd_op.flatten: (24192xi64) <- (2x12096xi64)
        ...
        # pd_op.gather: (24192xi32) <- (22xi32, 24192xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_3 = [2, 12096]

        # pd_op.reshape: (2x12096xi32) <- (24192xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x12096xb) <- (2x12096xf32, xf32)
        ...
        # pd_op.full_like: (2x12096xi32) <- (2x12096xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_6

        # pd_op.where: (2x12096xi32) <- (2x12096xb, 2x12096xi32, 2x12096xi32)
        ...
        # pd_op.one_hot: (2x12096x5xf32) <- (2x12096xi32, 1xi32)
        ...
        # pd_op.index_select: (2x12096x4xf32) <- (2x12096x5xf32, 4xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_1, assign_value__0, -1)
        del assign_value__0, one_hot_1

        # pd_op.multiply: (2x11x12096xf32) <- (2x11x12096xf32, 2x11x12096xf32)
        ...
        # pd_op.max: (2x11x1xf32) <- (2x11x12096xf32, 1xi64)
