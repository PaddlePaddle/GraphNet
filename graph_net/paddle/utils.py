...
def get_input_dict(model_path):
    inputs_params = load_converted_from_text(f"{model_path}")
    params = inputs_params["weight_info"]
    inputs = inputs_params["input_info"]

    params.update(inputs)
    state_dict = {k: replay_tensor(v) for k, v in params.items()}
    return state_dict


def get_input_spec(model_path):
    inputs_params_list = load_converted_list_from_text(f"{model_path}")
