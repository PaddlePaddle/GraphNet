...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.distribute_fpn_proposals: ([-1x4xf32, -1x4xf32, -1x4xf32, -1x4xf32], [-1xi32, -1xi32, -1xi32, -1xi32], -1x1xi32) <- (10x4xf32, 1xi64)
        (
        ...
        # pd_op.roi_align: (-1x256x14x14xf32) <- (1x256x168x256xf32, -1x4xf32, -1xi32)
        roi_align_0 = paddle._C_ops.roi_align(
            data_5, split_0, split_4, 14, 14, float("0.25"), 0, True
        )

        # pd_op.roi_align: (-1x256x14x14xf32) <- (1x256x84x128xf32, -1x4xf32, -1xi32)
        roi_align_1 = paddle._C_ops.roi_align(
            data_6, split_1, split_5, 14, 14, float("0.125"), 0, True
        )

        # pd_op.roi_align: (-1x256x14x14xf32) <- (1x256x42x64xf32, -1x4xf32, -1xi32)
        roi_align_2 = paddle._C_ops.roi_align(
            data_7, split_2, split_6, 14, 14, float("0.0625"), 0, True
        )

        # pd_op.roi_align: (-1x256x14x14xf32) <- (1x256x21x32xf32, -1x4xf32, -1xi32)
        ...
        # pd_op.one_hot: (10x2xf32) <- (10xi32, 1xi32)
        ...
        # pd_op.unsqueeze: (10x2x1x1xf32) <- (10x2xf32, 2xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(one_hot_0, full_int_array_3)

        # pd_op.expand_as: (-1x2x28x28xf32) <- (10x2x1x1xf32, -1x2x28x28xf32)
        ...
        # pd_op.cast: (10x28x28xf32) <- (10x28x28xi32)
        cast_0 = paddle._C_ops.cast(data_3, paddle.float32)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_6 = [1, 2]

        # pd_op.unsqueeze: (10x1x1xf32) <- (10xf32, 2xi64)
        ...
        # pd_op.sigmoid_cross_entropy_with_logits: (-1x28x28xf32) <- (-1x28x28xf32, 10x28x28xf32, None)
        ...
        # pd_op.multiply: (10x28x28xf32) <- (-1x28x28xf32, 10x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(
            sigmoid_cross_entropy_with_logits_0, unsqueeze_0
        )

        # pd_op.mean_all: (xf32) <- (10x28x28xf32)
