...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3):
        ...
        # pd_op.slice: (6804xf32) <- (6804x4xf32, 1xi64, 1xi64)
        slice_1 = paddle._C_ops.slice(
            data_1, [1], full_int_array_0, full_int_array_1, [1], [1]
        )

        # pd_op.unsqueeze: (6804x1xf32) <- (6804xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(slice_1, full_int_array_1)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_2 = [1, 3]

        # pd_op.tile: (6804x3xf32) <- (6804x1xf32, 2xi64)
        ...
        # pd_op.slice: (3xf32) <- (3x4xf32, 1xi64, 1xi64)
        slice_5 = paddle._C_ops.slice(
            data_3, [1], full_int_array_0, full_int_array_1, [1], [1]
        )

        # pd_op.subtract: (6804x3xf32) <- (6804x3xf32, 3xf32)
        ...
        # pd_op.subtract: (6804x3xf32) <- (3xf32, 6804x3xf32)
        ...
        # builtin.combine: ([6804x3xf32, 6804x3xf32, 6804x3xf32, 6804x3xf32]) <- (6804x3xf32, 6804x3xf32, 6804x3xf32, 6804x3xf32)
        combine_0 = [subtract_0, subtract_1, subtract_2, subtract_3]

        # pd_op.stack: (6804x4x3xf32) <- ([6804x3xf32, 6804x3xf32, 6804x3xf32, 6804x3xf32])
        stack_0 = paddle._C_ops.stack(combine_0, 1)

        # pd_op.min: (6804x3xf32) <- (6804x4x3xf32, 1xi64)
        ...
        # pd_op.greater_than: (6804x3xb) <- (6804x3xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(min_0, full_0)

        # pd_op.sum: (6804xi64) <- (6804x3xb, 1xi64)
        ...
        # pd_op.greater_than: (6804xb) <- (6804xi64, xi64)
        greater_than_1 = paddle._C_ops.greater_than(sum_0, full_1)

        # pd_op.add: (3xf32) <- (3xf32, 3xf32)
        ...
        # pd_op.scale: (6804x3xf32) <- (6804x3xf32, 1xf32)
        ...
        # pd_op.add: (6804x3xf32) <- (3xf32, 6804x3xf32)
        add_2 = paddle._C_ops.add(scale_0, scale_2)

        # pd_op.add: (6804x3xf32) <- (3xf32, 6804x3xf32)
        add_3 = paddle._C_ops.add(scale_1, scale_3)

        # pd_op.subtract: (6804x3xf32) <- (6804x3xf32, 6804x3xf32)
        ...
        # pd_op.cast: (6804x3xi64) <- (6804x3xb)
        ...
        # pd_op.gather: (-1x3xi64) <- (6804x3xi64, -1xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(cast_0, squeeze_0, full_4)

        # pd_op.cast: (-1x3xb) <- (-1x3xi64)
        ...
        # pd_op.logical_and: (-1x3xb) <- (-1x3xb, -1x3xb)
        ...
        # pd_op.gather_nd: (-1x4xf32) <- (6804x4xf32, -1x1xi64)
