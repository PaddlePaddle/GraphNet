...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        matmul_0 = paddle._C_ops.matmul(transpose_0, parameter_42, False, False)
        ...
        scale_2 = paddle._C_ops.scale(max_0, full_3, float("2"), True)
        ...
        slice_8 = paddle._C_ops.slice(data_1, [1], full_int_array_2, stack_0, [-1], [])
        ...
        slice_0 = paddle._C_ops.slice(
            slice_8, [1], full_int_array_2, full_int_array_3, [1], []
        )

        # pd_op.embedding: (4x-1x384xf32) <- (4x-1xi64, 6629x384xf32)
        embedding_0 = paddle._C_ops.embedding(slice_0, parameter_41, 0, False)
        ...
        scale_3 = paddle._C_ops.scale(embedding_0, full_0, float("0"), True)

        # pd_op.transpose: (-1x4x384xf32) <- (4x-1x384xf32)
        transpose_1 = paddle._C_ops.transpose(scale_3, [1, 0, 2])
        ...
        add_12 = paddle._C_ops.add(transpose_1, slice_1)
        ...
        dropout_19, dropout_0 = (lambda x, f: f(x))(
        ...
        transpose_2 = paddle._C_ops.transpose(dropout_19, [1, 0, 2])
        ...
        add_13 = paddle._C_ops.add(full_with_tensor_0, triu_0)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_6 = [0, 1]

        # pd_op.unsqueeze: (1x1x-1x-1xf32) <- (-1x-1xf32, 2xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(add_13, full_int_array_6)

        # pd_op.matmul: (4x-1x1152xf32) <- (4x-1x384xf32, 384x1152xf32)
        matmul_1 = paddle._C_ops.matmul(transpose_2, parameter_40, False, False)

        # pd_op.add: (4x-1x1152xf32) <- (4x-1x1152xf32, 1152xf32)
        add_0 = paddle._C_ops.add(matmul_1, parameter_39)
        ...
        combine_4 = [full_6, slice_10, full_7, full_8, full_9]
        ...
        reshape_4 = paddle._C_ops.reshape(add_0, stack_4)

        # pd_op.transpose: (3x4x12x-1x32xf32) <- (4x-1x3x12x32xf32)
        transpose_3 = paddle._C_ops.transpose(reshape_4, [2, 0, 3, 1, 4])
        ...
        transpose_4 = paddle._C_ops.transpose(slice_11, [0, 1, 3, 2])

        # pd_op.matmul: (4x12x-1x-1xf32) <- (4x12x-1x32xf32, 4x12x32x-1xf32)
        matmul_16 = paddle._C_ops.matmul(slice_2, transpose_4, False, False)
        ...
        scale_0 = paddle._C_ops.scale(matmul_16, full_2, float("0"), True)

        # pd_op.add: (4x12x-1x-1xf32) <- (4x12x-1x-1xf32, 1x1x-1x-1xf32)
        add_14 = paddle._C_ops.add(scale_0, unsqueeze_0)

        # pd_op.softmax: (4x12x-1x-1xf32) <- (4x12x-1x-1xf32)
        softmax_0 = paddle._C_ops.softmax(add_14, -1)

        # pd_op.matmul: (4x12x-1x32xf32) <- (4x12x-1x-1xf32, 4x12x-1x32xf32)
        matmul_17 = paddle._C_ops.matmul(softmax_0, slice_3, False, False)

        # pd_op.transpose: (4x-1x12x32xf32) <- (4x12x-1x32xf32)
        transpose_5 = paddle._C_ops.transpose(matmul_17, [0, 2, 1, 3])
        ...
        combine_5 = [full_6, slice_10, full_10]
        ...
        matmul_2 = paddle._C_ops.matmul(reshape_0, parameter_38, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_15 = paddle._C_ops.add(matmul_2, parameter_37)

        # pd_op.dropout: (4x-1x384xf32, 4x-1x384xui8) <- (4x-1x384xf32, None, 1xf32)
        dropout_1, dropout_2 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_15, None, full_1, False, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        ...
        layer_norm_0, layer_norm_1, layer_norm_2 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_1, parameter_36, parameter_35, float("1e-05"), 2
            ),
        ...
        # pd_op.matmul: (4x-1x384xf32) <- (4x-1x384xf32, 384x384xf32)
        matmul_3 = paddle._C_ops.matmul(layer_norm_0, parameter_34, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_2 = paddle._C_ops.add(matmul_3, parameter_33)

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_6 = [full_6, slice_12, full_8, full_9]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_6 = paddle._C_ops.stack(combine_6, 0)

        # pd_op.reshape: (4x-1x12x32xf32) <- (4x-1x384xf32, 4xi64)
        reshape_5 = paddle._C_ops.reshape(add_2, stack_6)
        ...
        matmul_4 = paddle._C_ops.matmul(matmul_0, parameter_32, False, False)

        # pd_op.add: (4x40x768xf32) <- (4x40x768xf32, 768xf32)
        add_3 = paddle._C_ops.add(matmul_4, parameter_31)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_7 = [0, 40, 2, 12, 32]

        # pd_op.reshape: (4x40x2x12x32xf32) <- (4x40x768xf32, 5xi64)
        reshape_6 = paddle._C_ops.reshape(add_3, full_int_array_7)

        # pd_op.transpose: (2x4x12x40x32xf32) <- (4x40x2x12x32xf32)
        transpose_7 = paddle._C_ops.transpose(reshape_6, [2, 0, 3, 1, 4])
        ...
        transpose_8 = paddle._C_ops.transpose(slice_13, [0, 1, 3, 2])

        # pd_op.matmul: (4x12x-1x40xf32) <- (4x12x-1x32xf32, 4x12x32x40xf32)
        matmul_18 = paddle._C_ops.matmul(transpose_6, transpose_8, False, False)

        # pd_op.scale: (4x12x-1x40xf32) <- (4x12x-1x40xf32, 1xf32)
        scale_4 = paddle._C_ops.scale(matmul_18, full_2, float("0"), True)

        # pd_op.softmax: (4x12x-1x40xf32) <- (4x12x-1x40xf32)
        softmax_1 = paddle._C_ops.softmax(scale_4, -1)

        # pd_op.matmul: (4x12x-1x32xf32) <- (4x12x-1x40xf32, 4x12x40x32xf32)
        matmul_19 = paddle._C_ops.matmul(softmax_1, slice_4, False, False)

        # pd_op.transpose: (4x-1x12x32xf32) <- (4x12x-1x32xf32)
        transpose_9 = paddle._C_ops.transpose(matmul_19, [0, 2, 1, 3])

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_7 = [full_6, slice_12, full_10]
        ...
        # pd_op.matmul: (4x-1x384xf32) <- (4x-1x384xf32, 384x384xf32)
        matmul_5 = paddle._C_ops.matmul(reshape_1, parameter_30, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_16 = paddle._C_ops.add(matmul_5, parameter_29)

        # pd_op.dropout: (4x-1x384xf32, 4x-1x384xui8) <- (4x-1x384xf32, None, 1xf32)
        dropout_3, dropout_4 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_16, None, full_1, False, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        ...
        layer_norm_3, layer_norm_4, layer_norm_5 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_4, parameter_28, parameter_27, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )

        # pd_op.matmul: (4x-1x1536xf32) <- (4x-1x384xf32, 384x1536xf32)
        matmul_6 = paddle._C_ops.matmul(layer_norm_3, parameter_26, False, False)

        # pd_op.add: (4x-1x1536xf32) <- (4x-1x1536xf32, 1536xf32)
        add_17 = paddle._C_ops.add(matmul_6, parameter_25)
        ...
        matmul_7 = paddle._C_ops.matmul(dropout_5, parameter_24, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_18 = paddle._C_ops.add(matmul_7, parameter_23)

        # pd_op.dropout: (4x-1x384xf32, 4x-1x384xui8) <- (4x-1x384xf32, None, 1xf32)
        dropout_20, dropout_7 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_18, None, full_1, False, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        ...
        dropout_8, dropout_9 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                dropout_20, None, full_1, False, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        ...
        layer_norm_6, layer_norm_7, layer_norm_8 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_5, parameter_22, parameter_21, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.matmul: (4x-1x1152xf32) <- (4x-1x384xf32, 384x1152xf32)
        matmul_8 = paddle._C_ops.matmul(layer_norm_6, parameter_20, False, False)

        # pd_op.add: (4x-1x1152xf32) <- (4x-1x1152xf32, 1152xf32)
        add_6 = paddle._C_ops.add(matmul_8, parameter_19)

        # builtin.combine: ([xi64, xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64, xi64)
        combine_8 = [full_6, slice_14, full_7, full_8, full_9]

        # pd_op.stack: (5xi64) <- ([xi64, xi64, xi64, xi64, xi64])
        stack_8 = paddle._C_ops.stack(combine_8, 0)

        # pd_op.reshape: (4x-1x3x12x32xf32) <- (4x-1x1152xf32, 5xi64)
        reshape_7 = paddle._C_ops.reshape(add_6, stack_8)
        ...
        # pd_op.matmul: (4x12x-1x-1xf32) <- (4x12x-1x32xf32, 4x12x32x-1xf32)
        matmul_20 = paddle._C_ops.matmul(slice_5, transpose_11, False, False)

        # pd_op.scale: (4x12x-1x-1xf32) <- (4x12x-1x-1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(matmul_20, full_2, float("0"), True)

        # pd_op.add: (4x12x-1x-1xf32) <- (4x12x-1x-1xf32, 1x1x-1x-1xf32)
        add_19 = paddle._C_ops.add(scale_1, unsqueeze_0)

        # pd_op.softmax: (4x12x-1x-1xf32) <- (4x12x-1x-1xf32)
        softmax_2 = paddle._C_ops.softmax(add_19, -1)

        # pd_op.matmul: (4x12x-1x32xf32) <- (4x12x-1x-1xf32, 4x12x-1x32xf32)
        matmul_21 = paddle._C_ops.matmul(softmax_2, slice_6, False, False)

        # pd_op.transpose: (4x-1x12x32xf32) <- (4x12x-1x32xf32)
        transpose_12 = paddle._C_ops.transpose(matmul_21, [0, 2, 1, 3])

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_9 = [full_6, slice_14, full_10]
        ...
        # pd_op.matmul: (4x-1x384xf32) <- (4x-1x384xf32, 384x384xf32)
        matmul_9 = paddle._C_ops.matmul(reshape_2, parameter_18, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_20 = paddle._C_ops.add(matmul_9, parameter_17)

        # pd_op.dropout: (4x-1x384xf32, 4x-1x384xui8) <- (4x-1x384xf32, None, 1xf32)
        dropout_10, dropout_11 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_20, None, full_1, False, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        ...
        layer_norm_9, layer_norm_10, layer_norm_11 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_7, parameter_16, parameter_15, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )
        ...
        # pd_op.matmul: (4x-1x384xf32) <- (4x-1x384xf32, 384x384xf32)
        matmul_10 = paddle._C_ops.matmul(layer_norm_9, parameter_14, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_8 = paddle._C_ops.add(matmul_10, parameter_13)

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_10 = [full_6, slice_16, full_8, full_9]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_10 = paddle._C_ops.stack(combine_10, 0)

        # pd_op.reshape: (4x-1x12x32xf32) <- (4x-1x384xf32, 4xi64)
        reshape_8 = paddle._C_ops.reshape(add_8, stack_10)

        # pd_op.transpose: (4x12x-1x32xf32) <- (4x-1x12x32xf32)
        transpose_13 = paddle._C_ops.transpose(reshape_8, [0, 2, 1, 3])

        # pd_op.matmul: (4x40x768xf32) <- (4x40x384xf32, 384x768xf32)
        matmul_11 = paddle._C_ops.matmul(matmul_0, parameter_12, False, False)

        # pd_op.add: (4x40x768xf32) <- (4x40x768xf32, 768xf32)
        add_9 = paddle._C_ops.add(matmul_11, parameter_11)

        # pd_op.reshape: (4x40x2x12x32xf32) <- (4x40x768xf32, 5xi64)
        reshape_9 = paddle._C_ops.reshape(add_9, full_int_array_7)

        # pd_op.transpose: (2x4x12x40x32xf32) <- (4x40x2x12x32xf32)
        transpose_14 = paddle._C_ops.transpose(reshape_9, [2, 0, 3, 1, 4])
        ...
        # pd_op.matmul: (4x12x-1x40xf32) <- (4x12x-1x32xf32, 4x12x32x40xf32)
        matmul_22 = paddle._C_ops.matmul(transpose_13, transpose_15, False, False)

        # pd_op.scale: (4x12x-1x40xf32) <- (4x12x-1x40xf32, 1xf32)
        scale_5 = paddle._C_ops.scale(matmul_22, full_2, float("0"), True)

        # pd_op.softmax: (4x12x-1x40xf32) <- (4x12x-1x40xf32)
        softmax_3 = paddle._C_ops.softmax(scale_5, -1)

        # pd_op.matmul: (4x12x-1x32xf32) <- (4x12x-1x40xf32, 4x12x40x32xf32)
        matmul_23 = paddle._C_ops.matmul(softmax_3, slice_7, False, False)

        # pd_op.transpose: (4x-1x12x32xf32) <- (4x12x-1x32xf32)
        transpose_16 = paddle._C_ops.transpose(matmul_23, [0, 2, 1, 3])

        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_11 = [full_6, slice_16, full_10]
        ...
        # pd_op.matmul: (4x-1x384xf32) <- (4x-1x384xf32, 384x384xf32)
        matmul_12 = paddle._C_ops.matmul(reshape_3, parameter_10, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_21 = paddle._C_ops.add(matmul_12, parameter_9)

        # pd_op.dropout: (4x-1x384xf32, 4x-1x384xui8) <- (4x-1x384xf32, None, 1xf32)
        dropout_12, dropout_13 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_21, None, full_1, False, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        ...
        layer_norm_12, layer_norm_13, layer_norm_14 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_10, parameter_8, parameter_7, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )

        # pd_op.matmul: (4x-1x1536xf32) <- (4x-1x384xf32, 384x1536xf32)
        matmul_13 = paddle._C_ops.matmul(layer_norm_12, parameter_6, False, False)

        # pd_op.add: (4x-1x1536xf32) <- (4x-1x1536xf32, 1536xf32)
        add_22 = paddle._C_ops.add(matmul_13, parameter_5)

        # pd_op.relu: (4x-1x1536xf32) <- (4x-1x1536xf32)
        relu_1 = paddle._C_ops.relu(add_22)
        ...
        # pd_op.matmul: (4x-1x384xf32) <- (4x-1x1536xf32, 1536x384xf32)
        matmul_14 = paddle._C_ops.matmul(dropout_14, parameter_4, False, False)

        # pd_op.add: (4x-1x384xf32) <- (4x-1x384xf32, 384xf32)
        add_23 = paddle._C_ops.add(matmul_14, parameter_3)

        # pd_op.dropout: (4x-1x384xf32, 4x-1x384xui8) <- (4x-1x384xf32, None, 1xf32)
        dropout_21, dropout_16 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
        ...
        dropout_17, dropout_18 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                dropout_21, None, full_1, False, "upscale_in_train", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )
        ...
        layer_norm_15, layer_norm_16, layer_norm_17 = (lambda x, f: f(x))(
            paddle._C_ops.layer_norm(
                add_11, parameter_2, parameter_1, float("1e-05"), 2
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None, None),
        )

        # pd_op.matmul: (4x-1x6629xf32) <- (4x-1x384xf32, 384x6629xf32)
        matmul_15 = paddle._C_ops.matmul(layer_norm_15, parameter_0, False, False)
        return (
            transpose_0,
            matmul_0,
            slice_0,
