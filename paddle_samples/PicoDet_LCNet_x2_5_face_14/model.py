...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3):
        ...
        # pd_op.slice: (19125xf32) <- (19125x4xf32, 1xi64, 1xi64)
        slice_1 = paddle._C_ops.slice(
            data_1, [1], full_int_array_0, full_int_array_1, [1], [1]
        )

        # pd_op.unsqueeze: (19125x1xf32) <- (19125xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(slice_1, full_int_array_1)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_2 = [1, 73]

        # pd_op.tile: (19125x73xf32) <- (19125x1xf32, 2xi64)
        ...
        # pd_op.slice: (73xf32) <- (73x4xf32, 1xi64, 1xi64)
        slice_5 = paddle._C_ops.slice(
            data_3, [1], full_int_array_0, full_int_array_1, [1], [1]
        )

        # pd_op.subtract: (19125x73xf32) <- (19125x73xf32, 73xf32)
        ...
        # pd_op.subtract: (19125x73xf32) <- (73xf32, 19125x73xf32)
        ...
        # builtin.combine: ([19125x73xf32, 19125x73xf32, 19125x73xf32, 19125x73xf32]) <- (19125x73xf32, 19125x73xf32, 19125x73xf32, 19125x73xf32)
        combine_0 = [subtract_0, subtract_1, subtract_2, subtract_3]

        # pd_op.stack: (19125x4x73xf32) <- ([19125x73xf32, 19125x73xf32, 19125x73xf32, 19125x73xf32])
        stack_0 = paddle._C_ops.stack(combine_0, 1)

        # pd_op.min: (19125x73xf32) <- (19125x4x73xf32, 1xi64)
        ...
        # pd_op.greater_than: (19125x73xb) <- (19125x73xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(min_0, full_0)

        # pd_op.sum: (19125xi64) <- (19125x73xb, 1xi64)
        ...
        # pd_op.greater_than: (19125xb) <- (19125xi64, xi64)
        greater_than_1 = paddle._C_ops.greater_than(sum_0, full_1)

        # pd_op.add: (73xf32) <- (73xf32, 73xf32)
        ...
        # pd_op.scale: (73xf32) <- (73xf32, 1xf32)
        ...
        # pd_op.scale: (19125x73xf32) <- (19125x73xf32, 1xf32)
        ...
        # pd_op.add: (19125x73xf32) <- (73xf32, 19125x73xf32)
        add_2 = paddle._C_ops.add(scale_0, scale_2)

        # pd_op.add: (19125x73xf32) <- (73xf32, 19125x73xf32)
        add_3 = paddle._C_ops.add(scale_1, scale_3)

        # pd_op.subtract: (19125x73xf32) <- (19125x73xf32, 19125x73xf32)
        ...
        # pd_op.logical_or: (19125xb) <- (19125xb, 19125xb)
        logical_or_0 = paddle._C_ops.logical_or(greater_than_1, greater_than_3)

        # pd_op.nonzero: (-1x1xi64) <- (19125xb)
        ...
        # pd_op.cast: (19125x73xi64) <- (19125x73xb)
        ...
        # pd_op.gather: (-1x73xi64) <- (19125x73xi64, -1xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(cast_0, squeeze_0, full_4)

        # pd_op.cast: (-1x73xb) <- (-1x73xi64)
        ...
        # pd_op.logical_and: (-1x73xb) <- (-1x73xb, -1x73xb)
        ...
        # pd_op.gather_nd: (-1x4xf32) <- (19125x4xf32, -1x1xi64)
        gather_nd_0 = paddle._C_ops.gather_nd(data_2, unsqueeze_4)

        # pd_op.gather_nd: (-1x1xf32) <- (19125x1xf32, -1x1xi64)
        gather_nd_1 = paddle._C_ops.gather_nd(data_0, unsqueeze_4)

        # pd_op.shape64: (2xi64) <- (-1x4xf32)
        shape64_0 = paddle._C_ops.shape64(gather_nd_0)

        # pd_op.slice: (xi64) <- (2xi64, 1xi64, 1xi64)
        slice_0 = paddle._C_ops.slice(
            shape64_0, [0], full_int_array_0, full_int_array_1, [1], [0]
        )
        return gather_nd_0, slice_0, gather_nd_1, logical_and_0, logical_or_0
