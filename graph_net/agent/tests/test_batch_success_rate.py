...
def get_models_from_hf(task: Optional[str] = None, limit: int = 100) -> List[str]:
    ...
    try:
        print(f"[INFO] Fetching models from HuggingFace Hub (task={task}, limit={limit})...")
...
def run_batch_test(
    ...
    for idx, model_id in enumerate(model_ids, 1):
        ...
        print(f"[STATS] Current success rate: {current_success_rate:.2f}% ({results['success']}/{idx})")
...
def main():
    ...
    if not workspace:
        ...
        print("   Use --workspace or set GRAPH_NET_EXTRACT_WORKSPACE environment variable")
        sys.exit(1)
