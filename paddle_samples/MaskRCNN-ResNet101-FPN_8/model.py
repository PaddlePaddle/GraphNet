...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # builtin.combine: ([16x1xi64]) <- (16x1xi64)
        combine_0 = [data_0]

        # pd_op.concat: (16x1xi64) <- ([16x1xi64], 1xi32)
        ...
        # builtin.combine: ([16xi32]) <- (16xi32)
        combine_2 = [data_2]

        # pd_op.concat: (16xi32) <- ([16xi32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_2, full_0)

        # builtin.combine: ([16x28x28xi32]) <- (16x28x28xi32)
        combine_3 = [data_3]

        # pd_op.concat: (16x28x28xi32) <- ([16x28x28xi32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_3, full_0)

        # builtin.combine: ([16xf32]) <- (16xf32)
        combine_4 = [data_4]

        # pd_op.concat: (16xf32) <- ([16xf32], 1xi32)
