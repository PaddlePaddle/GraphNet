...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (256x16x112x112xf32) <- (256x8x112x112xf32, 16x8x1x1xf32)
        ...
        t170 = paddle._C_ops.depthwise_conv2d(
            t169, t15, [2, 2], [1, 1], "EXPLICIT", 16, [1, 1], "NCHW"
        )
        del t15
        ...
        # pd_op.conv2d: (256x24x56x56xf32) <- (256x16x56x56xf32, 24x16x1x1xf32)
        ...
        # pd_op.depthwise_conv2d: (256x24x56x56xf32) <- (256x24x56x56xf32, 24x1x3x3xf32)
        t186 = paddle._C_ops.depthwise_conv2d(
            t185, t25, [1, 1], [1, 1], "EXPLICIT", 24, [1, 1], "NCHW"
        )
        del t25
        ...
        t202 = paddle._C_ops.depthwise_conv2d(
            t201, t35, [2, 2], [1, 1], "EXPLICIT", 24, [1, 1], "NCHW"
        )
        del t35
        ...
        # pd_op.hardswish: (256x24x28x28xf32) <- (256x24x28x28xf32)
        t209 = paddle._C_ops.hardswish(t203)

        # pd_op.conv2d: (256x48x28x28xf32) <- (256x24x28x28xf32, 48x24x1x1xf32)
        ...
        t218 = paddle._C_ops.depthwise_conv2d(
            t217, t45, [1, 1], [1, 1], "EXPLICIT", 48, [1, 1], "NCHW"
        )
        del t45
        ...
        # pd_op.conv2d: (256x48x28x28xf32) <- (256x48x28x28xf32, 48x48x1x1xf32)
        ...
        # pd_op.depthwise_conv2d: (256x48x14x14xf32) <- (256x48x28x28xf32, 48x1x3x3xf32)
        t234 = paddle._C_ops.depthwise_conv2d(
            t233, t55, [2, 2], [1, 1], "EXPLICIT", 48, [1, 1], "NCHW"
        )
        del t55
        ...
        # pd_op.conv2d: (256x88x14x14xf32) <- (256x48x14x14xf32, 88x48x1x1xf32)
        ...
        t250 = paddle._C_ops.depthwise_conv2d(
            t249, t65, [1, 1], [2, 2], "EXPLICIT", 88, [1, 1], "NCHW"
        )
        del t65
        ...
        # pd_op.conv2d: (256x88x14x14xf32) <- (256x88x14x14xf32, 88x88x1x1xf32)
        ...
        t266 = paddle._C_ops.depthwise_conv2d(
            t265, t75, [1, 1], [2, 2], "EXPLICIT", 88, [1, 1], "NCHW"
        )
        del t75
        ...
        t282 = paddle._C_ops.depthwise_conv2d(
            t281, t85, [1, 1], [2, 2], "EXPLICIT", 88, [1, 1], "NCHW"
        )
        del t85
        ...
        t298 = paddle._C_ops.depthwise_conv2d(
            t297, t95, [1, 1], [2, 2], "EXPLICIT", 88, [1, 1], "NCHW"
        )
        del t95
        ...
        t314 = paddle._C_ops.depthwise_conv2d(
            t313, t105, [1, 1], [2, 2], "EXPLICIT", 88, [1, 1], "NCHW"
        )
        del t105
        ...
        # pd_op.depthwise_conv2d: (256x88x7x7xf32) <- (256x88x14x14xf32, 88x1x5x5xf32)
        t330 = paddle._C_ops.depthwise_conv2d(
            t329, t115, [2, 2], [2, 2], "EXPLICIT", 88, [1, 1], "NCHW"
        )
        del t115

        # pd_op.batch_norm_: (256x88x7x7xf32, 88xf32, 88xf32, 88xf32, 88xf32, -1xui8) <- (256x88x7x7xf32, 88xf32, 88xf32, 88xf32, 88xf32)
        ...
        # pd_op.hardswish: (256x88x7x7xf32) <- (256x88x7x7xf32)
        ...
        # pd_op.pool2d: (256x88x1x1xf32) <- (256x88x7x7xf32, 2xi64)
        ...
        # pd_op.multiply: (256x88x7x7xf32) <- (256x88x7x7xf32, 256x88x1x1xf32)
        t351 = paddle._C_ops.multiply(t337, t350)

        # pd_op.conv2d: (256x176x7x7xf32) <- (256x88x7x7xf32, 176x88x1x1xf32)
        t352 = paddle._C_ops.conv2d(
            t351, t124, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del t124

        # pd_op.batch_norm_: (256x176x7x7xf32, 176xf32, 176xf32, 176xf32, 176xf32, -1xui8) <- (256x176x7x7xf32, 176xf32, 176xf32, 176xf32, 176xf32)
        ...
        # pd_op.hardswish: (256x176x7x7xf32) <- (256x176x7x7xf32)
        t359 = paddle._C_ops.hardswish(t353)

        # pd_op.depthwise_conv2d: (256x176x7x7xf32) <- (256x176x7x7xf32, 176x1x5x5xf32)
        t360 = paddle._C_ops.depthwise_conv2d(
            t359, t129, [1, 1], [2, 2], "EXPLICIT", 176, [1, 1], "NCHW"
        )
        del t129
        ...
        # pd_op.pool2d: (256x176x1x1xf32) <- (256x176x7x7xf32, 2xi64)
        ...
        # pd_op.conv2d: (256x44x1x1xf32) <- (256x176x1x1xf32, 44x176x1x1xf32)
        ...
        # pd_op.add: (256x44x1x1xf32) <- (256x44x1x1xf32, 1x44x1x1xf32)
        t371 = paddle._C_ops.add(t369, t370)

        # pd_op.relu: (256x44x1x1xf32) <- (256x44x1x1xf32)
        t372 = paddle._C_ops.relu(t371)
        del t371

        # pd_op.conv2d: (256x176x1x1xf32) <- (256x44x1x1xf32, 176x44x1x1xf32)
        ...
        # pd_op.add: (256x176x1x1xf32) <- (256x176x1x1xf32, 1x176x1x1xf32)
        t375 = paddle._C_ops.add(t373, t374)

        # pd_op.hardsigmoid: (256x176x1x1xf32) <- (256x176x1x1xf32)
        t376 = paddle._C_ops.hardsigmoid(t375, float("0.166667"), float("0.5"))
        del t375

        # pd_op.multiply: (256x176x7x7xf32) <- (256x176x7x7xf32, 256x176x1x1xf32)
        t377 = paddle._C_ops.multiply(t367, t376)

        # pd_op.conv2d: (256x176x7x7xf32) <- (256x176x7x7xf32, 176x176x1x1xf32)
        ...
        # pd_op.conv2d: (256x1280x1x1xf32) <- (256x176x1x1xf32, 1280x176x1x1xf32)
