...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0):
        # pd_op.share_data_: (2x168x168xf32) <- (2x168x168xf32)
        share_data__0 = data_0.detach()

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_0 = [2, 1, 37632, 2]
        ...
        # pd_op.uniform: (2x1x37632x2xf32) <- (4xi64, 1xf32, 1xf32)
        ...
        # pd_op.unsqueeze: (2x1x168x168xf32) <- (2x168x168xf32, 1xi64)
        ...
        # pd_op.scale: (2x1x37632x2xf32) <- (2x1x37632x2xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(uniform_0, full_2, float("0"), True)

        # pd_op.scale: (2x1x37632x2xf32) <- (2x1x37632x2xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(scale_0, full_1, float("-1"), True)

        # pd_op.grid_sample: (2x1x1x37632xf32) <- (2x1x168x168xf32, 2x1x37632x2xf32)
        ...
        # pd_op.squeeze: (2x37632xf32) <- (2x1x1x37632xf32, 2xi64)
        squeeze_0 = paddle._C_ops.squeeze(grid_sample_0, full_int_array_2)

        # pd_op.abs: (2x37632xf32) <- (2x37632xf32)
        ...
        # pd_op.scale: (2x37632xf32) <- (2x37632xf32, 1xf32)
        ...
        # pd_op.topk: (2x9408xf32, 2x9408xi64) <- (2x37632xf32, 1xi32)
        ...
        # pd_op.tile: (2x9408xi64) <- (2x1xi64, 2xi64)
        tile_0 = paddle._C_ops.tile(unsqueeze_1, full_int_array_4)

        # builtin.combine: ([2x9408xi64, 2x9408xi64]) <- (2x9408xi64, 2x9408xi64)
        combine_0 = [tile_0, topk_1]

        # pd_op.stack: (2x9408x2xi64) <- ([2x9408xi64, 2x9408xi64])
        stack_0 = paddle._C_ops.stack(combine_0, -1)

        # pd_op.squeeze: (2x37632x2xf32) <- (2x1x37632x2xf32, 1xi64)
        squeeze_1 = paddle._C_ops.squeeze(uniform_0, full_int_array_1)

        # pd_op.gather_nd: (2x9408x2xf32) <- (2x37632x2xf32, 2x9408x2xi64)
        gather_nd_0 = paddle._C_ops.gather_nd(squeeze_1, stack_0)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [2, 3136, 2]

        # pd_op.uniform: (2x3136x2xf32) <- (3xi64, 1xf32, 1xf32)
        ...
        # builtin.combine: ([2x9408x2xf32, 2x3136x2xf32]) <- (2x9408x2xf32, 2x3136x2xf32)
        combine_1 = [gather_nd_0, uniform_1]

        # pd_op.concat: (2x12544x2xf32) <- ([2x9408x2xf32, 2x3136x2xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_1, full_8)
        return concat_0
