...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        slice_1 = paddle._C_ops.slice(
            shape64_0, [0], full_int_array_5, full_int_array_0, [1], [0]
        )
        ...
        # pd_op.shape64: (3xi64) <- (-1x200x304xf32)
        shape64_1 = paddle._C_ops.shape64(sigmoid_1)

        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        slice_2 = paddle._C_ops.slice(
            shape64_1, [0], full_int_array_5, full_int_array_0, [1], [0]
        )
        ...
        combine_0 = [slice_2, full_7]

        # pd_op.stack: (2xi64) <- ([xi64, xi64])
        stack_0 = paddle._C_ops.stack(combine_0, 0)

        # pd_op.reshape: (-1x-1xf32) <- (-1x200x304xf32, 2xi64)
        reshape_2 = paddle._C_ops.reshape(sigmoid_1, stack_0)

        # pd_op.shape64: (3xi64) <- (-1x200x304xf32)
        shape64_2 = paddle._C_ops.shape64(reshape_5)

        # pd_op.slice: (xi64) <- (3xi64, 1xi64, 1xi64)
        slice_3 = paddle._C_ops.slice(
            shape64_2, [0], full_int_array_5, full_int_array_0, [1], [0]
        )

        # builtin.combine: ([xi64, xi64]) <- (xi64, xi64)
        combine_1 = [slice_3, full_7]

        # pd_op.stack: (2xi64) <- ([xi64, xi64])
        stack_1 = paddle._C_ops.stack(combine_1, 0)

        # pd_op.reshape: (-1x-1xf32) <- (-1x200x304xf32, 2xi64)
        reshape_3 = paddle._C_ops.reshape(reshape_5, stack_1)
        ...
        combine_2 = [multiply_2, multiply_5]
        ...
        slice_0 = paddle._C_ops.slice(
            one_hot_0, [1], full_int_array_0, full_int_array_6, [1], []
        )
