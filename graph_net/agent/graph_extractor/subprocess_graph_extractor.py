...
class SubprocessGraphExtractor(BaseGraphExtractor):
    ...
    def extract(self, code_path: Path, model_id: str) -> Path:
        ...
        try:
            ...
            raise ExtractionError(f"Script execution timed out after {self.timeout} seconds")
    ...
    def _find_recent_sample_dir(self, workspace_path: Path) -> Optional[Path]:
        ...
        if candidate_dirs:
            ...
            self.logger.info(f"Found recent directory: {most_recent} (modified {time_diff:.1f}s ago)")
            return most_recent
        
        return None
    
    def _find_dir_by_pattern(self, workspace_path: Path, model_id: str, safe_model_id: str) -> Optional[Path]:
        """Find directory matching model_id patterns"""
        patterns = [safe_model_id, model_id.replace("/", "-"), model_id.replace("/", "_")]
    ...
    def _find_hash_named_dir(self, workspace_path: Path) -> Optional[Path]:
        ...
        for item in workspace_path.iterdir():
            if not item.is_dir():
                continue
            if (len(item.name) == HASH_DIR_LENGTH and
                all(c in '0123456789abcdef' for c in item.name.lower())):
                if self._is_valid_sample_dir(item):
                    try:
                        mtime = item.stat().st_mtime
