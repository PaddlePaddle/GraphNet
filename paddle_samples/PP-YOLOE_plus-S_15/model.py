...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3):
        ...
        # pd_op.topk: (8x1x9xf32, 8x1x9xi64) <- (8x1x484xf32, 1xi32)
        ...
        # pd_op.one_hot: (8x1x9x484xf32) <- (8x1x9xi64, 1xi32)
        ...
        # pd_op.sum: (8x1x484xf32) <- (8x1x9x484xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(one_hot_0, full_int_array_0, None, False)

        # pd_op.multiply: (8x1x484xf32) <- (8x1x484xf32, 8x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(sum_0, data_3)

        # pd_op.topk: (8x1x9xf32, 8x1x9xi64) <- (8x1x1936xf32, 1xi32)
        ...
        # pd_op.scale: (8x1x9xi64) <- (8x1x9xi64, 1xf32)
        scale_1 = paddle._C_ops.scale(topk_3, full_1, float("484"), True)
        ...
        # pd_op.one_hot: (8x1x9x1936xf32) <- (8x1x9xi64, 1xi32)
        one_hot_1 = paddle._C_ops.one_hot(
            topk_3 % paddle.cast(full_3, topk_3.dtype), full_3
        )

        # pd_op.sum: (8x1x1936xf32) <- (8x1x9x1936xf32, 1xi64)
        sum_1 = paddle._C_ops.sum(one_hot_1, full_int_array_0, None, False)

        # pd_op.multiply: (8x1x1936xf32) <- (8x1x1936xf32, 8x1x1xf32)
        multiply_1 = paddle._C_ops.multiply(sum_1, data_3)

        # pd_op.topk: (8x1x9xf32, 8x1x9xi64) <- (8x1x7744xf32, 1xi32)
        ...
        # pd_op.one_hot: (8x1x9x7744xf32) <- (8x1x9xi64, 1xi32)
        one_hot_2 = paddle._C_ops.one_hot(
            topk_5 % paddle.cast(full_4, topk_5.dtype), full_4
        )

        # pd_op.sum: (8x1x7744xf32) <- (8x1x9x7744xf32, 1xi64)
        sum_2 = paddle._C_ops.sum(one_hot_2, full_int_array_0, None, False)

        # pd_op.multiply: (8x1x7744xf32) <- (8x1x7744xf32, 8x1x1xf32)
        ...
        # builtin.combine: ([8x1x484xf32, 8x1x1936xf32, 8x1x7744xf32]) <- (8x1x484xf32, 8x1x1936xf32, 8x1x7744xf32)
        combine_0 = [multiply_0, multiply_1, multiply_2]

        # pd_op.concat: (8x1x10164xf32) <- ([8x1x484xf32, 8x1x1936xf32, 8x1x7744xf32], 1xi32)
