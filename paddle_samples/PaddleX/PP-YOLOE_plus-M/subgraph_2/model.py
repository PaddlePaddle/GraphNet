...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        # pd_op.cast: (8x10164xi32) <- (8x10164xb)
        ...
        # pd_op.unsqueeze: (8x10164x1xi32) <- (8x10164xi32, 1xi64)
        ...
        # pd_op.tile: (8x10164x4xi32) <- (8x10164x1xi32, 3xi64)
        tile_0 = paddle._C_ops.tile(unsqueeze_0, full_int_array_1)
        del full_int_array_1, unsqueeze_0

        # pd_op.cast: (8x10164x4xb) <- (8x10164x4xi32)
        cast_1 = paddle._C_ops.cast(tile_0, paddle.bool)
        del tile_0

        # pd_op.masked_select: (-1xf32) <- (8x10164x4xf32, 8x10164x4xb)
        ...
        # pd_op.sum: (8x10164xf32) <- (8x10164x4xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(data_3, full_int_array_0, None, False)
        del data_3

        # pd_op.masked_select: (-1xf32) <- (8x10164xf32, 8x10164xb)
        ...
        # pd_op.unsqueeze: (8x10164x1xb) <- (8x10164xb, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(data_0, full_int_array_0)
        del data_0

        # pd_op.cast: (8x10164x1xi32) <- (8x10164x1xb)
        ...
        # pd_op.tile: (8x10164x68xi32) <- (8x10164x1xi32, 3xi64)
        tile_1 = paddle._C_ops.tile(cast_2, full_int_array_4)
        del cast_2, full_int_array_4

        # pd_op.cast: (8x10164x68xb) <- (8x10164x68xi32)
        cast_3 = paddle._C_ops.cast(tile_1, paddle.bool)
        del tile_1

        # pd_op.masked_select: (-1xf32) <- (8x10164x68xf32, 8x10164x68xb)
        ...
        # pd_op.split_with_num: ([8x10164x2xf32, 8x10164x2xf32]) <- (8x10164x4xf32, 1xi32)
        split_with_num_2 = paddle._C_ops.split_with_num(data_2, 2, full_5)
        del data_2, full_5

        # builtin.split: (8x10164x2xf32, 8x10164x2xf32) <- ([8x10164x2xf32, 8x10164x2xf32])
        (
            split_8,
            split_9,
        ) = split_with_num_2
        ...
        # pd_op.subtract: (8x10164x2xf32) <- (10164x2xf32, 8x10164x2xf32)
        subtract_12 = paddle._C_ops.subtract(data_6, split_8)
        del split_8

        # pd_op.subtract: (8x10164x2xf32) <- (8x10164x2xf32, 10164x2xf32)
        ...
        # builtin.combine: ([8x10164x2xf32, 8x10164x2xf32]) <- (8x10164x2xf32, 8x10164x2xf32)
        combine_0 = [subtract_12, subtract_13]
        del subtract_12, subtract_13

        # pd_op.concat: (8x10164x4xf32) <- ([8x10164x2xf32, 8x10164x2xf32], 1xi32)
        ...
        # pd_op.clip: (8x10164x4xf32) <- (8x10164x4xf32, 1xf32, 1xf32)
