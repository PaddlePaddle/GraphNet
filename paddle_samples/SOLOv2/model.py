...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.cast: (1x-1x-1x-1xf32) <- (1x-1x-1x-1xui8)
        ...
        combine_0 = [full_6, slice_2, slice_3]

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_0 = paddle._C_ops.stack(combine_0, 0)

        # pd_op.reshape: (-1x-1x-1xf32) <- (1x-1x-1x-1xf32, 3xi64)
        reshape_4 = paddle._C_ops.reshape(cast_2, stack_0)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_4 = [1, 2]

        # pd_op.sum: (-1xf32) <- (-1x-1x-1xf32, 2xi64)
        sum_2 = paddle._C_ops.sum(reshape_4, full_int_array_4, None, False)
        ...
        greater_than_0 = paddle._C_ops.greater_than(sum_2, full_7)
        ...
        # pd_op.shape64: (3xi64) <- (-1x-1x-1xf32)
        shape64_1 = paddle._C_ops.shape64(sigmoid_0)
        ...
        # builtin.combine: ([xi64, xi64]) <- (xi64, xi64)
        combine_2 = [slice_7, full_6]

        # pd_op.stack: (2xi64) <- ([xi64, xi64])
        stack_2 = paddle._C_ops.stack(combine_2, 0)

        # pd_op.reshape: (-1x-1xf32) <- (-1x-1x-1xf32, 2xi64)
        reshape_1 = paddle._C_ops.reshape(reshape_4, stack_2)
        ...
        sum_3 = paddle._C_ops.sum(multiply_0, full_int_array_1, None, False)

        # pd_op.multiply: (-1x-1xf32) <- (-1x-1xf32, -1x-1xf32)
        multiply_1 = paddle._C_ops.multiply(reshape_0, reshape_0)

        # pd_op.sum: (-1xf32) <- (-1x-1xf32, 1xi64)
        sum_4 = paddle._C_ops.sum(multiply_1, full_int_array_1, None, False)
        ...
        # pd_op.sum: (-1xf32) <- (-1x-1xf32, 1xi64)
        sum_5 = paddle._C_ops.sum(multiply_10, full_int_array_1, None, False)
        ...
        sum_6 = paddle._C_ops.sum(cast_0, full_int_array_5, None, False)
        ...
        # pd_op.shape64: (3xi64) <- (-1x-1x-1xf32)
        shape64_3 = paddle._C_ops.shape64(data_1)
        ...
        # builtin.combine: ([xi64, xi64, xi64]) <- (xi64, xi64, xi64)
        combine_3 = [full_6, slice_11, slice_12]

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_3 = paddle._C_ops.stack(combine_3, 0)

        # pd_op.reshape: (-1x-1x-1xf32) <- (1x-1x-1x-1xf32, 3xi64)
        reshape_5 = paddle._C_ops.reshape(cast_3, stack_3)

        # pd_op.sum: (-1xf32) <- (-1x-1x-1xf32, 2xi64)
        sum_7 = paddle._C_ops.sum(reshape_5, full_int_array_4, None, False)

        # pd_op.greater_than: (-1xb) <- (-1xf32, xf32)
        greater_than_1 = paddle._C_ops.greater_than(sum_7, full_7)
        ...
        # pd_op.shape64: (3xi64) <- (-1x-1x-1xf32)
        shape64_4 = paddle._C_ops.shape64(sigmoid_1)
        ...
        # builtin.combine: ([xi64, xi64]) <- (xi64, xi64)
        combine_4 = [slice_13, full_6]

        # pd_op.stack: (2xi64) <- ([xi64, xi64])
        stack_4 = paddle._C_ops.stack(combine_4, 0)

        # pd_op.reshape: (-1x-1xf32) <- (-1x-1x-1xf32, 2xi64)
        reshape_2 = paddle._C_ops.reshape(sigmoid_1, stack_4)

        # pd_op.shape64: (3xi64) <- (-1x-1x-1xf32)
        shape64_5 = paddle._C_ops.shape64(reshape_5)
        ...
        # builtin.combine: ([xi64, xi64]) <- (xi64, xi64)
        combine_5 = [slice_16, full_6]

        # pd_op.stack: (2xi64) <- ([xi64, xi64])
        stack_5 = paddle._C_ops.stack(combine_5, 0)

        # pd_op.reshape: (-1x-1xf32) <- (-1x-1x-1xf32, 2xi64)
        reshape_3 = paddle._C_ops.reshape(reshape_5, stack_5)

        # pd_op.multiply: (-1x-1xf32) <- (-1x-1xf32, -1x-1xf32)
        multiply_3 = paddle._C_ops.multiply(reshape_2, reshape_3)

        # pd_op.sum: (-1xf32) <- (-1x-1xf32, 1xi64)
        sum_8 = paddle._C_ops.sum(multiply_3, full_int_array_1, None, False)

        # pd_op.multiply: (-1x-1xf32) <- (-1x-1xf32, -1x-1xf32)
        multiply_4 = paddle._C_ops.multiply(reshape_2, reshape_2)

        # pd_op.sum: (-1xf32) <- (-1x-1xf32, 1xi64)
        sum_9 = paddle._C_ops.sum(multiply_4, full_int_array_1, None, False)
        ...
        # pd_op.sum: (-1xf32) <- (-1x-1xf32, 1xi64)
        sum_10 = paddle._C_ops.sum(multiply_11, full_int_array_1, None, False)
        ...
        # pd_op.sum: (xf32) <- (-1xf32, 0xi64)
        sum_11 = paddle._C_ops.sum(cast_1, full_int_array_5, None, False)
        ...
        combine_6 = [multiply_2, multiply_5]

        # pd_op.concat: (-1xf32) <- ([-1xf32, -1xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_6, full_3)

        # pd_op.sum: (xf32) <- (-1xf32, 0xi64)
        sum_0 = paddle._C_ops.sum(concat_0, full_int_array_5, None, False)
        ...
        one_hot_0 = paddle._C_ops.one_hot(
            data_5 % paddle.cast(full_8, data_5.dtype), full_8
        )
        ...
        slice_0 = paddle._C_ops.slice(
            one_hot_0, [1], full_int_array_1, full_int_array_6, [1], []
        )
        ...
        sum_1 = paddle._C_ops.sum(divide_3, full_int_array_5, None, False)
        return (
            cast_0,
            sigmoid_0,
            reshape_0,
