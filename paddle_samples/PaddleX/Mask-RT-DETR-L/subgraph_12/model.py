...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.add: (2x576x256xf32) <- (2x576x256xf32, 1x576x256xf32)
        ...
        # pd_op.matmul: (2x576x256xf32) <- (2x576x256xf32, 256x256xf32)
        matmul_0 = paddle._C_ops.matmul(add_0, slice_0, False, False)

        # pd_op.add: (2x576x256xf32) <- (2x576x256xf32, 256xf32)
        add_1 = paddle._C_ops.add(matmul_0, slice_1)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_2 = [0, 0, 8, 32]

        # pd_op.reshape: (2x576x8x32xf32) <- (2x576x256xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(add_1, full_int_array_2)

        # pd_op.transpose: (2x8x576x32xf32) <- (2x576x8x32xf32)
        ...
        # pd_op.matmul: (2x8x576x576xf32) <- (2x8x576x32xf32, 2x8x576x32xf32)
        ...
        # pd_op.scale: (2x8x576x576xf32) <- (2x8x576x576xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(matmul_3, full_0, float("0"), True)
        del matmul_3

        # pd_op.softmax: (2x8x576x576xf32) <- (2x8x576x576xf32)
        softmax_0 = paddle._C_ops.softmax(scale_0, -1)
        del scale_0

        # pd_op.matmul: (2x8x576x32xf32) <- (2x8x576x576xf32, 2x8x576x32xf32)
        matmul_4 = paddle._C_ops.matmul(softmax_0, transpose_2, False, False)

        # pd_op.transpose: (2x576x8x32xf32) <- (2x8x576x32xf32)
        ...
        # pd_op.reshape: (2x576x256xf32) <- (2x576x8x32xf32, 3xi64)
        ...
        # pd_op.add: (2x576x256xf32) <- (2x576x256xf32, 2x576x256xf32)
        add_5 = paddle._C_ops.add(data_2, add_4)
        del data_2

        # pd_op.layer_norm: (2x576x256xf32, 2x576xf32, 2x576xf32) <- (2x576x256xf32, 256xf32, 256xf32)
        ...
        # pd_op.matmul: (2x576x1024xf32) <- (2x576x256xf32, 256x1024xf32)
        matmul_6 = paddle._C_ops.matmul(layer_norm_1, parameter_5, False, False)
        del parameter_5

        # pd_op.add: (2x576x1024xf32) <- (2x576x1024xf32, 1024xf32)
        add_6 = paddle._C_ops.add(matmul_6, parameter_4)
        del parameter_4

        # pd_op.gelu: (2x576x1024xf32) <- (2x576x1024xf32)
        gelu_0 = paddle._C_ops.gelu(add_6, False)

        # pd_op.matmul: (2x576x256xf32) <- (2x576x1024xf32, 1024x256xf32)
