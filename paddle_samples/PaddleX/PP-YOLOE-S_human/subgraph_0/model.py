...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.unsqueeze: (2x1x5376xf32) <- (2x5376xf32, 1xi64)
        ...
        # pd_op.greater_than: (2x1x5376xb) <- (2x1x5376xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(unsqueeze_0, full_0)
        del full_0, unsqueeze_0

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_1 = [1, 15, 1]

        # pd_op.tile: (2x15x5376xb) <- (2x1x5376xb, 3xi64)
        ...
        # pd_op.argmax: (2x5376xi64) <- (2x15x5376xf32, 1xi64)
        argmax_0 = paddle._C_ops.argmax(data_1, full_1, False, False, paddle.int64)

        # pd_op.full: (1xi32) <- ()
        full_2 = paddle._C_ops.full(
            [1], float("15"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (2x5376x15xf32) <- (2x5376xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )
        del argmax_0, full_2

        # pd_op.transpose: (2x15x5376xf32) <- (2x5376x15xf32)
        transpose_0 = paddle._C_ops.transpose(one_hot_0, [0, 2, 1])
        del one_hot_0

        # pd_op.where: (2x15x5376xf32) <- (2x15x5376xb, 2x15x5376xf32, 2x15x5376xf32)
        ...
        # pd_op.sum: (2x5376xf32) <- (2x15x5376xf32, 1xi64)
        ...
        # pd_op.add: (2x5376xi64) <- (2x5376xi64, 2x1xi64)
        add_0 = paddle._C_ops.add(argmax_1, cast_0)
        del argmax_1, cast_0

        # pd_op.flatten: (30xi32) <- (2x15x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_4, 0, 2)
        del data_4

        # pd_op.flatten: (10752xi64) <- (2x5376xi64)
        ...
        # pd_op.gather: (10752xi32) <- (30xi32, 10752xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_3 = [2, 5376]

        # pd_op.reshape: (2x5376xi32) <- (10752xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x5376xb) <- (2x5376xf32, xf32)
        ...
        # pd_op.full_like: (2x5376xi32) <- (2x5376xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )

        # pd_op.where: (2x5376xi32) <- (2x5376xb, 2x5376xi32, 2x5376xi32)
        ...
        # pd_op.one_hot: (2x5376x2xf32) <- (2x5376xi32, 1xi32)
        ...
        # pd_op.index_select: (2x5376x1xf32) <- (2x5376x2xf32, 1xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_1, assign_value__0, -1)
        del assign_value__0, one_hot_1

        # pd_op.multiply: (2x15x5376xf32) <- (2x15x5376xf32, 2x15x5376xf32)
        ...
        # pd_op.max: (2x15x1xf32) <- (2x15x5376xf32, 1xi64)
