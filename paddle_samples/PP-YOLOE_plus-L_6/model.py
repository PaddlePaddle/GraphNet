...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (8x6069xi64) <- (8x1x6069xf32, 1xi64)
        ...
        # pd_op.gather: (48552xi32) <- (8xi32, 48552xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_5)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [8, 6069]

        # pd_op.reshape: (8x6069xi32) <- (48552xi32, 2xi64)
        ...
        # pd_op.greater_than: (8x6069xb) <- (8x6069xf32, xf32)
        ...
        # pd_op.full_like: (8x6069xi32) <- (8x6069xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_7, paddle.int32, paddle.framework._current_expected_place()
        )

        # pd_op.where: (8x6069xi32) <- (8x6069xb, 8x6069xi32, 8x6069xi32)
        ...
        # pd_op.gather: (48552x4xf32) <- (8x4xf32, 48552xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_5)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [8, 6069, 4]

        # pd_op.reshape: (8x6069x4xf32) <- (48552x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (8x6069x5xf32) <- (8x6069xi32, 1xi32)
        ...
        # pd_op.index_select: (8x6069x4xf32) <- (8x6069x5xf32, 4xi64)
        ...
        # pd_op.unsqueeze: (8x1x6069x4xf32) <- (8x6069x4xf32, 1xi64)
        ...
        # pd_op.slice: (8x1x6069x2xf32) <- (8x1x6069x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (8x1x6069x2xf32) <- (8x1x1x2xf32, 8x1x6069x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (8x1x6069x2xf32) <- (8x1x1x2xf32, 8x1x6069x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (8x1x6069x2xf32) <- (8x1x6069x2xf32, 8x1x6069x2xf32)
        ...
        # pd_op.clip: (8x1x6069x2xf32) <- (8x1x6069x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_10, full_11)

        # pd_op.prod: (8x1x6069xf32) <- (8x1x6069x2xf32, 1xi64)
        ...
        # pd_op.add: (8x1x6069xf32) <- (8x1x1xf32, 8x1x6069xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)

        # pd_op.subtract: (8x1x6069xf32) <- (8x1x6069xf32, 8x1x6069xf32)
        subtract_3 = paddle._C_ops.subtract(add_1, prod_0)

        # pd_op.scale: (8x1x6069xf32) <- (8x1x6069xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_4, float("1e-09"), True)

        # pd_op.divide: (8x1x6069xf32) <- (8x1x6069xf32, 8x1x6069xf32)
        ...
        # pd_op.max: (8x6069xf32) <- (8x1x6069xf32, 1xi64)
        max_0 = paddle._C_ops.max(multiply_1, full_int_array_8, False)

        # pd_op.unsqueeze: (8x6069x1xf32) <- (8x6069xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(max_0, full_int_array_0)

        # pd_op.multiply: (8x6069x4xf32) <- (8x6069x4xf32, 8x6069x1xf32)
