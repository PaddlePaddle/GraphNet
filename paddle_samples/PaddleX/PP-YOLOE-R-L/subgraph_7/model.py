...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        full_int_array_1 = [1, 155, 1]

        # pd_op.tile: (1x155x21504xb) <- (1x1x21504xb, 3xi64)
        ...
        # pd_op.argmax: (1x21504xi64) <- (1x155x21504xf32, 1xi64)
        argmax_0 = paddle._C_ops.argmax(data_1, full_1, False, False, paddle.int64)

        # pd_op.full: (1xi32) <- ()
        full_2 = paddle._C_ops.full(
            [1], float("155"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (1x21504x155xf32) <- (1x21504xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )
        del argmax_0, full_2

        # pd_op.transpose: (1x155x21504xf32) <- (1x21504x155xf32)
        transpose_0 = paddle._C_ops.transpose(one_hot_0, [0, 2, 1])
        del one_hot_0

        # pd_op.where: (1x155x21504xf32) <- (1x155x21504xb, 1x155x21504xf32, 1x155x21504xf32)
        ...
        # pd_op.sum: (1x21504xf32) <- (1x155x21504xf32, 1xi64)
        ...
        full_3 = paddle._C_ops.full(
            [1], float("155"), paddle.float32, paddle.core.CPUPlace()
        )
        ...
        # pd_op.flatten: (155xi32) <- (1x155x1xi32)
        ...
        # pd_op.gather: (21504xi32) <- (155xi32, 21504xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_3 = [1, 21504]
        ...
        # pd_op.multiply: (1x155x21504xf32) <- (1x155x21504xf32, 1x155x21504xf32)
        ...
        # pd_op.max: (1x155x1xf32) <- (1x155x21504xf32, 1xi64)
        ...
        # pd_op.scale: (1x155x1xf32) <- (1x155x1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(max_0, full_9, float("1e-09"), True)
        del full_9, max_0

        # pd_op.divide: (1x155x21504xf32) <- (1x155x21504xf32, 1x155x1xf32)
        divide_0 = paddle._C_ops.divide(multiply_1, scale_1)
        del multiply_1, scale_1

        # pd_op.multiply: (1x155x21504xf32) <- (1x155x21504xf32, 1x155x1xf32)
        multiply_3 = paddle._C_ops.multiply(divide_0, max_1)
        del divide_0, max_1

        # pd_op.max: (1x21504xf32) <- (1x155x21504xf32, 1xi64)
