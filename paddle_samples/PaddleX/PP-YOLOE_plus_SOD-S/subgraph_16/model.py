...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.unsqueeze: (2x1x9261xf32) <- (2x9261xf32, 1xi64)
        ...
        # pd_op.greater_than: (2x1x9261xb) <- (2x1x9261xf32, xf32)
        ...
        # pd_op.tile: (2x38x9261xb) <- (2x1x9261xb, 3xi64)
        tile_0 = paddle._C_ops.tile(greater_than_0, full_int_array_1)
        del full_int_array_1, greater_than_0

        # pd_op.multiply: (2x38x9261xf32) <- (2x38x9261xf32, 2x38x9261xf32)
        ...
        # pd_op.argmax: (2x9261xi64) <- (2x38x9261xf32, 1xi64)
        ...
        # pd_op.one_hot: (2x9261x38xf32) <- (2x9261xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )
        del argmax_0, full_2

        # pd_op.transpose: (2x38x9261xf32) <- (2x9261x38xf32)
        transpose_0 = paddle._C_ops.transpose(one_hot_0, [0, 2, 1])
        del one_hot_0

        # pd_op.where: (2x38x9261xf32) <- (2x38x9261xb, 2x38x9261xf32, 2x38x9261xf32)
        ...
        # pd_op.sum: (2x9261xf32) <- (2x38x9261xf32, 1xi64)
        ...
        # pd_op.add: (2x9261xi64) <- (2x9261xi64, 2x1xi64)
        add_0 = paddle._C_ops.add(argmax_1, cast_0)
        del argmax_1, cast_0

        # pd_op.flatten: (76xi32) <- (2x38x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_4, 0, 2)
        del data_4

        # pd_op.flatten: (18522xi64) <- (2x9261xi64)
        ...
        # pd_op.gather: (18522xi32) <- (76xi32, 18522xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_3 = [2, 9261]

        # pd_op.reshape: (2x9261xi32) <- (18522xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x9261xb) <- (2x9261xf32, xf32)
        ...
        # pd_op.full_like: (2x9261xi32) <- (2x9261xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_6

        # pd_op.where: (2x9261xi32) <- (2x9261xb, 2x9261xi32, 2x9261xi32)
        ...
        # pd_op.index_select: (2x9261x10xf32) <- (2x9261x11xf32, 10xi64)
        ...
        # pd_op.max: (2x38x1xf32) <- (2x38x9261xf32, 1xi64)
