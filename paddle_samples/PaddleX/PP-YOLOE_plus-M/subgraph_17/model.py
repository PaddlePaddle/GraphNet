...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, data_0, data_1, data_2):
        # pd_op.divide: (10164x2xf32) <- (10164x2xf32, 10164x1xf32)
        divide_0 = paddle._C_ops.divide(data_1, data_2)
        del data_1

        # pd_op.shape64: (3xi64) <- (8x10164x68xf32)
        ...
        # pd_op.reshape: (-1x-1x4x17xf32) <- (8x10164x68xf32, 4xi64)
        ...
        # pd_op.add: (-1x10164x2xf32) <- (-1x-1x2xf32, 10164x2xf32)
        ...
        # builtin.combine: ([-1x10164x2xf32, -1x10164x2xf32]) <- (-1x10164x2xf32, -1x10164x2xf32)
        combine_1 = [add_0, add_1]

        # pd_op.concat: (-1x10164x4xf32) <- ([-1x10164x2xf32, -1x10164x2xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_1, full_5)
        del combine_1

        # pd_op.share_data_: (-1x10164x4xf32) <- (-1x10164x4xf32)
        share_data__0 = concat_0.detach()

        # pd_op.multiply: (-1x10164x4xf32) <- (-1x10164x4xf32, 10164x1xf32)
