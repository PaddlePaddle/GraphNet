...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (2x256x256x192xf32) <- (2x256x256x192xf32, 256x256x3x3xf32)
        ...
        # pd_op.add: (2x256x256x192xf32) <- (2x256x256x192xf32, 1x256x1x1xf32)
        add_10 = paddle._C_ops.add(conv2d_0, reshape_5)

        # pd_op.relu: (2x256x256x192xf32) <- (2x256x256x192xf32)
        relu_0 = paddle._C_ops.relu(add_10)
        del add_10

        # pd_op.conv2d: (2x256x128x96xf32) <- (2x256x128x96xf32, 256x256x3x3xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            data_1, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del data_1

        # pd_op.add: (2x256x128x96xf32) <- (2x256x128x96xf32, 1x256x1x1xf32)
        add_11 = paddle._C_ops.add(conv2d_1, reshape_5)

        # pd_op.relu: (2x256x128x96xf32) <- (2x256x128x96xf32)
        relu_1 = paddle._C_ops.relu(add_11)
        del add_11

        # pd_op.conv2d: (2x256x64x48xf32) <- (2x256x64x48xf32, 256x256x3x3xf32)
        conv2d_2 = paddle._C_ops.conv2d(
            data_2, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del data_2

        # pd_op.add: (2x256x64x48xf32) <- (2x256x64x48xf32, 1x256x1x1xf32)
        add_12 = paddle._C_ops.add(conv2d_2, reshape_5)

        # pd_op.relu: (2x256x64x48xf32) <- (2x256x64x48xf32)
        relu_2 = paddle._C_ops.relu(add_12)
        del add_12

        # pd_op.conv2d: (2x256x32x24xf32) <- (2x256x32x24xf32, 256x256x3x3xf32)
        conv2d_3 = paddle._C_ops.conv2d(
            data_3, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )
        del data_3

        # pd_op.add: (2x256x32x24xf32) <- (2x256x32x24xf32, 1x256x1x1xf32)
        add_13 = paddle._C_ops.add(conv2d_3, reshape_5)

        # pd_op.relu: (2x256x32x24xf32) <- (2x256x32x24xf32)
        ...
        # pd_op.conv2d: (2x3x256x192xf32) <- (2x256x256x192xf32, 3x256x1x1xf32)
        ...
        # pd_op.add: (2x3x256x192xf32) <- (2x3x256x192xf32, 1x3x1x1xf32)
        add_0 = paddle._C_ops.add(conv2d_5, reshape_6)

        # pd_op.conv2d: (2x12x256x192xf32) <- (2x256x256x192xf32, 12x256x1x1xf32)
        ...
        # pd_op.add: (2x12x256x192xf32) <- (2x12x256x192xf32, 1x12x1x1xf32)
        add_5 = paddle._C_ops.add(conv2d_6, reshape_7)

        # pd_op.conv2d: (2x3x128x96xf32) <- (2x256x128x96xf32, 3x256x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            relu_1, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (2x3x128x96xf32) <- (2x3x128x96xf32, 1x3x1x1xf32)
        add_1 = paddle._C_ops.add(conv2d_7, reshape_6)

        # pd_op.conv2d: (2x12x128x96xf32) <- (2x256x128x96xf32, 12x256x1x1xf32)
        conv2d_8 = paddle._C_ops.conv2d(
            relu_1, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (2x12x128x96xf32) <- (2x12x128x96xf32, 1x12x1x1xf32)
        add_6 = paddle._C_ops.add(conv2d_8, reshape_7)

        # pd_op.conv2d: (2x3x64x48xf32) <- (2x256x64x48xf32, 3x256x1x1xf32)
        conv2d_9 = paddle._C_ops.conv2d(
            relu_2, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (2x3x64x48xf32) <- (2x3x64x48xf32, 1x3x1x1xf32)
        add_2 = paddle._C_ops.add(conv2d_9, reshape_6)

        # pd_op.conv2d: (2x12x64x48xf32) <- (2x256x64x48xf32, 12x256x1x1xf32)
        conv2d_10 = paddle._C_ops.conv2d(
            relu_2, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (2x12x64x48xf32) <- (2x12x64x48xf32, 1x12x1x1xf32)
        add_7 = paddle._C_ops.add(conv2d_10, reshape_7)

        # pd_op.conv2d: (2x3x32x24xf32) <- (2x256x32x24xf32, 3x256x1x1xf32)
        conv2d_11 = paddle._C_ops.conv2d(
            relu_3, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (2x3x32x24xf32) <- (2x3x32x24xf32, 1x3x1x1xf32)
        add_3 = paddle._C_ops.add(conv2d_11, reshape_6)

        # pd_op.conv2d: (2x12x32x24xf32) <- (2x256x32x24xf32, 12x256x1x1xf32)
        conv2d_12 = paddle._C_ops.conv2d(
            relu_3, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (2x12x32x24xf32) <- (2x12x32x24xf32, 1x12x1x1xf32)
        ...
        # builtin.combine: ([256xf32, 192xf32]) <- (256xf32, 192xf32)
        combine_0 = [arange_1, arange_0]
        del arange_0, arange_1

        # pd_op.meshgrid: ([256x192xf32, 256x192xf32]) <- ([256xf32, 192xf32])
        meshgrid_0 = paddle._C_ops.meshgrid(combine_0)
        del combine_0

        # builtin.split: (256x192xf32, 256x192xf32) <- ([256x192xf32, 256x192xf32])
        (
            split_0,
            split_1,
        ) = meshgrid_0
        ...
        # pd_op.reshape: (49152xf32) <- (256x192xf32, 1xi64)
        ...
        # builtin.combine: ([49152xf32, 49152xf32, 49152xf32, 49152xf32]) <- (49152xf32, 49152xf32, 49152xf32, 49152xf32)
        combine_1 = [reshape_8, reshape_9, reshape_8, reshape_9]
        del reshape_8, reshape_9

        # pd_op.stack: (49152x4xf32) <- ([49152xf32, 49152xf32, 49152xf32, 49152xf32])
        ...
        # pd_op.reshape: (49152x1x4xf32) <- (49152x4xf32, 3xi64)
        ...
        # pd_op.add: (49152x3x4xf32) <- (49152x1x4xf32, 1x3x4xf32)
        ...
        # pd_op.reshape: (147456x4xf32) <- (49152x3x4xf32, 2xi64)
        ...
        # builtin.combine: ([128xf32, 96xf32]) <- (128xf32, 96xf32)
        combine_2 = [arange_3, arange_2]
        del arange_2, arange_3

        # pd_op.meshgrid: ([128x96xf32, 128x96xf32]) <- ([128xf32, 96xf32])
        meshgrid_1 = paddle._C_ops.meshgrid(combine_2)
        del combine_2

        # builtin.split: (128x96xf32, 128x96xf32) <- ([128x96xf32, 128x96xf32])
        (
            split_2,
            split_3,
        ) = meshgrid_1
        ...
        # pd_op.reshape: (12288xf32) <- (128x96xf32, 1xi64)
        ...
        # builtin.combine: ([12288xf32, 12288xf32, 12288xf32, 12288xf32]) <- (12288xf32, 12288xf32, 12288xf32, 12288xf32)
        combine_3 = [reshape_12, reshape_13, reshape_12, reshape_13]
        del reshape_12, reshape_13

        # pd_op.stack: (12288x4xf32) <- ([12288xf32, 12288xf32, 12288xf32, 12288xf32])
        stack_1 = paddle._C_ops.stack(combine_3, 1)
        del combine_3

        # pd_op.reshape: (12288x1x4xf32) <- (12288x4xf32, 3xi64)
        ...
        # pd_op.add: (12288x3x4xf32) <- (12288x1x4xf32, 1x3x4xf32)
        add_16 = paddle._C_ops.add(reshape_14, reshape_15)
        del reshape_14, reshape_15

        # pd_op.reshape: (36864x4xf32) <- (12288x3x4xf32, 2xi64)
        ...
        # builtin.combine: ([64xf32, 48xf32]) <- (64xf32, 48xf32)
        combine_4 = [arange_5, arange_4]
        del arange_4, arange_5

        # pd_op.meshgrid: ([64x48xf32, 64x48xf32]) <- ([64xf32, 48xf32])
        meshgrid_2 = paddle._C_ops.meshgrid(combine_4)
        del combine_4

        # builtin.split: (64x48xf32, 64x48xf32) <- ([64x48xf32, 64x48xf32])
        (
            split_4,
            split_5,
        ) = meshgrid_2
        ...
        # pd_op.reshape: (3072xf32) <- (64x48xf32, 1xi64)
        ...
        # builtin.combine: ([3072xf32, 3072xf32, 3072xf32, 3072xf32]) <- (3072xf32, 3072xf32, 3072xf32, 3072xf32)
        combine_5 = [reshape_16, reshape_17, reshape_16, reshape_17]
        del reshape_16, reshape_17

        # pd_op.stack: (3072x4xf32) <- ([3072xf32, 3072xf32, 3072xf32, 3072xf32])
        stack_2 = paddle._C_ops.stack(combine_5, 1)
        del combine_5

        # pd_op.reshape: (3072x1x4xf32) <- (3072x4xf32, 3xi64)
        ...
        # pd_op.add: (3072x3x4xf32) <- (3072x1x4xf32, 1x3x4xf32)
        add_17 = paddle._C_ops.add(reshape_18, reshape_19)
        del reshape_18, reshape_19

        # pd_op.reshape: (9216x4xf32) <- (3072x3x4xf32, 2xi64)
        ...
        # builtin.combine: ([32xf32, 24xf32]) <- (32xf32, 24xf32)
        combine_6 = [arange_7, arange_6]
        del arange_6, arange_7

        # pd_op.meshgrid: ([32x24xf32, 32x24xf32]) <- ([32xf32, 24xf32])
        meshgrid_3 = paddle._C_ops.meshgrid(combine_6)
        del combine_6

        # builtin.split: (32x24xf32, 32x24xf32) <- ([32x24xf32, 32x24xf32])
        (
            split_6,
            split_7,
        ) = meshgrid_3
        ...
        # pd_op.reshape: (768xf32) <- (32x24xf32, 1xi64)
        ...
        # builtin.combine: ([768xf32, 768xf32, 768xf32, 768xf32]) <- (768xf32, 768xf32, 768xf32, 768xf32)
        combine_7 = [reshape_20, reshape_21, reshape_20, reshape_21]
        del reshape_20, reshape_21

        # pd_op.stack: (768x4xf32) <- ([768xf32, 768xf32, 768xf32, 768xf32])
        stack_3 = paddle._C_ops.stack(combine_7, 1)
        del combine_7

        # pd_op.reshape: (768x1x4xf32) <- (768x4xf32, 3xi64)
        ...
        # pd_op.add: (768x3x4xf32) <- (768x1x4xf32, 1x3x4xf32)
        add_18 = paddle._C_ops.add(reshape_22, reshape_23)
        del reshape_22, reshape_23

        # pd_op.reshape: (2304x4xf32) <- (768x3x4xf32, 2xi64)
