...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.unsqueeze: (96x1x4xf32) <- (96x4xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(reshape_2, full_int_array_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [0]

        # pd_op.unsqueeze: (1x6069x4xf32) <- (6069x4xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(data_0, full_int_array_2)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_3 = [2]

        # pd_op.slice: (96x1x2xf32) <- (96x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.slice: (1x6069x2xf32) <- (1x6069x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (96x6069x2xf32) <- (96x1x2xf32, 1x6069x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (96x6069x2xf32) <- (96x1x2xf32, 1x6069x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (96x6069x2xf32) <- (96x6069x2xf32, 96x6069x2xf32)
        ...
        # pd_op.scale: (96x6069xf32) <- (96x6069xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_3, full_2, float("1e-10"), True)

        # pd_op.divide: (96x6069xf32) <- (96x6069xf32, 96x6069xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_0)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_6 = [4, -1, 6069]

        # pd_op.reshape: (4x24x6069xf32) <- (96x6069xf32, 3xi64)
        reshape_1 = paddle._C_ops.reshape(divide_0, full_int_array_6)

        # pd_op.slice: (96xf32) <- (96x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.add: (96xf32) <- (96xf32, 96xf32)
        ...
        # pd_op.stack: (96x2xf32) <- ([96xf32, 96xf32])
        stack_1 = paddle._C_ops.stack(combine_0, -1)

        # pd_op.unsqueeze: (96x1x2xf32) <- (96x2xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(stack_1, full_int_array_1)

        # pd_op.slice: (6069xf32) <- (6069x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.add: (6069xf32) <- (6069xf32, 6069xf32)
        add_3 = paddle._C_ops.add(slice_8, slice_9)

        # pd_op.scale: (6069xf32) <- (6069xf32, 1xf32)
        ...
        # builtin.combine: ([6069xf32, 6069xf32]) <- (6069xf32, 6069xf32)
        combine_1 = [scale_3, scale_4]

        # pd_op.stack: (6069x2xf32) <- ([6069xf32, 6069xf32])
        stack_0 = paddle._C_ops.stack(combine_1, -1)

        # pd_op.unsqueeze: (1x6069x2xf32) <- (6069x2xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(stack_0, full_int_array_2)

        # pd_op.subtract: (96x6069x2xf32) <- (96x1x2xf32, 1x6069x2xf32)
        subtract_4 = paddle._C_ops.subtract(unsqueeze_2, unsqueeze_3)

        # pd_op.p_norm: (96x6069xf32) <- (96x6069x2xf32)
