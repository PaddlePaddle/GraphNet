...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        # pd_op.cast: (2x2541xi32) <- (2x2541xb)
        ...
        # pd_op.sum: (2x2541xf32) <- (2x2541x4xf32, 1xi64)
        sum_2 = paddle._C_ops.sum(data_3, full_int_array_1, None, False)

        # pd_op.masked_select: (-1xf32) <- (2x2541xf32, 2x2541xb)
        ...
        # pd_op.split_with_num: ([2x2541x2xf32, 2x2541x2xf32]) <- (2x2541x4xf32, 1xi32)
        split_with_num_2 = paddle._C_ops.split_with_num(data_2, 2, full_5)

        # builtin.split: (2x2541x2xf32, 2x2541x2xf32) <- ([2x2541x2xf32, 2x2541x2xf32])
        (
            split_8,
            split_9,
        ) = split_with_num_2

        # pd_op.subtract: (2x2541x2xf32) <- (2541x2xf32, 2x2541x2xf32)
        subtract_13 = paddle._C_ops.subtract(data_6, split_8)

        # pd_op.subtract: (2x2541x2xf32) <- (2x2541x2xf32, 2541x2xf32)
        ...
        # builtin.combine: ([2x2541x2xf32, 2x2541x2xf32]) <- (2x2541x2xf32, 2x2541x2xf32)
        combine_0 = [subtract_13, subtract_14]

        # pd_op.concat: (2x2541x4xf32) <- ([2x2541x2xf32, 2x2541x2xf32], 1xi32)
        ...
        # pd_op.clip: (2x2541x4xf32) <- (2x2541x4xf32, 1xf32, 1xf32)
