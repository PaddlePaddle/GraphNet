...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.argmax: (2x2541xi64) <- (2x11x2541xf32, 1xi64)
        ...
        # pd_op.gather: (5082xi32) <- (22xi32, 5082xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_2)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_0 = [2, 2541]

        # pd_op.reshape: (2x2541xi32) <- (5082xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x2541xb) <- (2x2541xf32, xf32)
        ...
        # pd_op.full_like: (2x2541xi32) <- (2x2541xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_4, paddle.int32, paddle.framework._current_expected_place()
        )

        # pd_op.where: (2x2541xi32) <- (2x2541xb, 2x2541xi32, 2x2541xi32)
        ...
        # pd_op.gather: (5082x4xf32) <- (22x4xf32, 5082xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_2)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [2, 2541, 4]

        # pd_op.reshape: (2x2541x4xf32) <- (5082x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (2x2541x5xf32) <- (2x2541xi32, 1xi32)
        ...
        # pd_op.index_select: (2x2541x4xf32) <- (2x2541x5xf32, 4xi64)
        ...
        # pd_op.max: (2x11x1xf32) <- (2x11x2541xf32, 1xi64)
        ...
        # pd_op.divide: (2x11x2541xf32) <- (2x11x2541xf32, 2x11x1xf32)
        ...
        # pd_op.max: (2x2541xf32) <- (2x11x2541xf32, 1xi64)
        max_2 = paddle._C_ops.max(multiply_3, full_int_array_4, False)

        # pd_op.unsqueeze: (2x2541x1xf32) <- (2x2541xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(max_2, full_int_array_3)

        # pd_op.multiply: (2x2541x4xf32) <- (2x2541x4xf32, 2x2541x1xf32)
