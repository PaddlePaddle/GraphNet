...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.slice: (1x3x240x176xf32) <- (2x3x240x176xf32, 1xi64, 1xi64)
        slice_0 = paddle._C_ops.slice(
            data_0, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x240x176xf32) <- (2x12x240x176xf32, 1xi64, 1xi64)
        ...
        # pd_op.full_like: (126720x4xf32) <- (126720x4xf32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            data_10, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x240x176xf32, 1x12x240x176xf32, 1x2xf32, 126720x4xf32, 126720x4xf32)
        ...
        # pd_op.slice: (1x3x120x88xf32) <- (2x3x120x88xf32, 1xi64, 1xi64)
        slice_3 = paddle._C_ops.slice(
            data_1, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x120x88xf32) <- (2x12x120x88xf32, 1xi64, 1xi64)
        slice_4 = paddle._C_ops.slice(
            data_6, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (31680x4xf32) <- (31680x4xf32, 1xf32)
        full_like_1 = paddle._C_ops.full_like(
            data_11, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x120x88xf32, 1x12x120x88xf32, 1x2xf32, 31680x4xf32, 31680x4xf32)
        ...
        # pd_op.slice: (1x3x60x44xf32) <- (2x3x60x44xf32, 1xi64, 1xi64)
        slice_5 = paddle._C_ops.slice(
            data_2, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x60x44xf32) <- (2x12x60x44xf32, 1xi64, 1xi64)
        slice_6 = paddle._C_ops.slice(
            data_7, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (7920x4xf32) <- (7920x4xf32, 1xf32)
        full_like_2 = paddle._C_ops.full_like(
            data_12, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x60x44xf32, 1x12x60x44xf32, 1x2xf32, 7920x4xf32, 7920x4xf32)
        ...
        # pd_op.slice: (1x3x30x22xf32) <- (2x3x30x22xf32, 1xi64, 1xi64)
        slice_7 = paddle._C_ops.slice(
            data_3, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x30x22xf32) <- (2x12x30x22xf32, 1xi64, 1xi64)
        slice_8 = paddle._C_ops.slice(
            data_8, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (1980x4xf32) <- (1980x4xf32, 1xf32)
        full_like_3 = paddle._C_ops.full_like(
            data_13, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x30x22xf32, 1x12x30x22xf32, 1x2xf32, 1980x4xf32, 1980x4xf32)
        ...
        # pd_op.slice: (1x3x15x11xf32) <- (2x3x15x11xf32, 1xi64, 1xi64)
        slice_9 = paddle._C_ops.slice(
            data_4, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x15x11xf32) <- (2x12x15x11xf32, 1xi64, 1xi64)
        slice_10 = paddle._C_ops.slice(
            data_9, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (495x4xf32) <- (495x4xf32, 1xf32)
        full_like_4 = paddle._C_ops.full_like(
            data_14, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x15x11xf32, 1x12x15x11xf32, 1x2xf32, 495x4xf32, 495x4xf32)
