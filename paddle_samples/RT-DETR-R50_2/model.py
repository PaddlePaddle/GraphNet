...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.matmul: (4x8400x256xf32) <- (4x8400x256xf32, 256x256xf32)
        matmul_0 = paddle._C_ops.matmul(data_4, parameter_15, False, False)

        # pd_op.add: (4x8400x256xf32) <- (4x8400x256xf32, 256xf32)
        add_2 = paddle._C_ops.add(matmul_0, parameter_14)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_1 = [4, 8400, 8, 32]

        # pd_op.reshape: (4x8400x8x32xf32) <- (4x8400x256xf32, 4xi64)
        reshape_4 = paddle._C_ops.reshape(add_2, full_int_array_1)

        # pd_op.matmul: (4x498x192xf32) <- (4x498x256xf32, 256x192xf32)
        matmul_1 = paddle._C_ops.matmul(add_1, parameter_13, False, False)

        # pd_op.add: (4x498x192xf32) <- (4x498x192xf32, 192xf32)
        add_3 = paddle._C_ops.add(matmul_1, parameter_12)

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_2 = [4, 498, 8, 3, 4, 2]

        # pd_op.reshape: (4x498x8x3x4x2xf32) <- (4x498x192xf32, 6xi64)
        reshape_5 = paddle._C_ops.reshape(add_3, full_int_array_2)

        # pd_op.matmul: (4x498x96xf32) <- (4x498x256xf32, 256x96xf32)
        matmul_2 = paddle._C_ops.matmul(add_1, parameter_11, False, False)

        # pd_op.add: (4x498x96xf32) <- (4x498x96xf32, 96xf32)
        add_4 = paddle._C_ops.add(matmul_2, parameter_10)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_3 = [4, 498, 8, 12]

        # pd_op.reshape: (4x498x8x12xf32) <- (4x498x96xf32, 4xi64)
        reshape_6 = paddle._C_ops.reshape(add_4, full_int_array_3)

        # pd_op.softmax: (4x498x8x12xf32) <- (4x498x8x12xf32)
        softmax_0 = paddle._C_ops.softmax(reshape_6, -1)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_4 = [4, 498, 8, 3, 4]

        # pd_op.reshape: (4x498x8x3x4xf32) <- (4x498x8x12xf32, 5xi64)
        ...
        # pd_op.slice: (4x498x1x2xf32) <- (4x498x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.unsqueeze: (4x498x1x1x1x2xf32) <- (4x498x1x2xf32, 2xi64)
        ...
        # pd_op.scale: (4x498x8x3x4x2xf32) <- (4x498x8x3x4x2xf32, 1xf32)
        ...
        # pd_op.multiply: (4x498x8x3x4x2xf32) <- (4x498x8x3x4x2xf32, 4x498x1x1x1x2xf32)
        ...
        # pd_op.add: (4x498x8x3x4x2xf32) <- (4x498x1x1x1x2xf32, 4x498x8x3x4x2xf32)
        ...
        # pd_op.split: ([-1x-1x-1x-1xf32, -1x-1x-1x-1xf32, -1x-1x-1x-1xf32]) <- (4x8400x8x32xf32, 3xi64, 1xi32)
        ...
        # pd_op.slice: (4x498x8x4x2xf32) <- (4x498x8x3x4x2xf32, 1xi64, 1xi64)
        slice_14 = paddle._C_ops.slice(
            scale_2, [3], full_int_array_5, full_int_array_9, [1], [3]
        )

        # pd_op.transpose: (4x8x498x4x2xf32) <- (4x498x8x4x2xf32)
        transpose_1 = paddle._C_ops.transpose(slice_14, [0, 2, 1, 3, 4])

        # pd_op.flatten: (32x498x4x2xf32) <- (4x8x498x4x2xf32)
        flatten_0 = paddle._C_ops.flatten(transpose_1, 0, 1)

        # pd_op.grid_sample: (32x32x498x4xf32) <- (32x32x-1x-1xf32, 32x498x4x2xf32)
        ...
        # pd_op.transpose: (4x8x498x3x4xf32) <- (4x498x8x3x4xf32)
        transpose_6 = paddle._C_ops.transpose(reshape_7, [0, 2, 1, 3, 4])

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_11 = [32, 1, 498, 12]

        # pd_op.reshape: (32x1x498x12xf32) <- (4x8x498x3x4xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(transpose_6, full_int_array_11)

        # builtin.combine: ([32x32x498x4xf32, 32x32x498x4xf32, 32x32x498x4xf32]) <- (32x32x498x4xf32, 32x32x498x4xf32, 32x32x498x4xf32)
        combine_4 = [grid_sample_0, grid_sample_1, grid_sample_2]

        # pd_op.stack: (32x32x498x3x4xf32) <- ([32x32x498x4xf32, 32x32x498x4xf32, 32x32x498x4xf32])
        stack_0 = paddle._C_ops.stack(combine_4, -2)

        # pd_op.flatten: (32x32x498x12xf32) <- (32x32x498x3x4xf32)
        flatten_3 = paddle._C_ops.flatten(stack_0, 3, 4)

        # pd_op.multiply: (32x32x498x12xf32) <- (32x32x498x12xf32, 32x1x498x12xf32)
        multiply_0 = paddle._C_ops.multiply(flatten_3, reshape_3)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [-1]

        # pd_op.sum: (32x32x498xf32) <- (32x32x498x12xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(multiply_0, full_int_array_0, None, False)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_12 = [4, 256, 498]

        # pd_op.reshape: (4x256x498xf32) <- (32x32x498xf32, 3xi64)
