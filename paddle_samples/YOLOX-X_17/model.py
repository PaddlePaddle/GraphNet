...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (2x320x84x84xf32) <- (2x320x84x84xf32, 320x320x3x3xf32)
        ...
        # pd_op.conv2d: (2x4x84x84xf32) <- (2x320x84x84xf32, 4x320x1x1xf32)
        ...
        # pd_op.add: (2x4x84x84xf32) <- (2x4x84x84xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (2x5x84x84xf32) <- (2x320x84x84xf32, 5x320x1x1xf32)
        ...
        # pd_op.add: (2x5x84x84xf32) <- (2x5x84x84xf32, 1x5x1x1xf32)
        add_3 = paddle._C_ops.add(conv2d_6, reshape_1)

        # pd_op.sigmoid: (2x4x84x84xf32) <- (2x4x84x84xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_2)

        # pd_op.flatten: (2x4x7056xf32) <- (2x4x84x84xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (2x7056x4xf32) <- (2x4x7056xf32)
        ...
        # pd_op.split: ([2x4x84x84xf32, 2x1x84x84xf32]) <- (2x5x84x84xf32, 2xi64, 1xi32)
        split_4 = paddle._C_ops.split(add_3, full_int_array_1, full_0)

        # builtin.split: (2x4x84x84xf32, 2x1x84x84xf32) <- ([2x4x84x84xf32, 2x1x84x84xf32])
        (
            split_0,
            split_5,
        ) = split_4
        ...
        # pd_op.flatten: (2x1x7056xf32) <- (2x1x84x84xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_6, 2, 3)

        # pd_op.transpose: (2x7056x1xf32) <- (2x1x7056xf32)
        ...
        # pd_op.conv2d: (2x4x42x42xf32) <- (2x320x42x42xf32, 4x320x1x1xf32)
        ...
        # pd_op.add: (2x4x42x42xf32) <- (2x4x42x42xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (2x5x42x42xf32) <- (2x320x42x42xf32, 5x320x1x1xf32)
        ...
        # pd_op.add: (2x5x42x42xf32) <- (2x5x42x42xf32, 1x5x1x1xf32)
        add_5 = paddle._C_ops.add(conv2d_13, reshape_3)

        # pd_op.sigmoid: (2x4x42x42xf32) <- (2x4x42x42xf32)
        sigmoid_12 = paddle._C_ops.sigmoid(add_4)

        # pd_op.flatten: (2x4x1764xf32) <- (2x4x42x42xf32)
        flatten_3 = paddle._C_ops.flatten(sigmoid_12, 2, 3)

        # pd_op.transpose: (2x1764x4xf32) <- (2x4x1764xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])

        # pd_op.split: ([2x4x42x42xf32, 2x1x42x42xf32]) <- (2x5x42x42xf32, 2xi64, 1xi32)
        split_6 = paddle._C_ops.split(add_5, full_int_array_1, full_0)

        # builtin.split: (2x4x42x42xf32, 2x1x42x42xf32) <- ([2x4x42x42xf32, 2x1x42x42xf32])
        (
            split_1,
            split_7,
        ) = split_6
        ...
        # pd_op.flatten: (2x1x1764xf32) <- (2x1x42x42xf32)
        flatten_5 = paddle._C_ops.flatten(sigmoid_13, 2, 3)

        # pd_op.transpose: (2x1764x1xf32) <- (2x1x1764xf32)
        transpose_5 = paddle._C_ops.transpose(flatten_5, [0, 2, 1])

        # pd_op.conv2d: (2x320x21x21xf32) <- (2x1280x21x21xf32, 320x1280x1x1xf32)
        ...
        # pd_op.sigmoid: (2x320x21x21xf32) <- (2x320x21x21xf32)
        sigmoid_14 = paddle._C_ops.sigmoid(batch_norm__60)

        # pd_op.multiply: (2x320x21x21xf32) <- (2x320x21x21xf32, 2x320x21x21xf32)
        multiply_10 = paddle._C_ops.multiply(batch_norm__60, sigmoid_14)

        # pd_op.conv2d: (2x320x21x21xf32) <- (2x320x21x21xf32, 320x320x3x3xf32)
        ...
        # pd_op.conv2d: (2x4x21x21xf32) <- (2x320x21x21xf32, 4x320x1x1xf32)
        ...
        # pd_op.add: (2x4x21x21xf32) <- (2x4x21x21xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (2x5x21x21xf32) <- (2x320x21x21xf32, 5x320x1x1xf32)
        ...
        # pd_op.add: (2x5x21x21xf32) <- (2x5x21x21xf32, 1x5x1x1xf32)
        add_7 = paddle._C_ops.add(conv2d_20, reshape_5)

        # pd_op.sigmoid: (2x4x21x21xf32) <- (2x4x21x21xf32)
        sigmoid_19 = paddle._C_ops.sigmoid(add_6)

        # pd_op.flatten: (2x4x441xf32) <- (2x4x21x21xf32)
        flatten_6 = paddle._C_ops.flatten(sigmoid_19, 2, 3)

        # pd_op.transpose: (2x441x4xf32) <- (2x4x441xf32)
        transpose_6 = paddle._C_ops.transpose(flatten_6, [0, 2, 1])

        # pd_op.split: ([2x4x21x21xf32, 2x1x21x21xf32]) <- (2x5x21x21xf32, 2xi64, 1xi32)
        split_8 = paddle._C_ops.split(add_7, full_int_array_1, full_0)

        # builtin.split: (2x4x21x21xf32, 2x1x21x21xf32) <- ([2x4x21x21xf32, 2x1x21x21xf32])
        (
            split_2,
            split_9,
        ) = split_8
        ...
        # pd_op.flatten: (2x1x441xf32) <- (2x1x21x21xf32)
        flatten_8 = paddle._C_ops.flatten(sigmoid_20, 2, 3)

        # pd_op.transpose: (2x441x1xf32) <- (2x1x441xf32)
        transpose_8 = paddle._C_ops.transpose(flatten_8, [0, 2, 1])

        # builtin.combine: ([2x7056x4xf32, 2x1764x4xf32, 2x441x4xf32]) <- (2x7056x4xf32, 2x1764x4xf32, 2x441x4xf32)
        combine_0 = [transpose_0, transpose_3, transpose_6]

        # pd_op.concat: (2x9261x4xf32) <- ([2x7056x4xf32, 2x1764x4xf32, 2x441x4xf32], 1xi32)
        ...
        # builtin.combine: ([2x7056x1xf32, 2x1764x1xf32, 2x441x1xf32]) <- (2x7056x1xf32, 2x1764x1xf32, 2x441x1xf32)
        combine_2 = [transpose_2, transpose_5, transpose_8]

        # pd_op.concat: (2x9261x1xf32) <- ([2x7056x1xf32, 2x1764x1xf32, 2x441x1xf32], 1xi32)
        ...
        # builtin.combine: ([7056x2xf32, 1764x2xf32, 441x2xf32]) <- (7056x2xf32, 1764x2xf32, 441x2xf32)
        combine_9 = [reshape_9, reshape_10, reshape_11]

        # pd_op.concat: (9261x2xf32) <- ([7056x2xf32, 1764x2xf32, 441x2xf32], 1xi32)
        concat_5 = paddle._C_ops.concat(combine_9, full_16)

        # builtin.combine: ([7056x1xf32, 1764x1xf32, 441x1xf32]) <- (7056x1xf32, 1764x1xf32, 441x1xf32)
        combine_10 = [full_9, full_12, full_15]

        # pd_op.concat: (9261x1xf32) <- ([7056x1xf32, 1764x1xf32, 441x1xf32], 1xi32)
        concat_6 = paddle._C_ops.concat(combine_10, full_16)

        # pd_op.assign: (9261x1xf32) <- (9261x1xf32)
        assign_5 = concat_6
        ...
        # pd_op.split_with_num: ([2x9261x2xf32, 2x9261x2xf32]) <- (2x9261x4xf32, 1xi32)
        split_with_num_0 = paddle._C_ops.split_with_num(concat_4, 2, full_1)

        # builtin.split: (2x9261x2xf32, 2x9261x2xf32) <- ([2x9261x2xf32, 2x9261x2xf32])
        (
            split_3,
            split_16,
        ) = split_with_num_0

        # pd_op.divide: (9261x2xf32) <- (9261x2xf32, 9261x1xf32)
        divide_0 = paddle._C_ops.divide(concat_5, concat_6)

        # pd_op.add: (2x9261x2xf32) <- (2x9261x2xf32, 9261x2xf32)
        add_0 = paddle._C_ops.add(split_3, divide_0)

        # pd_op.exp: (2x9261x2xf32) <- (2x9261x2xf32)
        ...
        # pd_op.scale: (2x9261x2xf32) <- (2x9261x2xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(exp_0, full_2, float("0"), True)

        # pd_op.subtract: (2x9261x2xf32) <- (2x9261x2xf32, 2x9261x2xf32)
        subtract_0 = paddle._C_ops.subtract(add_0, scale_0)

        # pd_op.add: (2x9261x2xf32) <- (2x9261x2xf32, 2x9261x2xf32)
        ...
        # builtin.combine: ([2x9261x2xf32, 2x9261x2xf32]) <- (2x9261x2xf32, 2x9261x2xf32)
        combine_11 = [subtract_0, add_1]

        # pd_op.concat: (2x9261x4xf32) <- ([2x9261x2xf32, 2x9261x2xf32], 1xi32)
