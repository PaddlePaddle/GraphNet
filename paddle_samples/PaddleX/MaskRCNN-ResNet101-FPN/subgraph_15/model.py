...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.slice: (6xf32) <- (6x6xf32, 1xi64, 1xi64)
        slice_1 = paddle._C_ops.slice(
            data_0, [1], full_int_array_2, full_int_array_3, [1], [1]
        )
        del data_0

        # pd_op.cast: (6xi32) <- (6xf32)
        cast_0 = paddle._C_ops.cast(slice_1, paddle.int32)
        del slice_1

        # pd_op.distribute_fpn_proposals: ([-1x4xf32, -1x4xf32, -1x4xf32, -1x4xf32], [-1xi32, -1xi32, -1xi32, -1xi32], -1x1xi32) <- (6x4xf32, 1xi32)
        (
        ...
        # builtin.combine: ([-1xi32, 6xi32]) <- (-1xi32, 6xi32)
        combine_1 = [cast_1, cast_0]
        del cast_0

        # pd_op.broadcast_tensors: ([6xi32, 6xi32]) <- ([-1xi32, 6xi32])
        broadcast_tensors_0 = paddle._C_ops.broadcast_tensors(combine_1)
        del combine_1

        # builtin.split: (6xi32, 6xi32) <- ([6xi32, 6xi32])
        (
            split_8,
            split_9,
        ) = broadcast_tensors_0
        ...
        # builtin.combine: ([6xi32, 6xi32]) <- (6xi32, 6xi32)
        combine_2 = [split_8, split_9]
        del split_8, split_9

        # pd_op.stack: (6x2xi32) <- ([6xi32, 6xi32])
        stack_0 = paddle._C_ops.stack(combine_2, -1)
        del combine_2

        # pd_op.gather_nd: (6x28x28xf32) <- (-1x2x28x28xf32, 6x2xi32)
        ...
        # pd_op.reshape: (-1x28x28xf32) <- (6x28x28xf32, 3xi64)
