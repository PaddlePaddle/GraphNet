...
def main():
    ...
    parser = argparse.ArgumentParser(description="Batch test model extraction success rate")
    parser.add_argument(
        "--count",
        type=int,
        default=100,
        help="Number of models to test (default: 100)"
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    try:
        ...
        if args.task:
            ...
            if len(model_ids) < model_count:
                ...
                additional = get_models_from_hf(task=None, limit=model_count - len(model_ids))
    ...
    print("=" * 70)
    print(f"[INFO] Starting batch test")
