...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.topk: (2x8x9xf32, 2x8x9xi64) <- (2x8x7744xf32, 1xi32)
        ...
        full_2 = paddle._C_ops.full(
            [1], float("7744"), paddle.int32, paddle.core.CPUPlace()
        )

        # pd_op.one_hot: (2x8x9x7744xf32) <- (2x8x9xi64, 1xi32)
        ...
        # pd_op.sum: (2x8x7744xf32) <- (2x8x9x7744xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(one_hot_0, full_int_array_0, None, False)

        # pd_op.multiply: (2x8x7744xf32) <- (2x8x7744xf32, 2x8x1xf32)
        multiply_0 = paddle._C_ops.multiply(sum_0, data_4)

        # pd_op.topk: (2x8x9xf32, 2x8x9xi64) <- (2x8x1936xf32, 1xi32)
        ...
        # pd_op.scale: (2x8x9xi64) <- (2x8x9xi64, 1xf32)
        scale_1 = paddle._C_ops.scale(topk_3, full_1, float("7744"), True)
        ...
        # pd_op.one_hot: (2x8x9x1936xf32) <- (2x8x9xi64, 1xi32)
        one_hot_1 = paddle._C_ops.one_hot(
            topk_3 % paddle.cast(full_3, topk_3.dtype), full_3
        )

        # pd_op.sum: (2x8x1936xf32) <- (2x8x9x1936xf32, 1xi64)
        sum_1 = paddle._C_ops.sum(one_hot_1, full_int_array_0, None, False)

        # pd_op.multiply: (2x8x1936xf32) <- (2x8x1936xf32, 2x8x1xf32)
        multiply_1 = paddle._C_ops.multiply(sum_1, data_4)

        # pd_op.topk: (2x8x9xf32, 2x8x9xi64) <- (2x8x484xf32, 1xi32)
        ...
        # pd_op.scale: (2x8x9xi64) <- (2x8x9xi64, 1xf32)
        scale_2 = paddle._C_ops.scale(topk_5, full_1, float("9680"), True)
        ...
        # pd_op.scale: (2x8x9xi64) <- (2x8x9xi64, 1xf32)
        scale_3 = paddle._C_ops.scale(topk_7, full_1, float("10164"), True)
        ...
        # builtin.combine: ([2x8x7744xf32, 2x8x1936xf32, 2x8x484xf32, 2x8x121xf32]) <- (2x8x7744xf32, 2x8x1936xf32, 2x8x484xf32, 2x8x121xf32)
        combine_0 = [multiply_0, multiply_1, multiply_2, multiply_3]

        # pd_op.concat: (2x8x10285xf32) <- ([2x8x7744xf32, 2x8x1936xf32, 2x8x484xf32, 2x8x121xf32], 1xi32)
