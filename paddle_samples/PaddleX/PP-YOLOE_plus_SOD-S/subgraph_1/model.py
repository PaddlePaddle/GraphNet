...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.argmax: (2x6069xi64) <- (2x13x6069xf32, 1xi64)
        argmax_0 = paddle._C_ops.argmax(data_0, full_0, False, False, paddle.int64)
        del full_0

        # pd_op.full: (1xf32) <- ()
        full_1 = paddle._C_ops.full(
            [1], float("13"), paddle.float32, paddle.core.CPUPlace()
        )
        ...
        # pd_op.add: (2x6069xi64) <- (2x6069xi64, 2x1xi64)
        add_0 = paddle._C_ops.add(argmax_0, cast_0)
        del argmax_0, cast_0

        # pd_op.flatten: (26xi32) <- (2x13x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_2, 0, 2)
        del data_2

        # pd_op.flatten: (12138xi64) <- (2x6069xi64)
        ...
        # pd_op.gather: (12138xi32) <- (26xi32, 12138xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_2)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_0 = [2, 6069]

        # pd_op.reshape: (2x6069xi32) <- (12138xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x6069xb) <- (2x6069xf32, xf32)
        ...
        # pd_op.full_like: (2x6069xi32) <- (2x6069xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_4, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_4

        # pd_op.where: (2x6069xi32) <- (2x6069xb, 2x6069xi32, 2x6069xi32)
        ...
        # pd_op.one_hot: (2x6069x11xf32) <- (2x6069xi32, 1xi32)
        ...
        # pd_op.index_select: (2x6069x10xf32) <- (2x6069x11xf32, 10xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_0, assign_value__0, -1)
        del assign_value__0, one_hot_0

        # pd_op.multiply: (2x13x6069xf32) <- (2x13x6069xf32, 2x13x6069xf32)
        ...
        # pd_op.max: (2x13x1xf32) <- (2x13x6069xf32, 1xi64)
        ...
        # pd_op.scale: (2x13x1xf32) <- (2x13x1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(max_0, full_7, float("1e-09"), True)
        del full_7, max_0

        # pd_op.divide: (2x13x6069xf32) <- (2x13x6069xf32, 2x13x1xf32)
        divide_0 = paddle._C_ops.divide(multiply_1, scale_1)
        del multiply_1, scale_1

        # pd_op.multiply: (2x13x6069xf32) <- (2x13x6069xf32, 2x13x1xf32)
