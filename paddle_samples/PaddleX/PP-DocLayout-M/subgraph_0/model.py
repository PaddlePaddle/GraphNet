...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (1x8500xi64) <- (1x2x8500xf32, 1xi64)
        ...
        arange_0 = paddle.arange(full_1, full_2, full_2, dtype="int32")
        del full_1, full_2
        ...
        scale_0 = paddle._C_ops.scale(unsqueeze_0, full_3, float("0"), True)
        del full_3, unsqueeze_0

        # pd_op.cast: (1x1xi64) <- (1x1xi32)
        cast_0 = paddle._C_ops.cast(scale_0, paddle.int64)
        del scale_0

        # pd_op.add: (1x8500xi64) <- (1x8500xi64, 1x1xi64)
        add_0 = paddle._C_ops.add(argmax_0, cast_0)
        del argmax_0, cast_0

        # pd_op.flatten: (2xi32) <- (1x2x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_1, 0, 2)
        del data_1

        # pd_op.flatten: (8500xi64) <- (1x8500xi64)
        ...
        # pd_op.gather: (8500xi32) <- (2xi32, 8500xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [1, 8500]

        # pd_op.reshape: (1x8500xi32) <- (8500xi32, 2xi64)
        ...
        # pd_op.greater_than: (1x8500xb) <- (1x8500xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(data_2, full_5)
        del data_2, full_5
        ...
        # pd_op.full_like: (1x8500xi32) <- (1x8500xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_6

        # pd_op.where: (1x8500xi32) <- (1x8500xb, 1x8500xi32, 1x8500xi32)
        ...
        # pd_op.reshape: (2x4xf32) <- (1x2x4xf32, 2xi64)
        reshape_2 = paddle._C_ops.reshape(data_3, full_int_array_2)
        del full_int_array_2

        # pd_op.gather: (8500x4xf32) <- (2x4xf32, 8500xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_4)
        del flatten_1, full_4, reshape_2

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [1, 8500, 4]

        # pd_op.reshape: (1x8500x4xf32) <- (8500x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (1x8500x12xf32) <- (1x8500xi32, 1xi32)
        ...
        # pd_op.index_select: (1x8500x11xf32) <- (1x8500x12xf32, 11xi64)
        ...
        # pd_op.unsqueeze: (1x2x1x4xf32) <- (1x2x4xf32, 1xi64)
        ...
        # pd_op.unsqueeze: (1x1x8500x4xf32) <- (1x8500x4xf32, 1xi64)
        ...
        # pd_op.slice: (1x2x1x2xf32) <- (1x2x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.slice: (1x1x8500x2xf32) <- (1x1x8500x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (1x2x8500x2xf32) <- (1x2x1x2xf32, 1x1x8500x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (1x2x8500x2xf32) <- (1x2x1x2xf32, 1x1x8500x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (1x2x8500x2xf32) <- (1x2x8500x2xf32, 1x2x8500x2xf32)
        ...
        # pd_op.clip: (1x2x8500x2xf32) <- (1x2x8500x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_9, full_10)
        del subtract_0

        # pd_op.prod: (1x2x8500xf32) <- (1x2x8500x2xf32, 1xi64)
        prod_0 = paddle._C_ops.prod(clip_0, full_int_array_0, False, False)
        del clip_0

        # pd_op.subtract: (1x2x1x2xf32) <- (1x2x1x2xf32, 1x2x1x2xf32)
        subtract_1 = paddle._C_ops.subtract(slice_1, slice_0)
        del slice_0, slice_1

        # pd_op.clip: (1x2x1x2xf32) <- (1x2x1x2xf32, 1xf32, 1xf32)
        clip_1 = paddle._C_ops.clip(subtract_1, full_9, full_10)
        del subtract_1

        # pd_op.prod: (1x2x1xf32) <- (1x2x1x2xf32, 1xi64)
        prod_1 = paddle._C_ops.prod(clip_1, full_int_array_0, False, False)
        del clip_1

        # pd_op.subtract: (1x1x8500x2xf32) <- (1x1x8500x2xf32, 1x1x8500x2xf32)
        subtract_2 = paddle._C_ops.subtract(slice_3, slice_2)
        del slice_2, slice_3

        # pd_op.clip: (1x1x8500x2xf32) <- (1x1x8500x2xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(subtract_2, full_9, full_10)
        del full_10, full_9, subtract_2

        # pd_op.prod: (1x1x8500xf32) <- (1x1x8500x2xf32, 1xi64)
        prod_2 = paddle._C_ops.prod(clip_2, full_int_array_0, False, False)
        del clip_2

        # pd_op.add: (1x2x8500xf32) <- (1x2x1xf32, 1x1x8500xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)
        del prod_1, prod_2

        # pd_op.subtract: (1x2x8500xf32) <- (1x2x8500xf32, 1x2x8500xf32)
        ...
        # pd_op.scale: (1x2x8500xf32) <- (1x2x8500xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_11, float("1e-09"), True)
        del full_11, subtract_3

        # pd_op.divide: (1x2x8500xf32) <- (1x2x8500xf32, 1x2x8500xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_1)
        del prod_0, scale_1

        # pd_op.multiply: (1x2x8500xf32) <- (1x2x8500xf32, 1x2x8500xf32)
