...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3):
        ...
        full_int_array_2 = [1, 10]

        # pd_op.tile: (9261x10xf32) <- (9261x1xf32, 2xi64)
        ...
        # pd_op.slice: (10xf32) <- (10x4xf32, 1xi64, 1xi64)
        slice_5 = paddle._C_ops.slice(
            data_3, [1], full_int_array_0, full_int_array_1, [1], [1]
        )

        # pd_op.subtract: (9261x10xf32) <- (9261x10xf32, 10xf32)
        ...
        # pd_op.subtract: (9261x10xf32) <- (10xf32, 9261x10xf32)
        ...
        # builtin.combine: ([9261x10xf32, 9261x10xf32, 9261x10xf32, 9261x10xf32]) <- (9261x10xf32, 9261x10xf32, 9261x10xf32, 9261x10xf32)
        combine_0 = [subtract_0, subtract_1, subtract_2, subtract_3]

        # pd_op.stack: (9261x4x10xf32) <- ([9261x10xf32, 9261x10xf32, 9261x10xf32, 9261x10xf32])
        stack_0 = paddle._C_ops.stack(combine_0, 1)

        # pd_op.min: (9261x10xf32) <- (9261x4x10xf32, 1xi64)
        ...
        # pd_op.greater_than: (9261x10xb) <- (9261x10xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(min_0, full_0)

        # pd_op.sum: (9261xi64) <- (9261x10xb, 1xi64)
        ...
        # pd_op.add: (10xf32) <- (10xf32, 10xf32)
        ...
        # pd_op.scale: (9261x10xf32) <- (9261x10xf32, 1xf32)
        ...
        # pd_op.add: (9261x10xf32) <- (10xf32, 9261x10xf32)
        add_2 = paddle._C_ops.add(scale_0, scale_2)

        # pd_op.add: (9261x10xf32) <- (10xf32, 9261x10xf32)
        add_3 = paddle._C_ops.add(scale_1, scale_3)

        # pd_op.subtract: (9261x10xf32) <- (9261x10xf32, 9261x10xf32)
        ...
        # pd_op.cast: (9261x10xi64) <- (9261x10xb)
        ...
        # pd_op.gather: (-1x10xi64) <- (9261x10xi64, -1xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(cast_0, squeeze_0, full_4)

        # pd_op.cast: (-1x10xb) <- (-1x10xi64)
        ...
        # pd_op.logical_and: (-1x10xb) <- (-1x10xb, -1x10xb)
