...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        shape64_0 = paddle._C_ops.shape64(data_10)
        ...
        # pd_op.matmul: (4x500x192xf32) <- (4x500x256xf32, 256x192xf32)
        matmul_1 = paddle._C_ops.matmul(data_8, parameter_5, False, False)

        # pd_op.add: (4x500x192xf32) <- (4x500x192xf32, 192xf32)
        add_1 = paddle._C_ops.add(matmul_1, parameter_4)

        # pd_op.full_int_array: (6xi64) <- ()
        full_int_array_3 = [4, 500, 8, 3, 4, 2]

        # pd_op.reshape: (4x500x8x3x4x2xf32) <- (4x500x192xf32, 6xi64)
        reshape_5 = paddle._C_ops.reshape(add_1, full_int_array_3)

        # pd_op.matmul: (4x500x96xf32) <- (4x500x256xf32, 256x96xf32)
        matmul_2 = paddle._C_ops.matmul(data_8, parameter_3, False, False)

        # pd_op.add: (4x500x96xf32) <- (4x500x96xf32, 96xf32)
        add_2 = paddle._C_ops.add(matmul_2, parameter_2)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_4 = [4, 500, 8, 12]

        # pd_op.reshape: (4x500x8x12xf32) <- (4x500x96xf32, 4xi64)
        reshape_6 = paddle._C_ops.reshape(add_2, full_int_array_4)

        # pd_op.softmax: (4x500x8x12xf32) <- (4x500x8x12xf32)
        softmax_0 = paddle._C_ops.softmax(reshape_6, -1)

        # pd_op.full_int_array: (5xi64) <- ()
        full_int_array_5 = [4, 500, 8, 3, 4]

        # pd_op.reshape: (4x500x8x3x4xf32) <- (4x500x8x12xf32, 5xi64)
        ...
        # pd_op.slice: (4x500x1x2xf32) <- (4x500x1x4xf32, 1xi64, 1xi64)
        slice_1 = paddle._C_ops.slice(
            data_9, [3], full_int_array_6, full_int_array_2, [1], []
        )

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_7 = [2, 4]

        # pd_op.unsqueeze: (4x500x1x1x1x2xf32) <- (4x500x1x2xf32, 2xi64)
        ...
        # pd_op.scale: (4x500x8x3x4x2xf32) <- (4x500x8x3x4x2xf32, 1xf32)
        ...
        slice_2 = paddle._C_ops.slice(
            data_9, [3], full_int_array_2, full_int_array_8, [1], []
        )

        # pd_op.unsqueeze: (4x500x1x1x1x2xf32) <- (4x500x1x2xf32, 2xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(slice_2, full_int_array_7)

        # pd_op.multiply: (4x500x8x3x4x2xf32) <- (4x500x8x3x4x2xf32, 4x500x1x1x1x2xf32)
        ...
        # pd_op.add: (4x500x8x3x4x2xf32) <- (4x500x1x1x1x2xf32, 4x500x8x3x4x2xf32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_7 = [full_7, full_7, slice_5, slice_6]
        ...
        # pd_op.slice: (4x500x8x4x2xf32) <- (4x500x8x3x4x2xf32, 1xi64, 1xi64)
        slice_13 = paddle._C_ops.slice(
            scale_2, [3], full_int_array_6, full_int_array_1, [1], [3]
        )

        # pd_op.transpose: (4x8x500x4x2xf32) <- (4x500x8x4x2xf32)
        transpose_1 = paddle._C_ops.transpose(slice_13, [0, 2, 1, 3, 4])

        # pd_op.flatten: (32x500x4x2xf32) <- (4x8x500x4x2xf32)
        flatten_0 = paddle._C_ops.flatten(transpose_1, 0, 1)

        # pd_op.grid_sample: (32x32x500x4xf32) <- (32x32x-1x-1xf32, 32x500x4x2xf32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_8 = [full_7, full_7, slice_8, slice_9]
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_9 = [full_7, full_7, slice_11, slice_12]
        ...
        # pd_op.transpose: (4x8x500x3x4xf32) <- (4x500x8x3x4xf32)
        transpose_6 = paddle._C_ops.transpose(reshape_7, [0, 2, 1, 3, 4])

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_10 = [32, 1, 500, 12]

        # pd_op.reshape: (32x1x500x12xf32) <- (4x8x500x3x4xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(transpose_6, full_int_array_10)

        # builtin.combine: ([32x32x500x4xf32, 32x32x500x4xf32, 32x32x500x4xf32]) <- (32x32x500x4xf32, 32x32x500x4xf32, 32x32x500x4xf32)
        combine_10 = [grid_sample_0, grid_sample_1, grid_sample_2]

        # pd_op.stack: (32x32x500x3x4xf32) <- ([32x32x500x4xf32, 32x32x500x4xf32, 32x32x500x4xf32])
        stack_0 = paddle._C_ops.stack(combine_10, -2)

        # pd_op.flatten: (32x32x500x12xf32) <- (32x32x500x3x4xf32)
        flatten_3 = paddle._C_ops.flatten(stack_0, 3, 4)

        # pd_op.multiply: (32x32x500x12xf32) <- (32x32x500x12xf32, 32x1x500x12xf32)
        multiply_0 = paddle._C_ops.multiply(flatten_3, reshape_3)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [-1]

        # pd_op.sum: (32x32x500xf32) <- (32x32x500x12xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(multiply_0, full_int_array_0, None, False)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_11 = [4, 256, 500]

        # pd_op.reshape: (4x256x500xf32) <- (32x32x500xf32, 3xi64)
