...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (8x128x72x72xf32) <- (8x128x72x72xf32, 128x128x3x3xf32)
        ...
        # pd_op.conv2d: (8x4x72x72xf32) <- (8x128x72x72xf32, 4x128x1x1xf32)
        ...
        # pd_op.conv2d: (8x5x72x72xf32) <- (8x128x72x72xf32, 5x128x1x1xf32)
        ...
        # pd_op.add: (8x5x72x72xf32) <- (8x5x72x72xf32, 1x5x1x1xf32)
        ...
        # pd_op.split: ([8x4x72x72xf32, 8x1x72x72xf32]) <- (8x5x72x72xf32, 2xi64, 1xi32)
        split_4 = paddle._C_ops.split(add_3, full_int_array_1, full_0)

        # builtin.split: (8x4x72x72xf32, 8x1x72x72xf32) <- ([8x4x72x72xf32, 8x1x72x72xf32])
        (
            split_0,
            split_5,
        ) = split_4
        ...
        # pd_op.flatten: (8x1x5184xf32) <- (8x1x72x72xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_6, 2, 3)

        # pd_op.transpose: (8x5184x1xf32) <- (8x1x5184xf32)
        ...
        # pd_op.conv2d: (8x4x36x36xf32) <- (8x128x36x36xf32, 4x128x1x1xf32)
        ...
        # pd_op.conv2d: (8x5x36x36xf32) <- (8x128x36x36xf32, 5x128x1x1xf32)
        ...
        # pd_op.add: (8x5x36x36xf32) <- (8x5x36x36xf32, 1x5x1x1xf32)
        ...
        # pd_op.split: ([8x4x36x36xf32, 8x1x36x36xf32]) <- (8x5x36x36xf32, 2xi64, 1xi32)
        split_6 = paddle._C_ops.split(add_5, full_int_array_1, full_0)

        # builtin.split: (8x4x36x36xf32, 8x1x36x36xf32) <- ([8x4x36x36xf32, 8x1x36x36xf32])
        (
            split_1,
            split_7,
        ) = split_6
        ...
        # pd_op.flatten: (8x1x1296xf32) <- (8x1x36x36xf32)
        flatten_5 = paddle._C_ops.flatten(sigmoid_13, 2, 3)

        # pd_op.transpose: (8x1296x1xf32) <- (8x1x1296xf32)
        transpose_5 = paddle._C_ops.transpose(flatten_5, [0, 2, 1])

        # pd_op.conv2d: (8x128x18x18xf32) <- (8x512x18x18xf32, 128x512x1x1xf32)
        conv2d_14 = paddle._C_ops.conv2d(
            data_2, parameter_28, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (8x128x18x18xf32, 128xf32, 128xf32, 128xf32, 128xf32, -1xui8) <- (8x128x18x18xf32, 128xf32, 128xf32, 128xf32, 128xf32)
        (
            batch_norm__60,
            batch_norm__61,
            batch_norm__62,
        ...
        # pd_op.sigmoid: (8x128x18x18xf32) <- (8x128x18x18xf32)
        sigmoid_14 = paddle._C_ops.sigmoid(batch_norm__60)

        # pd_op.multiply: (8x128x18x18xf32) <- (8x128x18x18xf32, 8x128x18x18xf32)
        multiply_10 = paddle._C_ops.multiply(batch_norm__60, sigmoid_14)

        # pd_op.conv2d: (8x128x18x18xf32) <- (8x128x18x18xf32, 128x128x3x3xf32)
        ...
        # pd_op.conv2d: (8x4x18x18xf32) <- (8x128x18x18xf32, 4x128x1x1xf32)
        ...
        # pd_op.conv2d: (8x5x18x18xf32) <- (8x128x18x18xf32, 5x128x1x1xf32)
        ...
        # pd_op.add: (8x5x18x18xf32) <- (8x5x18x18xf32, 1x5x1x1xf32)
        ...
        # pd_op.split: ([8x4x18x18xf32, 8x1x18x18xf32]) <- (8x5x18x18xf32, 2xi64, 1xi32)
        split_8 = paddle._C_ops.split(add_7, full_int_array_1, full_0)

        # builtin.split: (8x4x18x18xf32, 8x1x18x18xf32) <- ([8x4x18x18xf32, 8x1x18x18xf32])
        (
            split_2,
            split_9,
        ) = split_8
        ...
        # pd_op.flatten: (8x1x324xf32) <- (8x1x18x18xf32)
        flatten_8 = paddle._C_ops.flatten(sigmoid_20, 2, 3)

        # pd_op.transpose: (8x324x1xf32) <- (8x1x324xf32)
        transpose_8 = paddle._C_ops.transpose(flatten_8, [0, 2, 1])

        # builtin.combine: ([8x5184x4xf32, 8x1296x4xf32, 8x324x4xf32]) <- (8x5184x4xf32, 8x1296x4xf32, 8x324x4xf32)
        combine_0 = [transpose_0, transpose_3, transpose_6]

        # pd_op.concat: (8x6804x4xf32) <- ([8x5184x4xf32, 8x1296x4xf32, 8x324x4xf32], 1xi32)
        ...
        # builtin.combine: ([8x5184x1xf32, 8x1296x1xf32, 8x324x1xf32]) <- (8x5184x1xf32, 8x1296x1xf32, 8x324x1xf32)
        combine_2 = [transpose_2, transpose_5, transpose_8]

        # pd_op.concat: (8x6804x1xf32) <- ([8x5184x1xf32, 8x1296x1xf32, 8x324x1xf32], 1xi32)
        ...
        # builtin.combine: ([5184x2xf32, 1296x2xf32, 324x2xf32]) <- (5184x2xf32, 1296x2xf32, 324x2xf32)
        combine_9 = [reshape_9, reshape_10, reshape_11]

        # pd_op.concat: (6804x2xf32) <- ([5184x2xf32, 1296x2xf32, 324x2xf32], 1xi32)
        concat_5 = paddle._C_ops.concat(combine_9, full_16)

        # builtin.combine: ([5184x1xf32, 1296x1xf32, 324x1xf32]) <- (5184x1xf32, 1296x1xf32, 324x1xf32)
        combine_10 = [full_9, full_12, full_15]

        # pd_op.concat: (6804x1xf32) <- ([5184x1xf32, 1296x1xf32, 324x1xf32], 1xi32)
        concat_6 = paddle._C_ops.concat(combine_10, full_16)

        # pd_op.assign: (6804x1xf32) <- (6804x1xf32)
        assign_5 = concat_6
        ...
        # pd_op.split_with_num: ([8x6804x2xf32, 8x6804x2xf32]) <- (8x6804x4xf32, 1xi32)
        split_with_num_0 = paddle._C_ops.split_with_num(concat_4, 2, full_1)

        # builtin.split: (8x6804x2xf32, 8x6804x2xf32) <- ([8x6804x2xf32, 8x6804x2xf32])
        (
            split_3,
            split_16,
        ) = split_with_num_0

        # pd_op.divide: (6804x2xf32) <- (6804x2xf32, 6804x1xf32)
        divide_0 = paddle._C_ops.divide(concat_5, concat_6)

        # pd_op.add: (8x6804x2xf32) <- (8x6804x2xf32, 6804x2xf32)
        add_0 = paddle._C_ops.add(split_3, divide_0)

        # pd_op.exp: (8x6804x2xf32) <- (8x6804x2xf32)
        ...
        # pd_op.scale: (8x6804x2xf32) <- (8x6804x2xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(exp_0, full_2, float("0"), True)

        # pd_op.subtract: (8x6804x2xf32) <- (8x6804x2xf32, 8x6804x2xf32)
        subtract_0 = paddle._C_ops.subtract(add_0, scale_0)

        # pd_op.add: (8x6804x2xf32) <- (8x6804x2xf32, 8x6804x2xf32)
        ...
        # builtin.combine: ([8x6804x2xf32, 8x6804x2xf32]) <- (8x6804x2xf32, 8x6804x2xf32)
        combine_11 = [subtract_0, add_1]

        # pd_op.concat: (8x6804x4xf32) <- ([8x6804x2xf32, 8x6804x2xf32], 1xi32)
