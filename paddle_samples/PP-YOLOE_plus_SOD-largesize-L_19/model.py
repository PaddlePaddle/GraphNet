...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.unsqueeze: (1x1x44436xf32) <- (1x44436xf32, 1xi64)
        ...
        # pd_op.greater_than: (1x1x44436xb) <- (1x1x44436xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(unsqueeze_0, full_0)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_1 = [1, 38, 1]

        # pd_op.tile: (1x38x44436xb) <- (1x1x44436xb, 3xi64)
        ...
        # pd_op.argmax: (1x44436xi64) <- (1x38x44436xf32, 1xi64)
        ...
        # pd_op.one_hot: (1x44436x38xf32) <- (1x44436xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )

        # pd_op.transpose: (1x38x44436xf32) <- (1x44436x38xf32)
        ...
        # pd_op.gather: (44436xi32) <- (38xi32, 44436xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_3 = [1, 44436]

        # pd_op.reshape: (1x44436xi32) <- (44436xi32, 2xi64)
        ...
        # pd_op.greater_than: (1x44436xb) <- (1x44436xf32, xf32)
        ...
        # pd_op.full_like: (1x44436xi32) <- (1x44436xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )

        # pd_op.where: (1x44436xi32) <- (1x44436xb, 1x44436xi32, 1x44436xi32)
        ...
        # pd_op.gather: (44436x4xf32) <- (38x4xf32, 44436xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_4)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [1, 44436, 4]

        # pd_op.reshape: (1x44436x4xf32) <- (44436x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (1x44436x11xf32) <- (1x44436xi32, 1xi32)
        ...
        # pd_op.index_select: (1x44436x10xf32) <- (1x44436x11xf32, 10xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_1, assign_value__0, -1)

        # pd_op.multiply: (1x38x44436xf32) <- (1x38x44436xf32, 1x38x44436xf32)
        multiply_2 = paddle._C_ops.multiply(data_6, where_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_6 = [-1]

        # pd_op.max: (1x38x1xf32) <- (1x38x44436xf32, 1xi64)
        max_0 = paddle._C_ops.max(multiply_2, full_int_array_6, True)

        # pd_op.multiply: (1x38x44436xf32) <- (1x38x44436xf32, 1x38x44436xf32)
        multiply_3 = paddle._C_ops.multiply(data_1, where_1)
        ...
        # pd_op.divide: (1x38x44436xf32) <- (1x38x44436xf32, 1x38x1xf32)
        ...
        # pd_op.max: (1x44436xf32) <- (1x38x44436xf32, 1xi64)
        max_2 = paddle._C_ops.max(multiply_4, full_int_array_2, False)

        # pd_op.unsqueeze: (1x44436x1xf32) <- (1x44436xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(max_2, full_int_array_6)

        # pd_op.multiply: (1x44436x10xf32) <- (1x44436x10xf32, 1x44436x1xf32)
