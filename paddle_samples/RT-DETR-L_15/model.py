...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.shape64: (3xi64) <- (8x-1x256xf32)
        shape64_1 = paddle._C_ops.shape64(data_12)
        ...
        combine_0 = [full_5, slice_1, full_5, full_6]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_1 = paddle._C_ops.stack(combine_0, 0)

        # pd_op.reshape: (8x-1x8x32xf32) <- (8x-1x256xf32, 4xi64)
        reshape_4 = paddle._C_ops.reshape(add_2, stack_1)

        # pd_op.matmul: (8x-1x192xf32) <- (8x-1x256xf32, 256x192xf32)
        matmul_1 = paddle._C_ops.matmul(add_1, parameter_13, False, False)

        # pd_op.add: (8x-1x192xf32) <- (8x-1x192xf32, 192xf32)
        ...
        combine_1 = [full_5, slice_0, full_5, full_7, full_8, full_9]

        # pd_op.stack: (6xi64) <- ([xi64, xi64, xi64, xi64, xi64, xi64])
        stack_2 = paddle._C_ops.stack(combine_1, 0)

        # pd_op.reshape: (8x-1x8x3x4x2xf32) <- (8x-1x192xf32, 6xi64)
        reshape_5 = paddle._C_ops.reshape(add_3, stack_2)

        # pd_op.matmul: (8x-1x96xf32) <- (8x-1x256xf32, 256x96xf32)
        matmul_2 = paddle._C_ops.matmul(add_1, parameter_11, False, False)

        # pd_op.add: (8x-1x96xf32) <- (8x-1x96xf32, 96xf32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_2 = [full_5, slice_0, full_5, full_10]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_3 = paddle._C_ops.stack(combine_2, 0)

        # pd_op.reshape: (8x-1x8x12xf32) <- (8x-1x96xf32, 4xi64)
        reshape_6 = paddle._C_ops.reshape(add_4, stack_3)

        # pd_op.softmax: (8x-1x8x12xf32) <- (8x-1x8x12xf32)
        softmax_0 = paddle._C_ops.softmax(reshape_6, -1)

        # builtin.combine: ([xi64, xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64, xi64)
        combine_3 = [full_5, slice_0, full_5, full_7, full_8]

        # pd_op.stack: (5xi64) <- ([xi64, xi64, xi64, xi64, xi64])
        stack_4 = paddle._C_ops.stack(combine_3, 0)

        # pd_op.reshape: (8x-1x8x3x4xf32) <- (8x-1x8x12xf32, 5xi64)
        reshape_7 = paddle._C_ops.reshape(softmax_0, stack_4)

        # pd_op.shape64: (4xi64) <- (8x-1x1x4xf32)
        shape64_2 = paddle._C_ops.shape64(data_11)
        ...
        slice_3 = paddle._C_ops.slice(
            data_11, [3], full_int_array_3, full_int_array_2, [1], []
        )

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_4 = [2, 4]

        # pd_op.unsqueeze: (8x-1x1x1x1x2xf32) <- (8x-1x1x2xf32, 2xi64)
        ...
        # pd_op.scale: (8x-1x8x3x4x2xf32) <- (8x-1x8x3x4x2xf32, 1xf32)
        ...
        slice_4 = paddle._C_ops.slice(
            data_11, [3], full_int_array_2, full_int_array_5, [1], []
        )

        # pd_op.unsqueeze: (8x-1x1x1x1x2xf32) <- (8x-1x1x2xf32, 2xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(slice_4, full_int_array_4)

        # pd_op.multiply: (8x-1x8x3x4x2xf32) <- (8x-1x8x3x4x2xf32, 8x-1x1x1x1x2xf32)
        ...
        # pd_op.add: (8x-1x8x3x4x2xf32) <- (8x-1x1x1x1x2xf32, 8x-1x8x3x4x2xf32)
        ...
        # pd_op.shape64: (4xi64) <- (8x-1x8x32xf32)
        ...
        # pd_op.shape64: (6xi64) <- (8x-1x8x3x4x2xf32)
        shape64_4 = paddle._C_ops.shape64(add_9)
        ...
        # pd_op.split: ([-1x-1x-1x-1xf32, -1x-1x-1x-1xf32, -1x-1x-1x-1xf32]) <- (8x-1x8x32xf32, 3xi64, 1xi32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_10 = [full_12, full_6, slice_8, slice_9]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_11 = paddle._C_ops.stack(combine_10, 0)

        # pd_op.reshape: (64x32x-1x-1xf32) <- (-1x-1x-1xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(transpose_0, stack_11)

        # pd_op.slice: (8x-1x8x4x2xf32) <- (8x-1x8x3x4x2xf32, 1xi64, 1xi64)
        slice_16 = paddle._C_ops.slice(
            scale_2, [3], full_int_array_3, full_int_array_1, [1], [3]
        )

        # pd_op.transpose: (8x8x-1x4x2xf32) <- (8x-1x8x4x2xf32)
        transpose_1 = paddle._C_ops.transpose(slice_16, [0, 2, 1, 3, 4])

        # pd_op.flatten: (64x-1x4x2xf32) <- (8x8x-1x4x2xf32)
        flatten_0 = paddle._C_ops.flatten(transpose_1, 0, 1)

        # pd_op.grid_sample: (64x32x-1x4xf32) <- (64x32x-1x-1xf32, 64x-1x4x2xf32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_11 = [full_12, full_6, slice_11, slice_12]
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_12 = [full_12, full_6, slice_14, slice_15]
        ...
        # pd_op.transpose: (8x8x-1x3x4xf32) <- (8x-1x8x3x4xf32)
        ...
        # pd_op.reshape: (64x1x-1x12xf32) <- (8x8x-1x3x4xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(transpose_6, stack_14)

        # builtin.combine: ([64x32x-1x4xf32, 64x32x-1x4xf32, 64x32x-1x4xf32]) <- (64x32x-1x4xf32, 64x32x-1x4xf32, 64x32x-1x4xf32)
        combine_14 = [grid_sample_0, grid_sample_1, grid_sample_2]

        # pd_op.stack: (64x32x-1x3x4xf32) <- ([64x32x-1x4xf32, 64x32x-1x4xf32, 64x32x-1x4xf32])
        stack_0 = paddle._C_ops.stack(combine_14, -2)

        # pd_op.flatten: (64x32x-1x12xf32) <- (64x32x-1x3x4xf32)
        flatten_3 = paddle._C_ops.flatten(stack_0, 3, 4)

        # pd_op.multiply: (64x32x-1x12xf32) <- (64x32x-1x12xf32, 64x1x-1x12xf32)
        multiply_0 = paddle._C_ops.multiply(flatten_3, reshape_3)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [-1]

        # pd_op.sum: (64x32x-1xf32) <- (64x32x-1x12xf32, 1xi64)
        ...
        # pd_op.reshape: (8x256x-1xf32) <- (64x32x-1xf32, 3xi64)
        ...
        # pd_op.relu: (8x-1x1024xf32) <- (8x-1x1024xf32)
