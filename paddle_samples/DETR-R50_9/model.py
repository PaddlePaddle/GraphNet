...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, parameter_1, data_0, data_1):
        # pd_op.conv2d: (4x256x34x30xf32) <- (4x2048x34x30xf32, 256x2048x1x1xf32)
        ...
        # pd_op.add: (4x256x34x30xf32) <- (4x256x34x30xf32, 1x256x1x1xf32)
        add_0 = paddle._C_ops.add(conv2d_0, reshape_0)

        # pd_op.flatten: (4x256x1020xf32) <- (4x256x34x30xf32)
        flatten_1 = paddle._C_ops.flatten(add_0, 2, 3)

        # pd_op.transpose: (4x1020x256xf32) <- (4x256x1020xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_1 = [0]

        # pd_op.unsqueeze: (1x4x1069x929xf32) <- (4x1069x929xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(data_1, full_int_array_1)

        # pd_op.nearest_interp: (1x4x34x30xf32) <- (1x4x1069x929xf32, None, None, None)
        nearest_interp_0 = paddle._C_ops.nearest_interp(
            unsqueeze_0, None, None, None, "NCHW", -1, 34, 30, [], "nearest", False, 0
        )

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [1]

        # pd_op.slice: (4x34x30xf32) <- (1x4x34x30xf32, 1xi64, 1xi64)
        slice_0 = paddle._C_ops.slice(
            nearest_interp_0, [0], full_int_array_1, full_int_array_2, [1], [0]
        )
        ...
        # pd_op.cumsum: (4x34x30xf32) <- (4x34x30xf32, 1xi32)
        cumsum_0 = paddle._C_ops.cumsum(slice_0, full_0, False, False, False)
        ...
        # pd_op.cumsum: (4x34x30xf32) <- (4x34x30xf32, 1xi32)
        cumsum_1 = paddle._C_ops.cumsum(slice_0, full_1, False, False, False)
        ...
        # pd_op.scale: (4x34x30xf32) <- (4x34x30xf32, 1xf32)
        ...
        # pd_op.slice: (4x1x30xf32) <- (4x34x30xf32, 1xi64, 1xi64)
        slice_1 = paddle._C_ops.slice(
            cumsum_0, [1], full_int_array_3, full_int_array_4, [1], []
        )

        # pd_op.scale: (4x1x30xf32) <- (4x1x30xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(slice_1, full_2, float("1e-06"), True)

        # pd_op.divide: (4x34x30xf32) <- (4x34x30xf32, 4x1x30xf32)
        ...
        # pd_op.slice: (4x34x1xf32) <- (4x34x30xf32, 1xi64, 1xi64)
        slice_2 = paddle._C_ops.slice(
            cumsum_1, [2], full_int_array_3, full_int_array_4, [1], []
        )

        # pd_op.scale: (4x34x1xf32) <- (4x34x1xf32, 1xf32)
        scale_4 = paddle._C_ops.scale(slice_2, full_2, float("1e-06"), True)

        # pd_op.divide: (4x34x30xf32) <- (4x34x30xf32, 4x34x1xf32)
        ...
        # pd_op.unsqueeze: (4x34x30x1xf32) <- (4x34x30xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(scale_5, full_int_array_3)

        # pd_op.divide: (4x34x30x128xf32) <- (4x34x30x1xf32, 128xf32)
        divide_2 = paddle._C_ops.divide(unsqueeze_1, elementwise_pow_0)

        # pd_op.unsqueeze: (4x34x30x1xf32) <- (4x34x30xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(scale_2, full_int_array_3)
        ...
        # pd_op.strided_slice: (4x34x30x64xf32) <- (4x34x30x128xf32, 1xi64, 1xi64, 1xi64)
        strided_slice_0 = paddle._C_ops.strided_slice(
            divide_2, [3], full_int_array_1, full_int_array_4, full_int_array_5
        )

        # pd_op.sin: (4x34x30x64xf32) <- (4x34x30x64xf32)
        sin_0 = paddle._C_ops.sin(strided_slice_0)

        # pd_op.strided_slice: (4x34x30x64xf32) <- (4x34x30x128xf32, 1xi64, 1xi64, 1xi64)
        strided_slice_1 = paddle._C_ops.strided_slice(
            divide_2, [3], full_int_array_2, full_int_array_4, full_int_array_5
        )

        # pd_op.cos: (4x34x30x64xf32) <- (4x34x30x64xf32)
        cos_0 = paddle._C_ops.cos(strided_slice_1)

        # builtin.combine: ([4x34x30x64xf32, 4x34x30x64xf32]) <- (4x34x30x64xf32, 4x34x30x64xf32)
        combine_0 = [sin_0, cos_0]

        # pd_op.stack: (4x34x30x64x2xf32) <- ([4x34x30x64xf32, 4x34x30x64xf32])
        stack_0 = paddle._C_ops.stack(combine_0, 4)

        # pd_op.flatten: (4x34x30x128xf32) <- (4x34x30x64x2xf32)
        flatten_2 = paddle._C_ops.flatten(stack_0, 3, 4)

        # pd_op.strided_slice: (4x34x30x64xf32) <- (4x34x30x128xf32, 1xi64, 1xi64, 1xi64)
        strided_slice_2 = paddle._C_ops.strided_slice(
            divide_3, [3], full_int_array_1, full_int_array_4, full_int_array_5
        )
        ...
        strided_slice_3 = paddle._C_ops.strided_slice(
            divide_3, [3], full_int_array_2, full_int_array_4, full_int_array_5
        )
        ...
        # builtin.combine: ([4x34x30x128xf32, 4x34x30x128xf32]) <- (4x34x30x128xf32, 4x34x30x128xf32)
        combine_2 = [flatten_3, flatten_2]

        # pd_op.concat: (4x34x30x256xf32) <- ([4x34x30x128xf32, 4x34x30x128xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_2, full_11)

        # pd_op.flatten: (4x1020x256xf32) <- (4x34x30x256xf32)
        flatten_0 = paddle._C_ops.flatten(concat_0, 1, 2)

        # pd_op.scale: (4x34x30xf32) <- (4x34x30xf32, 1xf32)
        scale_8 = paddle._C_ops.scale(slice_0, full_2, float("-1"), True)
        ...
        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_6 = [4, 1, 1, 1020]

        # pd_op.reshape: (4x1x1x1020xf32) <- (4x34x30xf32, 4xi64)
        reshape_1 = paddle._C_ops.reshape(scale_9, full_int_array_6)
        return conv2d_0, reshape_0, transpose_0, reshape_1, flatten_0, add_0
