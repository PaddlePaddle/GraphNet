...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (2x4165xi64) <- (2x7x4165xf32, 1xi64)
        ...
        # pd_op.add: (2x4165xi64) <- (2x4165xi64, 2x1xi64)
        add_0 = paddle._C_ops.add(argmax_0, cast_0)

        # pd_op.flatten: (14xi32) <- (2x7x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_1, 0, 2)

        # pd_op.flatten: (8330xi64) <- (2x4165xi64)
        ...
        # pd_op.gather: (8330xi32) <- (14xi32, 8330xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_5)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [2, 4165]

        # pd_op.reshape: (2x4165xi32) <- (8330xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x4165xb) <- (2x4165xf32, xf32)
        ...
        # pd_op.one_hot: (2x4165x12xf32) <- (2x4165xi32, 1xi32)
        ...
        # pd_op.index_select: (2x4165x11xf32) <- (2x4165x12xf32, 11xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_0, assign_value__0, -1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_4 = [2]

        # pd_op.unsqueeze: (2x7x1x4xf32) <- (2x7x4xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(data_3, full_int_array_4)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_5 = [1]

        # pd_op.unsqueeze: (2x1x4165x4xf32) <- (2x4165x4xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(data_4, full_int_array_5)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_6 = [0]

        # pd_op.slice: (2x7x1x2xf32) <- (2x7x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.slice: (2x1x4165x2xf32) <- (2x1x4165x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (2x7x4165x2xf32) <- (2x7x1x2xf32, 2x1x4165x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (2x7x4165x2xf32) <- (2x7x1x2xf32, 2x1x4165x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (2x7x4165x2xf32) <- (2x7x4165x2xf32, 2x7x4165x2xf32)
        ...
        # pd_op.clip: (2x7x4165x2xf32) <- (2x7x4165x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_10, full_11)

        # pd_op.prod: (2x7x4165xf32) <- (2x7x4165x2xf32, 1xi64)
        prod_0 = paddle._C_ops.prod(clip_0, full_int_array_0, False, False)

        # pd_op.subtract: (2x7x1x2xf32) <- (2x7x1x2xf32, 2x7x1x2xf32)
        subtract_1 = paddle._C_ops.subtract(slice_1, slice_0)

        # pd_op.clip: (2x7x1x2xf32) <- (2x7x1x2xf32, 1xf32, 1xf32)
        clip_1 = paddle._C_ops.clip(subtract_1, full_10, full_11)

        # pd_op.prod: (2x7x1xf32) <- (2x7x1x2xf32, 1xi64)
        prod_1 = paddle._C_ops.prod(clip_1, full_int_array_0, False, False)

        # pd_op.subtract: (2x1x4165x2xf32) <- (2x1x4165x2xf32, 2x1x4165x2xf32)
        subtract_2 = paddle._C_ops.subtract(slice_3, slice_2)

        # pd_op.clip: (2x1x4165x2xf32) <- (2x1x4165x2xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(subtract_2, full_10, full_11)

        # pd_op.prod: (2x1x4165xf32) <- (2x1x4165x2xf32, 1xi64)
        prod_2 = paddle._C_ops.prod(clip_2, full_int_array_0, False, False)

        # pd_op.add: (2x7x4165xf32) <- (2x7x1xf32, 2x1x4165xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)

        # pd_op.subtract: (2x7x4165xf32) <- (2x7x4165xf32, 2x7x4165xf32)
        ...
        # pd_op.scale: (2x7x4165xf32) <- (2x7x4165xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_12, float("1e-09"), True)

        # pd_op.divide: (2x7x4165xf32) <- (2x7x4165xf32, 2x7x4165xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_1)

        # pd_op.multiply: (2x7x4165xf32) <- (2x7x4165xf32, 2x7x4165xf32)
        multiply_1 = paddle._C_ops.multiply(divide_0, data_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_8 = [-2]

        # pd_op.max: (2x4165xf32) <- (2x7x4165xf32, 1xi64)
        max_0 = paddle._C_ops.max(multiply_1, full_int_array_8, False)

        # pd_op.unsqueeze: (2x4165x1xf32) <- (2x4165xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(max_0, full_int_array_0)

        # pd_op.multiply: (2x4165x11xf32) <- (2x4165x11xf32, 2x4165x1xf32)
