...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        full_int_array_3 = [2, 500, 8, 3, 4, 2]
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_7 = [full_9, full_7, slice_5, slice_6]
        ...
        # pd_op.slice: (2x500x8x4x2xf32) <- (2x500x8x3x4x2xf32, 1xi64, 1xi64)
        slice_13 = paddle._C_ops.slice(
            scale_2, [3], full_int_array_6, full_int_array_1, [1], [3]
        )

        # pd_op.transpose: (2x8x500x4x2xf32) <- (2x500x8x4x2xf32)
        transpose_1 = paddle._C_ops.transpose(slice_13, [0, 2, 1, 3, 4])

        # pd_op.flatten: (16x500x4x2xf32) <- (2x8x500x4x2xf32)
        flatten_0 = paddle._C_ops.flatten(transpose_1, 0, 1)

        # pd_op.grid_sample: (16x32x500x4xf32) <- (16x32x-1x-1xf32, 16x500x4x2xf32)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_8 = [full_9, full_7, slice_8, slice_9]
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_9 = [full_9, full_7, slice_11, slice_12]
        ...
        # pd_op.transpose: (2x8x500x3x4xf32) <- (2x500x8x3x4xf32)
        transpose_6 = paddle._C_ops.transpose(reshape_7, [0, 2, 1, 3, 4])

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_10 = [16, 1, 500, 12]

        # pd_op.reshape: (16x1x500x12xf32) <- (2x8x500x3x4xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(transpose_6, full_int_array_10)

        # builtin.combine: ([16x32x500x4xf32, 16x32x500x4xf32, 16x32x500x4xf32]) <- (16x32x500x4xf32, 16x32x500x4xf32, 16x32x500x4xf32)
        combine_10 = [grid_sample_0, grid_sample_1, grid_sample_2]

        # pd_op.stack: (16x32x500x3x4xf32) <- ([16x32x500x4xf32, 16x32x500x4xf32, 16x32x500x4xf32])
        stack_0 = paddle._C_ops.stack(combine_10, -2)

        # pd_op.flatten: (16x32x500x12xf32) <- (16x32x500x3x4xf32)
        flatten_3 = paddle._C_ops.flatten(stack_0, 3, 4)

        # pd_op.multiply: (16x32x500x12xf32) <- (16x32x500x12xf32, 16x1x500x12xf32)
        multiply_0 = paddle._C_ops.multiply(flatten_3, reshape_3)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [-1]

        # pd_op.sum: (16x32x500xf32) <- (16x32x500x12xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(multiply_0, full_int_array_0, None, False)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_11 = [2, 256, 500]

        # pd_op.reshape: (2x256x500xf32) <- (16x32x500xf32, 3xi64)
        reshape_8 = paddle._C_ops.reshape(sum_0, full_int_array_11)

        # pd_op.transpose: (2x500x256xf32) <- (2x256x500xf32)
