...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (2x10285xi64) <- (2x8x10285xf32, 1xi64)
        ...
        # pd_op.gather: (20570xi32) <- (16xi32, 20570xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_5)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [2, 10285]

        # pd_op.reshape: (2x10285xi32) <- (20570xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x10285xb) <- (2x10285xf32, xf32)
        ...
        # pd_op.full_like: (2x10285xi32) <- (2x10285xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_7, paddle.int32, paddle.framework._current_expected_place()
        )

        # pd_op.where: (2x10285xi32) <- (2x10285xb, 2x10285xi32, 2x10285xi32)
        ...
        # pd_op.gather: (20570x4xf32) <- (16x4xf32, 20570xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_5)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [2, 10285, 4]

        # pd_op.reshape: (2x10285x4xf32) <- (20570x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (2x10285x12xf32) <- (2x10285xi32, 1xi32)
        ...
        # pd_op.index_select: (2x10285x11xf32) <- (2x10285x12xf32, 11xi64)
        ...
        # pd_op.maximum: (2x8x10285x2xf32) <- (2x8x1x2xf32, 2x1x10285x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (2x8x10285x2xf32) <- (2x8x1x2xf32, 2x1x10285x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (2x8x10285x2xf32) <- (2x8x10285x2xf32, 2x8x10285x2xf32)
        ...
        # pd_op.clip: (2x8x10285x2xf32) <- (2x8x10285x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_10, full_11)

        # pd_op.prod: (2x8x10285xf32) <- (2x8x10285x2xf32, 1xi64)
        ...
        # pd_op.add: (2x8x10285xf32) <- (2x8x1xf32, 2x1x10285xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)

        # pd_op.subtract: (2x8x10285xf32) <- (2x8x10285xf32, 2x8x10285xf32)
        ...
        # pd_op.scale: (2x8x10285xf32) <- (2x8x10285xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_12, float("1e-09"), True)

        # pd_op.divide: (2x8x10285xf32) <- (2x8x10285xf32, 2x8x10285xf32)
        ...
        # pd_op.max: (2x10285xf32) <- (2x8x10285xf32, 1xi64)
        max_0 = paddle._C_ops.max(multiply_1, full_int_array_8, False)

        # pd_op.unsqueeze: (2x10285x1xf32) <- (2x10285xf32, 1xi64)
