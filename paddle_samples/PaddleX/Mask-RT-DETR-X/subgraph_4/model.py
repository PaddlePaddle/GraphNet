...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0):
        ...
        # pd_op.split_with_num: ([300x1x2xf32, 300x1x2xf32]) <- (300x1x4xf32, 1xi32)
        split_with_num_0 = paddle._C_ops.split_with_num(data_0, 2, full_0)
        del data_0, full_0

        # builtin.split: (300x1x2xf32, 300x1x2xf32) <- ([300x1x2xf32, 300x1x2xf32])
        (
            split_0,
            split_1,
        ) = split_with_num_0
        ...
        # pd_op.scale: (300x1x2xf32) <- (300x1x2xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(split_1, full_1, float("0"), True)
        del full_1, split_1

        # pd_op.subtract: (300x1x2xf32) <- (300x1x2xf32, 300x1x2xf32)
        subtract_0 = paddle._C_ops.subtract(split_0, scale_0)

        # pd_op.add: (300x1x2xf32) <- (300x1x2xf32, 300x1x2xf32)
        ...
        # builtin.combine: ([300x1x2xf32, 300x1x2xf32]) <- (300x1x2xf32, 300x1x2xf32)
        combine_0 = [subtract_0, add_0]
        del add_0, subtract_0

        # pd_op.concat: (300x1x4xf32) <- ([300x1x2xf32, 300x1x2xf32], 1xi32)
