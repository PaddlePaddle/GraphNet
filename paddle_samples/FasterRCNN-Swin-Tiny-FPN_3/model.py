...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.matmul: (-1x49x2304xf32) <- (-1x49x-1xf32, 768x2304xf32)
        ...
        # pd_op.reshape: (-1x49x3x-1x-1xf32) <- (-1x49x2304xf32, 5xi64)
        reshape_2 = paddle._C_ops.reshape(add_0, stack_0)
        ...
        index_select_0 = paddle._C_ops.index_select(data_4, flatten_0, 0)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_4 = [49, 49, -1]

        # pd_op.reshape: (49x49x-1xf32) <- (2401x-1xf32, 3xi64)
        reshape_3 = paddle._C_ops.reshape(index_select_0, full_int_array_4)
        ...
        shape64_1 = paddle._C_ops.shape64(data_2)
        ...
        # builtin.combine: ([xi64, xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64, xi64)
        combine_1 = [full_1, slice_5, data_0, full_2, full_2]

        # pd_op.stack: (5xi64) <- ([xi64, xi64, xi64, xi64, xi64])
        stack_1 = paddle._C_ops.stack(combine_1, 0)

        # pd_op.reshape: (-1x-1x-1x49x49xf32) <- (-1x-1x49x49xf32, 5xi64)
        reshape_0 = paddle._C_ops.reshape(add_1, stack_1)

        # pd_op.unsqueeze: (-1x1x49x49xf32) <- (-1x49x49xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(data_2, full_int_array_1)

        # pd_op.unsqueeze: (1x-1x1x49x49xf32) <- (-1x1x49x49xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(unsqueeze_2, full_int_array_0)

        # pd_op.add: (-1x-1x-1x49x49xf32) <- (-1x-1x-1x49x49xf32, 1x-1x1x49x49xf32)
        add_2 = paddle._C_ops.add(reshape_0, unsqueeze_1)

        # builtin.combine: ([xi64, xi64, xi64, xi64]) <- (xi64, xi64, xi64, xi64)
        combine_2 = [full_1, data_0, full_2, full_2]

        # pd_op.stack: (4xi64) <- ([xi64, xi64, xi64, xi64])
        stack_2 = paddle._C_ops.stack(combine_2, 0)

        # pd_op.reshape: (-1x-1x49x49xf32) <- (-1x-1x-1x49x49xf32, 4xi64)
        ...
        combine_3 = [full_1, full_2, slice_2]

        # pd_op.stack: (3xi64) <- ([xi64, xi64, xi64])
        stack_3 = paddle._C_ops.stack(combine_3, 0)

        # pd_op.reshape: (-1x49x-1xf32) <- (-1x49x-1x-1xf32, 3xi64)
        reshape_1 = paddle._C_ops.reshape(transpose_3, stack_3)

        # pd_op.matmul: (-1x49x768xf32) <- (-1x49x-1xf32, 768x768xf32)
