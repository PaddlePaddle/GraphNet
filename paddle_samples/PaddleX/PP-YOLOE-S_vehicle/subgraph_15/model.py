...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # builtin.combine: ([361x4xf32, 1444x4xf32, 5776x4xf32]) <- (361x4xf32, 1444x4xf32, 5776x4xf32)
        combine_9 = [reshape_0, reshape_2, reshape_4]

        # pd_op.concat: (7581x4xf32) <- ([361x4xf32, 1444x4xf32, 5776x4xf32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_9, full_12)
        del combine_9

        # builtin.combine: ([361x2xf32, 1444x2xf32, 5776x2xf32]) <- (361x2xf32, 1444x2xf32, 5776x2xf32)
        combine_10 = [reshape_1, reshape_3, reshape_5]
        del reshape_1, reshape_3, reshape_5

        # pd_op.concat: (7581x2xf32) <- ([361x2xf32, 1444x2xf32, 5776x2xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_10, full_12)
        del combine_10

        # builtin.combine: ([361x1xf32, 1444x1xf32, 5776x1xf32]) <- (361x1xf32, 1444x1xf32, 5776x1xf32)
        combine_11 = [full_5, full_8, full_11]
        del full_11, full_5, full_8

        # pd_op.concat: (7581x1xf32) <- ([361x1xf32, 1444x1xf32, 5776x1xf32], 1xi32)
        ...
        # pd_op.pool2d: (2x384x1x1xf32) <- (2x384x19x19xf32, 2xi64)
        ...
        # pd_op.add: (2x384x19x19xf32) <- (2x384x19x19xf32, 2x384x19x19xf32)
        add_1 = paddle._C_ops.add(swish_0, data_0)

        # pd_op.conv2d: (2x4x19x19xf32) <- (2x384x19x19xf32, 4x384x3x3xf32)
        ...
        # pd_op.add: (2x4x19x19xf32) <- (2x4x19x19xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (2x68x19x19xf32) <- (2x384x19x19xf32, 68x384x3x3xf32)
        ...
        # pd_op.add: (2x68x19x19xf32) <- (2x68x19x19xf32, 1x68x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_5, reshape_9)

        # pd_op.sigmoid: (2x4x19x19xf32) <- (2x4x19x19xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_2)
        del add_2

        # pd_op.flatten: (2x4x361xf32) <- (2x4x19x19xf32)
        flatten_0 = paddle._C_ops.flatten(sigmoid_2, 2, 3)

        # pd_op.transpose: (2x361x4xf32) <- (2x4x361xf32)
        transpose_0 = paddle._C_ops.transpose(flatten_0, [0, 2, 1])
        del flatten_0

        # pd_op.flatten: (2x68x361xf32) <- (2x68x19x19xf32)
        flatten_1 = paddle._C_ops.flatten(add_4, 2, 3)

        # pd_op.transpose: (2x361x68xf32) <- (2x68x361xf32)
        transpose_1 = paddle._C_ops.transpose(flatten_1, [0, 2, 1])
        del flatten_1

        # pd_op.pool2d: (2x192x1x1xf32) <- (2x192x38x38xf32, 2xi64)
        ...
        # pd_op.add: (2x192x38x38xf32) <- (2x192x38x38xf32, 2x192x38x38xf32)
        add_6 = paddle._C_ops.add(swish_2, data_1)

        # pd_op.conv2d: (2x4x38x38xf32) <- (2x192x38x38xf32, 4x192x3x3xf32)
        ...
        # pd_op.add: (2x4x38x38xf32) <- (2x4x38x38xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (2x68x38x38xf32) <- (2x192x38x38xf32, 68x192x3x3xf32)
        ...
        # pd_op.add: (2x68x38x38xf32) <- (2x68x38x38xf32, 1x68x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_11, reshape_13)

        # pd_op.sigmoid: (2x4x38x38xf32) <- (2x4x38x38xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_7)
        del add_7

        # pd_op.flatten: (2x4x1444xf32) <- (2x4x38x38xf32)
        flatten_2 = paddle._C_ops.flatten(sigmoid_5, 2, 3)

        # pd_op.transpose: (2x1444x4xf32) <- (2x4x1444xf32)
        transpose_2 = paddle._C_ops.transpose(flatten_2, [0, 2, 1])
        del flatten_2

        # pd_op.flatten: (2x68x1444xf32) <- (2x68x38x38xf32)
        flatten_3 = paddle._C_ops.flatten(add_9, 2, 3)

        # pd_op.transpose: (2x1444x68xf32) <- (2x68x1444xf32)
        transpose_3 = paddle._C_ops.transpose(flatten_3, [0, 2, 1])
        del flatten_3

        # pd_op.pool2d: (2x96x1x1xf32) <- (2x96x76x76xf32, 2xi64)
        ...
        # pd_op.add: (2x96x76x76xf32) <- (2x96x76x76xf32, 2x96x76x76xf32)
        add_11 = paddle._C_ops.add(swish_4, data_2)

        # pd_op.conv2d: (2x4x76x76xf32) <- (2x96x76x76xf32, 4x96x3x3xf32)
        ...
        # pd_op.add: (2x4x76x76xf32) <- (2x4x76x76xf32, 1x4x1x1xf32)
        ...
        # pd_op.conv2d: (2x68x76x76xf32) <- (2x96x76x76xf32, 68x96x3x3xf32)
        ...
        # pd_op.add: (2x68x76x76xf32) <- (2x68x76x76xf32, 1x68x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_17, reshape_17)

        # pd_op.sigmoid: (2x4x76x76xf32) <- (2x4x76x76xf32)
        sigmoid_8 = paddle._C_ops.sigmoid(add_12)
        del add_12

        # pd_op.flatten: (2x4x5776xf32) <- (2x4x76x76xf32)
        flatten_4 = paddle._C_ops.flatten(sigmoid_8, 2, 3)

        # pd_op.transpose: (2x5776x4xf32) <- (2x4x5776xf32)
        transpose_4 = paddle._C_ops.transpose(flatten_4, [0, 2, 1])
        del flatten_4

        # pd_op.flatten: (2x68x5776xf32) <- (2x68x76x76xf32)
        flatten_5 = paddle._C_ops.flatten(add_14, 2, 3)

        # pd_op.transpose: (2x5776x68xf32) <- (2x68x5776xf32)
        ...
        # builtin.combine: ([2x361x4xf32, 2x1444x4xf32, 2x5776x4xf32]) <- (2x361x4xf32, 2x1444x4xf32, 2x5776x4xf32)
        combine_12 = [transpose_0, transpose_2, transpose_4]

        # pd_op.concat: (2x7581x4xf32) <- ([2x361x4xf32, 2x1444x4xf32, 2x5776x4xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_12, full_13)
        del combine_12

        # builtin.combine: ([2x361x68xf32, 2x1444x68xf32, 2x5776x68xf32]) <- (2x361x68xf32, 2x1444x68xf32, 2x5776x68xf32)
        combine_13 = [transpose_1, transpose_3, transpose_5]

        # pd_op.concat: (2x7581x68xf32) <- ([2x361x68xf32, 2x1444x68xf32, 2x5776x68xf32], 1xi32)
