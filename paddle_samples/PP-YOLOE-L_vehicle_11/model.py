...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        full_1 = paddle._C_ops.full(
            [1], float("23"), paddle.float64, paddle.core.CPUPlace()
        )
        ...
        # pd_op.arange: (23xi64) <- (1xf64, 1xf64, 1xf64)
        arange_0 = paddle.arange(full_0, full_1, full_2, dtype="int64")

        # pd_op.cast: (23xf32) <- (23xi64)
        ...
        # pd_op.scale: (23xf32) <- (23xf32, 1xf32)
        ...
        # pd_op.scale: (23x23xf32) <- (23x23xf32, 1xf32)
        ...
        # builtin.combine: ([23x23xf32, 23x23xf32, 23x23xf32, 23x23xf32]) <- (23x23xf32, 23x23xf32, 23x23xf32, 23x23xf32)
        combine_1 = [scale_2, scale_3, scale_4, scale_5]

        # pd_op.stack: (23x23x4xf32) <- ([23x23xf32, 23x23xf32, 23x23xf32, 23x23xf32])
        stack_0 = paddle._C_ops.stack(combine_1, -1)

        # builtin.combine: ([23x23xf32, 23x23xf32]) <- (23x23xf32, 23x23xf32)
        combine_2 = [split_1, split_0]

        # pd_op.stack: (23x23x2xf32) <- ([23x23xf32, 23x23xf32])
        stack_1 = paddle._C_ops.stack(combine_2, -1)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_0 = [-1, 4]

        # pd_op.reshape: (529x4xf32) <- (23x23x4xf32, 2xi64)
        reshape_0 = paddle._C_ops.reshape(stack_0, full_int_array_0)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [-1, 2]

        # pd_op.reshape: (529x2xf32) <- (23x23x2xf32, 2xi64)
        reshape_3 = paddle._C_ops.reshape(stack_1, full_int_array_1)

        # pd_op.full: (529x1xf32) <- ()
        ...
        full_6 = paddle._C_ops.full(
            [1], float("46"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (46xi64) <- (1xf64, 1xf64, 1xf64)
        arange_1 = paddle.arange(full_0, full_6, full_2, dtype="int64")

        # pd_op.cast: (46xf32) <- (46xi64)
        cast_1 = paddle._C_ops.cast(arange_1, paddle.float32)

        # pd_op.scale: (46xf32) <- (46xf32, 1xf32)
        ...
        # builtin.combine: ([46xf32, 46xf32]) <- (46xf32, 46xf32)
        combine_3 = [scale_7, scale_7]

        # pd_op.meshgrid: ([46x46xf32, 46x46xf32]) <- ([46xf32, 46xf32])
        meshgrid_1 = paddle._C_ops.meshgrid(combine_3)

        # builtin.split: (46x46xf32, 46x46xf32) <- ([46x46xf32, 46x46xf32])
        (
            split_2,
            split_3,
        ) = meshgrid_1

        # pd_op.scale: (46x46xf32) <- (46x46xf32, 1xf32)
        ...
        # builtin.combine: ([46x46xf32, 46x46xf32, 46x46xf32, 46x46xf32]) <- (46x46xf32, 46x46xf32, 46x46xf32, 46x46xf32)
        combine_4 = [scale_8, scale_9, scale_10, scale_11]

        # pd_op.stack: (46x46x4xf32) <- ([46x46xf32, 46x46xf32, 46x46xf32, 46x46xf32])
        stack_2 = paddle._C_ops.stack(combine_4, -1)

        # builtin.combine: ([46x46xf32, 46x46xf32]) <- (46x46xf32, 46x46xf32)
        combine_5 = [split_3, split_2]

        # pd_op.stack: (46x46x2xf32) <- ([46x46xf32, 46x46xf32])
        stack_3 = paddle._C_ops.stack(combine_5, -1)

        # pd_op.reshape: (2116x4xf32) <- (46x46x4xf32, 2xi64)
        reshape_1 = paddle._C_ops.reshape(stack_2, full_int_array_0)

        # pd_op.reshape: (2116x2xf32) <- (46x46x2xf32, 2xi64)
        reshape_4 = paddle._C_ops.reshape(stack_3, full_int_array_1)

        # pd_op.full: (2116x1xf32) <- ()
        ...
        full_9 = paddle._C_ops.full(
            [1], float("92"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (92xi64) <- (1xf64, 1xf64, 1xf64)
        arange_2 = paddle.arange(full_0, full_9, full_2, dtype="int64")

        # pd_op.cast: (92xf32) <- (92xi64)
        cast_2 = paddle._C_ops.cast(arange_2, paddle.float32)

        # pd_op.scale: (92xf32) <- (92xf32, 1xf32)
        ...
        # builtin.combine: ([92xf32, 92xf32]) <- (92xf32, 92xf32)
        combine_6 = [scale_13, scale_13]

        # pd_op.meshgrid: ([92x92xf32, 92x92xf32]) <- ([92xf32, 92xf32])
        meshgrid_2 = paddle._C_ops.meshgrid(combine_6)

        # builtin.split: (92x92xf32, 92x92xf32) <- ([92x92xf32, 92x92xf32])
        (
            split_4,
            split_5,
        ) = meshgrid_2

        # pd_op.scale: (92x92xf32) <- (92x92xf32, 1xf32)
        ...
        # builtin.combine: ([92x92xf32, 92x92xf32, 92x92xf32, 92x92xf32]) <- (92x92xf32, 92x92xf32, 92x92xf32, 92x92xf32)
        combine_7 = [scale_14, scale_15, scale_16, scale_17]

        # pd_op.stack: (92x92x4xf32) <- ([92x92xf32, 92x92xf32, 92x92xf32, 92x92xf32])
        stack_4 = paddle._C_ops.stack(combine_7, -1)

        # builtin.combine: ([92x92xf32, 92x92xf32]) <- (92x92xf32, 92x92xf32)
        combine_8 = [split_5, split_4]

        # pd_op.stack: (92x92x2xf32) <- ([92x92xf32, 92x92xf32])
        stack_5 = paddle._C_ops.stack(combine_8, -1)

        # pd_op.reshape: (8464x4xf32) <- (92x92x4xf32, 2xi64)
        reshape_2 = paddle._C_ops.reshape(stack_4, full_int_array_0)

        # pd_op.reshape: (8464x2xf32) <- (92x92x2xf32, 2xi64)
        reshape_5 = paddle._C_ops.reshape(stack_5, full_int_array_1)

        # pd_op.full: (8464x1xf32) <- ()
        ...
        # builtin.combine: ([529x4xf32, 2116x4xf32, 8464x4xf32]) <- (529x4xf32, 2116x4xf32, 8464x4xf32)
        combine_9 = [reshape_0, reshape_1, reshape_2]

        # pd_op.concat: (11109x4xf32) <- ([529x4xf32, 2116x4xf32, 8464x4xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_9, full_12)

        # builtin.combine: ([529x2xf32, 2116x2xf32, 8464x2xf32]) <- (529x2xf32, 2116x2xf32, 8464x2xf32)
        combine_10 = [reshape_3, reshape_4, reshape_5]

        # pd_op.concat: (11109x2xf32) <- ([529x2xf32, 2116x2xf32, 8464x2xf32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_10, full_12)

        # builtin.combine: ([529x1xf32, 2116x1xf32, 8464x1xf32]) <- (529x1xf32, 2116x1xf32, 8464x1xf32)
        combine_11 = [full_5, full_8, full_11]

        # pd_op.concat: (11109x1xf32) <- ([529x1xf32, 2116x1xf32, 8464x1xf32], 1xi32)
