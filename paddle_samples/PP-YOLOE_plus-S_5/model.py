...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.unsqueeze: (24x1x4xf32) <- (24x4xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(data_0, full_int_array_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_1 = [0]

        # pd_op.unsqueeze: (1x4725x4xf32) <- (4725x4xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(data_1, full_int_array_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [2]

        # pd_op.slice: (24x1x2xf32) <- (24x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.slice: (1x4725x2xf32) <- (1x4725x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (24x4725x2xf32) <- (24x1x2xf32, 1x4725x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (24x4725x2xf32) <- (24x1x2xf32, 1x4725x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (24x4725x2xf32) <- (24x4725x2xf32, 24x4725x2xf32)
        ...
        # pd_op.scale: (24x4725xf32) <- (24x4725xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_3, full_2, float("1e-10"), True)

        # pd_op.divide: (24x4725xf32) <- (24x4725xf32, 24x4725xf32)
