...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.full_like: (8x-1x256xf32) <- (8x-1x256xf32, 1xf32)
        ...
        # pd_op.add: (8x-1x256xf32) <- (8x-1x256xf32, xf32)
        add_12 = paddle._C_ops.add(full_like_6, full_like_7)

        # pd_op.add: (8x-1x256xf32) <- (8x-1x256xf32, -1x-1x1xf32)
        ...
        # pd_op.add: (8x-1x256xf32) <- (xf32, 8x-1x256xf32)
        add_2 = paddle._C_ops.add(assign_value__0, add_0)

        # pd_op.add: (8x-1x256xf32) <- (-1x-1x1xf32, 8x-1x256xf32)
        add_13 = paddle._C_ops.add(cast_11, add_0)

        # pd_op.cast: (8x-1x256xb) <- (8x-1x256xf32)
        cast_0 = paddle._C_ops.cast(add_13, paddle.bool)

        # pd_op.where: (8x-1x256xf32) <- (8x-1x256xb, 8x-1x256xf32, 8x-1x256xf32)
        ...
        # pd_op.matmul: (8x-1x4xf32) <- (8x-1x256xf32, 256x4xf32)
        matmul_1 = paddle._C_ops.matmul(layer_norm_0, parameter_7, False, False)

        # pd_op.add: (8x-1x4xf32) <- (8x-1x4xf32, 4xf32)
        ...
        # pd_op.relu: (8x-1x256xf32) <- (8x-1x256xf32)
        ...
        # pd_op.add: (8x-1x4xf32) <- (8x-1x4xf32, -1x-1x4xf32)
        add_6 = paddle._C_ops.add(add_5, where_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_3 = [-1]

        # pd_op.max: (8x-1xf32) <- (8x-1x4xf32, 1xi64)
        ...
        # pd_op.topk: (8x300xf32, 8x300xi64) <- (8x-1xf32, 1xi32)
        ...
        # pd_op.gather_nd: (8x300x4xf32) <- (8x-1x4xf32, 8x300x2xi64)
        ...
        # builtin.combine: ([8x-1x4xf32, 8x300x4xf32]) <- (8x-1x4xf32, 8x300x4xf32)
        combine_17 = [data_8, gather_nd_1]

        # pd_op.concat: (8x-1x4xf32) <- ([8x-1x4xf32, 8x300x4xf32], 1xi32)
        concat_5 = paddle._C_ops.concat(combine_17, full_8)

        # pd_op.share_data_: (8x-1x4xf32) <- (8x-1x4xf32)
        share_data__1 = concat_5.detach()

        # pd_op.gather_nd: (8x300x4xf32) <- (8x-1x4xf32, 8x300x2xi64)
        gather_nd_0 = paddle._C_ops.gather_nd(add_4, stack_0)

        # pd_op.gather_nd: (8x300x256xf32) <- (8x-1x256xf32, 8x300x2xi64)
        ...
        # builtin.combine: ([8x-1x256xf32, 8x300x256xf32]) <- (8x-1x256xf32, 8x300x256xf32)
        combine_18 = [data_7, share_data__0]

        # pd_op.concat: (8x-1x256xf32) <- ([8x-1x256xf32, 8x300x256xf32], 1xi32)
