...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.slice: (1x3x256x192xf32) <- (4x3x256x192xf32, 1xi64, 1xi64)
        slice_0 = paddle._C_ops.slice(
            data_0, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x256x192xf32) <- (4x12x256x192xf32, 1xi64, 1xi64)
        ...
        # pd_op.full_like: (147456x4xf32) <- (147456x4xf32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            data_10, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x256x192xf32, 1x12x256x192xf32, 1x2xf32, 147456x4xf32, 147456x4xf32)
        ...
        # pd_op.slice: (1x3x128x96xf32) <- (4x3x128x96xf32, 1xi64, 1xi64)
        slice_3 = paddle._C_ops.slice(
            data_1, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x128x96xf32) <- (4x12x128x96xf32, 1xi64, 1xi64)
        slice_4 = paddle._C_ops.slice(
            data_6, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (36864x4xf32) <- (36864x4xf32, 1xf32)
        full_like_1 = paddle._C_ops.full_like(
            data_11, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x128x96xf32, 1x12x128x96xf32, 1x2xf32, 36864x4xf32, 36864x4xf32)
        ...
        # pd_op.slice: (1x3x64x48xf32) <- (4x3x64x48xf32, 1xi64, 1xi64)
        slice_5 = paddle._C_ops.slice(
            data_2, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x64x48xf32) <- (4x12x64x48xf32, 1xi64, 1xi64)
        slice_6 = paddle._C_ops.slice(
            data_7, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (9216x4xf32) <- (9216x4xf32, 1xf32)
        full_like_2 = paddle._C_ops.full_like(
            data_12, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x64x48xf32, 1x12x64x48xf32, 1x2xf32, 9216x4xf32, 9216x4xf32)
        ...
        # pd_op.slice: (1x3x32x24xf32) <- (4x3x32x24xf32, 1xi64, 1xi64)
        slice_7 = paddle._C_ops.slice(
            data_3, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x32x24xf32) <- (4x12x32x24xf32, 1xi64, 1xi64)
        slice_8 = paddle._C_ops.slice(
            data_8, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (2304x4xf32) <- (2304x4xf32, 1xf32)
        full_like_3 = paddle._C_ops.full_like(
            data_13, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x32x24xf32, 1x12x32x24xf32, 1x2xf32, 2304x4xf32, 2304x4xf32)
        ...
        # pd_op.slice: (1x3x16x12xf32) <- (4x3x16x12xf32, 1xi64, 1xi64)
        slice_9 = paddle._C_ops.slice(
            data_4, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.slice: (1x12x16x12xf32) <- (4x12x16x12xf32, 1xi64, 1xi64)
        slice_10 = paddle._C_ops.slice(
            data_9, [0], full_int_array_0, full_int_array_1, [1], []
        )

        # pd_op.full_like: (576x4xf32) <- (576x4xf32, 1xf32)
        full_like_4 = paddle._C_ops.full_like(
            data_14, full_0, paddle.float32, paddle.framework._current_expected_place()
        )

        # pd_op.generate_proposals: (-1x4xf32, -1x1xf32, 1xf32) <- (1x3x16x12xf32, 1x12x16x12xf32, 1x2xf32, 576x4xf32, 576x4xf32)
