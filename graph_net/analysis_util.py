...
def parse_logs_to_data(log_file: str) -> list:
    ...
    This function handles file I/O and delegates single sample parsing
    to parse_single_sample_log_to_data().
    ...
    # Store processing lines to correctly set model_path
    processing_lines = []
    ...
    for i, line in enumerate(lines):
        ...
        if "[Processing]" in line:
            ...
            if current_run_key and current_run_lines:
                all_runs_data[current_run_key] = parse_single_sample_log_to_data(
                    current_run_lines
                )

            # Start new run
            current_run_key = line.split("[Processing]")[1].strip()
            current_run_lines = []
            continue

        # Collect lines for current run
        if current_run_key is not None:
            current_run_lines.append(line)

    # Process the last run if exists
    if current_run_key and current_run_lines:
        all_runs_data[current_run_key] = parse_single_sample_log_to_data(
            current_run_lines
        )

    # Set model_path for all runs
    for run_key, data in all_runs_data.items():
        # Find matching processing line (should always exist)
        for processing_line in processing_lines:
            if run_key in processing_line:
                data["model_path"] = processing_line.split()[-1]
                break
    ...
    samples = list(all_runs_data.values())
