...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        # pd_op.sigmoid: (182400x11xf32) <- (182400x11xf32)
        sigmoid_0 = paddle._C_ops.sigmoid(data_0)

        # pd_op.cast: (182400x11xf32) <- (182400x11xf64)
        ...
        # pd_op.greater_than: (182400x11xb) <- (182400x11xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(cast_0, full_3)

        # pd_op.cast: (182400x11xf32) <- (182400x11xb)
        cast_2 = paddle._C_ops.cast(greater_than_0, paddle.float32)

        # pd_op.multiply: (182400x11xf32) <- (182400x11xf32, 182400x11xf32)
        multiply_0 = paddle._C_ops.multiply(cast_0, cast_2)

        # pd_op.subtract: (182400x11xf32) <- (182400x11xf32, 182400x11xf32)
        subtract_0 = paddle._C_ops.subtract(sigmoid_0, cast_0)

        # pd_op.abs: (182400x11xf32) <- (182400x11xf32)
        abs_0 = paddle._C_ops.abs(subtract_0)

        # pd_op.pow: (182400x11xf32) <- (182400x11xf32)
        ...
        # pd_op.scale: (182400x11xf32) <- (182400x11xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(pow_0, full_0, float("0"), True)

        # pd_op.less_equal: (182400x11xb) <- (182400x11xf32, xf32)
        less_equal_0 = paddle._C_ops.less_equal(cast_0, full_3)
        ...
        # pd_op.add: (182400x11xf32) <- (182400x11xf32, 182400x11xf32)
        ...
        # pd_op.sigmoid_cross_entropy_with_logits: (182400x11xf32) <- (182400x11xf32, 182400x11xf32, None)
        ...
        # pd_op.scale: (182400x11xf32) <- (182400x11xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(multiply_2, full_1, float("0"), True)

        # pd_op.full_int_array: (0xi64) <- ()
        full_int_array_0 = []

        # pd_op.sum: (xf32) <- (182400x11xf32, 0xi64)
        sum_0 = paddle._C_ops.sum(scale_1, full_int_array_0, None, False)

        # pd_op.full: (1xf32) <- ()
        full_2 = paddle._C_ops.full(
            [1], float("0.00236967"), paddle.float32, paddle.core.CPUPlace()
        )

        # pd_op.scale: (xf32) <- (xf32, 1xf32)
        scale_2 = paddle._C_ops.scale(sum_0, full_2, float("0"), True)
        return (
            sigmoid_0,
            cast_0,
            multiply_0,
