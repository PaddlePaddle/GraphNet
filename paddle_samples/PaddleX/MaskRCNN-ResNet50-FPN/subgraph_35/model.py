...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.slice: (220968xf32) <- (220968x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.subtract: (220968xf32) <- (220968xf32, 220968xf32)
        ...
        # pd_op.multiply: (220968xf32) <- (220968xf32, 220968xf32)
        ...
        # pd_op.slice: (220968x2xf32) <- (220968x4xf32, 1xi64, 1xi64)
        slice_9 = paddle._C_ops.slice(
            data_1, [1], full_int_array_0, full_int_array_5, [1], []
        )
        del full_int_array_5

        # pd_op.minimum: (1x220968x2xf32) <- (1x1x2xf32, 220968x2xf32)
        ...
        # pd_op.maximum: (1x220968x2xf32) <- (1x1x2xf32, 220968x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_10, slice_11)
        del slice_10, slice_11

        # pd_op.subtract: (1x220968x2xf32) <- (1x220968x2xf32, 1x220968x2xf32)
        ...
        # pd_op.clip: (1x220968x2xf32) <- (1x220968x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_4, full_0, full_1)
        del full_1, subtract_4

        # pd_op.prod: (1x220968xf32) <- (1x220968x2xf32, 1xi64)
        ...
        # pd_op.greater_than: (1x220968xb) <- (1x220968xf32, xf32)
        ...
        # pd_op.add: (1x220968xf32) <- (1x1xf32, 220968xf32)
        add_0 = paddle._C_ops.add(unsqueeze_1, multiply_1)
        del multiply_1, unsqueeze_1

        # pd_op.subtract: (1x220968xf32) <- (1x220968xf32, 1x220968xf32)
        subtract_5 = paddle._C_ops.subtract(add_0, prod_0)
        del add_0

        # pd_op.divide: (1x220968xf32) <- (1x220968xf32, 1x220968xf32)
        divide_0 = paddle._C_ops.divide(prod_0, subtract_5)
        del subtract_5

        # pd_op.full_like: (1x220968xf32) <- (1x220968xf32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            prod_0, full_0, paddle.float32, paddle.framework._current_expected_place()
        )
        del full_0, prod_0

        # pd_op.where: (1x220968xf32) <- (1x220968xb, 1x220968xf32, 1x220968xf32)
