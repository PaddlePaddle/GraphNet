...
def compute_duplicates(hash_to_models: Dict[str, List[Path]]) -> Dict[str, List[Path]]:
    ...
    return {hash_value: dirs for hash_value, dirs in hash_to_models.items() if len(dirs) > 1}
...
def main() -> None:
    ...
    move_duplicates(duplicates, args.samples_dir, args.redundant_dir, dry_run=args.dry_run)
