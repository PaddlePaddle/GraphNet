    ...
    def forward(
        ...
        (
            ...
            paddle._C_ops.distribute_fpn_proposals(
                concat_1, data_4, 2, 5, 4, 224, False
            ),
        ...
        # pd_op.roi_align: (-1x256x7x7xf32) <- (4x256x272x-1xf32, -1x4xf32, -1xi32)
        roi_align_0 = paddle._C_ops.roi_align(
            data_5, split_0, split_4, 7, 7, float("0.25"), 0, True
        )

        # pd_op.roi_align: (-1x256x7x7xf32) <- (4x256x136x-1xf32, -1x4xf32, -1xi32)
        roi_align_1 = paddle._C_ops.roi_align(
            data_6, split_1, split_5, 7, 7, float("0.125"), 0, True
        )

        # pd_op.roi_align: (-1x256x7x7xf32) <- (4x256x68x-1xf32, -1x4xf32, -1xi32)
        roi_align_2 = paddle._C_ops.roi_align(
            data_7, split_2, split_6, 7, 7, float("0.0625"), 0, True
        )

        # pd_op.roi_align: (-1x256x7x7xf32) <- (4x256x34x-1xf32, -1x4xf32, -1xi32)
        roi_align_3 = paddle._C_ops.roi_align(
            data_8, split_3, split_7, 7, 7, float("0.03125"), 0, True
        )
        ...
        concat_0 = paddle._C_ops.concat(combine_1, full_4)

        # pd_op.gather: (-1x256x7x7xf32) <- (-1x256x7x7xf32, -1x1xi32, 1xi32)
        gather_0 = paddle._C_ops.gather(concat_0, distribute_fpn_proposals_0, full_4)
        ...
        add_6 = paddle._C_ops.add(matmul_0, parameter_6)

        # pd_op.relu: (-1x1024xf32) <- (-1x1024xf32)
        relu_0 = paddle._C_ops.relu(add_6)

        # pd_op.matmul: (-1x1024xf32) <- (-1x1024xf32, 1024x1024xf32)
        matmul_1 = paddle._C_ops.matmul(relu_0, parameter_5, False, False)

        # pd_op.add: (-1x1024xf32) <- (-1x1024xf32, 1024xf32)
        add_7 = paddle._C_ops.add(matmul_1, parameter_4)

        # pd_op.relu: (-1x1024xf32) <- (-1x1024xf32)
        relu_1 = paddle._C_ops.relu(add_7)
        ...
        add_4 = paddle._C_ops.add(matmul_2, parameter_2)
        ...
        add_5 = paddle._C_ops.add(matmul_3, parameter_0)
        ...
        slice_2 = paddle._C_ops.slice(
            concat_1, [1], full_int_array_2, full_int_array_5, [1], [1]
        )

        # pd_op.slice: (2048xf32) <- (2048x4xf32, 1xi64, 1xi64)
        slice_3 = paddle._C_ops.slice(
            concat_1, [1], full_int_array_4, full_int_array_1, [1], [1]
        )
        ...
        add_8 = paddle._C_ops.add(slice_1, scale_6)

        # pd_op.scale: (2048xf32) <- (2048xf32, 1xf32)
        scale_7 = paddle._C_ops.scale(subtract_3, full_5, float("0"), True)

        # pd_op.add: (2048xf32) <- (2048xf32, 2048xf32)
        add_9 = paddle._C_ops.add(slice_3, scale_7)
        ...
        strided_slice_0 = paddle._C_ops.strided_slice(
            add_5, [1], full_int_array_3, full_int_array_0, full_int_array_5
        )
        ...
        strided_slice_1 = paddle._C_ops.strided_slice(
            add_5, [1], full_int_array_4, full_int_array_0, full_int_array_5
        )
        ...
        strided_slice_2 = paddle._C_ops.strided_slice(
            add_5, [1], full_int_array_1, full_int_array_0, full_int_array_5
        )
        ...
        strided_slice_3 = paddle._C_ops.strided_slice(
            add_5, [1], full_int_array_2, full_int_array_0, full_int_array_5
        )
        ...
        # pd_op.assign: (2048x1xf32) <- (2048x1xf32)
        assign_18 = unsqueeze_2

        # pd_op.multiply: (2048x1xf32) <- (-1x1xf32, 2048x1xf32)
        multiply_1 = paddle._C_ops.multiply(scale_1, unsqueeze_2)

        # pd_op.unsqueeze: (2048x1xf32) <- (2048xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(add_9, full_int_array_4)
        ...
        combine_2 = [subtract_0, subtract_1, add_2, add_3]
        ...
        split_12 = paddle._C_ops.split(reshape_0, full_int_array_7, full_4)
