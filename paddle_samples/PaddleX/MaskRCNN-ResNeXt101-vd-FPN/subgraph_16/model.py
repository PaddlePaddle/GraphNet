...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # builtin.combine: ([139392x4xf32, 34848x4xf32, 8712x4xf32, 2178x4xf32, 561x4xf32]) <- (139392x4xf32, 34848x4xf32, 8712x4xf32, 2178x4xf32, 561x4xf32)
        combine_0 = [reshape_0, reshape_1, reshape_2, reshape_3, reshape_4]
        del reshape_0, reshape_1, reshape_2, reshape_3, reshape_4

        # pd_op.concat: (185691x4xf32) <- ([139392x4xf32, 34848x4xf32, 8712x4xf32, 2178x4xf32, 561x4xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_0, full_0)
        del combine_0, full_0

        # pd_op.transpose: (1x176x264x3xf32) <- (1x3x176x264xf32)
        ...
        # builtin.combine: ([1x139392x1xf32, 1x34848x1xf32, 1x8712x1xf32, 1x2178x1xf32, 1x561x1xf32]) <- (1x139392x1xf32, 1x34848x1xf32, 1x8712x1xf32, 1x2178x1xf32, 1x561x1xf32)
        combine_1 = [reshape_5, reshape_6, reshape_7, reshape_8, reshape_9]

        # pd_op.concat: (1x185691x1xf32) <- ([1x139392x1xf32, 1x34848x1xf32, 1x8712x1xf32, 1x2178x1xf32, 1x561x1xf32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_1, full_1)
        del combine_1

        # pd_op.transpose: (1x176x264x12xf32) <- (1x12x176x264xf32)
        ...
        # pd_op.reshape: (1x139392x4xf32) <- (1x176x264x12xf32, 3xi64)
        reshape_10 = paddle._C_ops.reshape(transpose_5, full_int_array_2)

        # pd_op.transpose: (1x88x132x12xf32) <- (1x12x88x132xf32)
        transpose_6 = paddle._C_ops.transpose(data_6, [0, 2, 3, 1])
        del data_6

        # pd_op.reshape: (1x34848x4xf32) <- (1x88x132x12xf32, 3xi64)
        reshape_11 = paddle._C_ops.reshape(transpose_6, full_int_array_2)

        # pd_op.transpose: (1x44x66x12xf32) <- (1x12x44x66xf32)
        transpose_7 = paddle._C_ops.transpose(data_7, [0, 2, 3, 1])
        del data_7

        # pd_op.reshape: (1x8712x4xf32) <- (1x44x66x12xf32, 3xi64)
        reshape_12 = paddle._C_ops.reshape(transpose_7, full_int_array_2)

        # pd_op.transpose: (1x22x33x12xf32) <- (1x12x22x33xf32)
        transpose_8 = paddle._C_ops.transpose(data_8, [0, 2, 3, 1])
        del data_8

        # pd_op.reshape: (1x2178x4xf32) <- (1x22x33x12xf32, 3xi64)
        reshape_13 = paddle._C_ops.reshape(transpose_8, full_int_array_2)

        # pd_op.transpose: (1x11x17x12xf32) <- (1x12x11x17xf32)
        transpose_9 = paddle._C_ops.transpose(data_9, [0, 2, 3, 1])
        del data_9

        # pd_op.reshape: (1x561x4xf32) <- (1x11x17x12xf32, 3xi64)
        reshape_14 = paddle._C_ops.reshape(transpose_9, full_int_array_2)
        del full_int_array_2

        # builtin.combine: ([1x139392x4xf32, 1x34848x4xf32, 1x8712x4xf32, 1x2178x4xf32, 1x561x4xf32]) <- (1x139392x4xf32, 1x34848x4xf32, 1x8712x4xf32, 1x2178x4xf32, 1x561x4xf32)
        combine_2 = [reshape_10, reshape_11, reshape_12, reshape_13, reshape_14]

        # pd_op.concat: (1x185691x4xf32) <- ([1x139392x4xf32, 1x34848x4xf32, 1x8712x4xf32, 1x2178x4xf32, 1x561x4xf32], 1xi32)
