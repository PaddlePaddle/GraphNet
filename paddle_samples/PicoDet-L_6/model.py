...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.unsqueeze: (8x3x1x4xf32) <- (8x3x4xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(data_0, full_int_array_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_1 = [1]

        # pd_op.unsqueeze: (8x1x10285x4xf32) <- (8x10285x4xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(data_1, full_int_array_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_2 = [0]

        # pd_op.slice: (8x3x1x2xf32) <- (8x3x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.slice: (8x1x10285x2xf32) <- (8x1x10285x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (8x3x10285x2xf32) <- (8x3x1x2xf32, 8x1x10285x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (8x3x10285x2xf32) <- (8x3x1x2xf32, 8x1x10285x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (8x3x10285x2xf32) <- (8x3x10285x2xf32, 8x3x10285x2xf32)
        ...
        # pd_op.clip: (8x3x10285x2xf32) <- (8x3x10285x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_0, full_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_4 = [-1]

        # pd_op.prod: (8x3x10285xf32) <- (8x3x10285x2xf32, 1xi64)
        prod_0 = paddle._C_ops.prod(clip_0, full_int_array_4, False, False)

        # pd_op.subtract: (8x3x1x2xf32) <- (8x3x1x2xf32, 8x3x1x2xf32)
        subtract_1 = paddle._C_ops.subtract(slice_1, slice_0)

        # pd_op.clip: (8x3x1x2xf32) <- (8x3x1x2xf32, 1xf32, 1xf32)
        clip_1 = paddle._C_ops.clip(subtract_1, full_0, full_1)

        # pd_op.prod: (8x3x1xf32) <- (8x3x1x2xf32, 1xi64)
        prod_1 = paddle._C_ops.prod(clip_1, full_int_array_4, False, False)

        # pd_op.subtract: (8x1x10285x2xf32) <- (8x1x10285x2xf32, 8x1x10285x2xf32)
        subtract_2 = paddle._C_ops.subtract(slice_3, slice_2)

        # pd_op.clip: (8x1x10285x2xf32) <- (8x1x10285x2xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(subtract_2, full_0, full_1)

        # pd_op.prod: (8x1x10285xf32) <- (8x1x10285x2xf32, 1xi64)
        prod_2 = paddle._C_ops.prod(clip_2, full_int_array_4, False, False)

        # pd_op.add: (8x3x10285xf32) <- (8x3x1xf32, 8x1x10285xf32)
        add_0 = paddle._C_ops.add(prod_1, prod_2)

        # pd_op.subtract: (8x3x10285xf32) <- (8x3x10285xf32, 8x3x10285xf32)
        ...
        # pd_op.scale: (8x3x10285xf32) <- (8x3x10285xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_3, full_2, float("1e-09"), True)

        # pd_op.divide: (8x3x10285xf32) <- (8x3x10285xf32, 8x3x10285xf32)
