...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, parameter_0, data_0, data_1, data_2, data_3):
        ...
        # pd_op.shape64: (3xi64) <- (2x6804x40xf32)
        ...
        # pd_op.reshape: (-1x-1x4x10xf32) <- (2x6804x40xf32, 4xi64)
        ...
        # pd_op.add: (-1x6804x2xf32) <- (-1x-1x2xf32, 6804x2xf32)
        ...
        # builtin.combine: ([-1x6804x2xf32, -1x6804x2xf32]) <- (-1x6804x2xf32, -1x6804x2xf32)
        combine_1 = [add_0, add_1]

        # pd_op.concat: (-1x6804x4xf32) <- ([-1x6804x2xf32, -1x6804x2xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_1, full_2)

        # pd_op.share_data_: (2x6804x10xf32) <- (2x6804x10xf32)
        share_data__0 = data_0.detach()

        # pd_op.share_data_: (-1x6804x4xf32) <- (-1x6804x4xf32)
        share_data__1 = concat_0.detach()

        # pd_op.multiply: (-1x6804x4xf32) <- (-1x6804x4xf32, 6804x1xf32)
