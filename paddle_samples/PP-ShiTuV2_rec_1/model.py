import paddle


class GraphModule(paddle.nn.Layer):
    def __init__(self):
        super().__init__()

    def forward(
        self,
        parameter_0,
        parameter_1,
        parameter_2,
        parameter_3,
        parameter_4,
        parameter_5,
        parameter_6,
        parameter_7,
        parameter_8,
        parameter_9,
        parameter_10,
        parameter_11,
        parameter_12,
        parameter_13,
        parameter_14,
        parameter_15,
        parameter_16,
        parameter_17,
        parameter_18,
        parameter_19,
        parameter_20,
        parameter_21,
        parameter_22,
        parameter_23,
        parameter_24,
        parameter_25,
        parameter_26,
        parameter_27,
        parameter_28,
        parameter_29,
        parameter_30,
        parameter_31,
        parameter_32,
        parameter_33,
        parameter_34,
        parameter_35,
        parameter_36,
        parameter_37,
        parameter_38,
        parameter_39,
        parameter_40,
        parameter_41,
        parameter_42,
        parameter_43,
        parameter_44,
        parameter_45,
        parameter_46,
        parameter_47,
        parameter_48,
        parameter_49,
        parameter_50,
        parameter_51,
        parameter_52,
        parameter_53,
        parameter_54,
        parameter_55,
        parameter_56,
        parameter_57,
        parameter_58,
        parameter_59,
        parameter_60,
        parameter_61,
        parameter_62,
        parameter_63,
        parameter_64,
        parameter_65,
        parameter_66,
        parameter_67,
        parameter_68,
        parameter_69,
        parameter_70,
        parameter_71,
        parameter_72,
        parameter_73,
        parameter_74,
        parameter_75,
        parameter_76,
        parameter_77,
        parameter_78,
        parameter_79,
        parameter_80,
        parameter_81,
        parameter_82,
        parameter_83,
        parameter_84,
        parameter_85,
        parameter_86,
        parameter_87,
        parameter_88,
        parameter_89,
        parameter_90,
        parameter_91,
        parameter_92,
        parameter_93,
        parameter_94,
        parameter_95,
        parameter_96,
        parameter_97,
        parameter_98,
        parameter_99,
        parameter_100,
        parameter_101,
        parameter_102,
        parameter_103,
        parameter_104,
        parameter_105,
        parameter_106,
        parameter_107,
        parameter_108,
        parameter_109,
        parameter_110,
        parameter_111,
        parameter_112,
        parameter_113,
        parameter_114,
        parameter_115,
        parameter_116,
        parameter_117,
        parameter_118,
        parameter_119,
        parameter_120,
        parameter_121,
        parameter_122,
        parameter_123,
        parameter_124,
        parameter_125,
        parameter_126,
        parameter_127,
        parameter_128,
        parameter_129,
        parameter_130,
        parameter_131,
        parameter_132,
        parameter_133,
        parameter_134,
        parameter_135,
        parameter_136,
        parameter_137,
        parameter_138,
        parameter_139,
        parameter_140,
        parameter_141,
        parameter_142,
        parameter_143,
        parameter_144,
        parameter_145,
        parameter_146,
        parameter_147,
        parameter_148,
        parameter_149,
        parameter_150,
        parameter_151,
        parameter_152,
        parameter_153,
        parameter_154,
        parameter_155,
        parameter_156,
        parameter_157,
        parameter_158,
        parameter_159,
        parameter_160,
        parameter_161,
        parameter_162,
        parameter_163,
        parameter_164,
        parameter_165,
        parameter_166,
        parameter_167,
        parameter_168,
        parameter_169,
        parameter_170,
        parameter_171,
        parameter_172,
        parameter_173,
        parameter_174,
        parameter_175,
        parameter_176,
        parameter_177,
        parameter_178,
        parameter_179,
        parameter_180,
        parameter_181,
        parameter_182,
        parameter_183,
        parameter_184,
        parameter_185,
        parameter_186,
        parameter_187,
        parameter_188,
        parameter_189,
        parameter_190,
        parameter_191,
        parameter_192,
        parameter_193,
        parameter_194,
        parameter_195,
        parameter_196,
        parameter_197,
        parameter_198,
        parameter_199,
        parameter_200,
        parameter_201,
        parameter_202,
        parameter_203,
        parameter_204,
        parameter_205,
        parameter_206,
        parameter_207,
        parameter_208,
        parameter_209,
        parameter_210,
        parameter_211,
        parameter_212,
        parameter_213,
        parameter_214,
        parameter_215,
        parameter_216,
        parameter_217,
        parameter_218,
        parameter_219,
        parameter_220,
        parameter_221,
        parameter_222,
        parameter_223,
        parameter_224,
        parameter_225,
        parameter_226,
        parameter_227,
        parameter_228,
        parameter_229,
        parameter_230,
        parameter_231,
        parameter_232,
        parameter_233,
        parameter_234,
        parameter_235,
        parameter_236,
        parameter_237,
        parameter_238,
        parameter_239,
        parameter_240,
        parameter_241,
        parameter_242,
        parameter_243,
        parameter_244,
        parameter_245,
        parameter_246,
        parameter_247,
        parameter_248,
        parameter_249,
        parameter_250,
        parameter_251,
        parameter_252,
        parameter_253,
        parameter_254,
        parameter_255,
        parameter_256,
        parameter_257,
        parameter_258,
        parameter_259,
        parameter_260,
        parameter_261,
        parameter_262,
        parameter_263,
        parameter_264,
        parameter_265,
        parameter_266,
        parameter_267,
        parameter_268,
        parameter_269,
        parameter_270,
        data_0,
    ):
        # pd_op.conv2d: (128x32x112x112xf32) <- (128x3x224x224xf32, 32x3x3x3xf32)
        conv2d_0 = paddle._C_ops.conv2d(
            data_0, parameter_270, [2, 2], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x32x112x112xf32, 32xf32, 32xf32, 32xf32, 32xf32, -1xui8) <- (128x32x112x112xf32, 32xf32, 32xf32, 32xf32, 32xf32)
        (
            batch_norm__269,
            batch_norm__0,
            batch_norm__1,
            batch_norm__2,
            batch_norm__3,
            batch_norm__4,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_0,
                parameter_269,
                parameter_268,
                parameter_267,
                parameter_266,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x32x112x112xf32) <- (128x32x112x112xf32)
        relu_0 = paddle._C_ops.relu(batch_norm__269)

        # pd_op.depthwise_conv2d: (128x32x112x112xf32) <- (128x32x112x112xf32, 32x1x3x3xf32)
        depthwise_conv2d_0 = paddle._C_ops.depthwise_conv2d(
            relu_0, parameter_265, [1, 1], [1, 1], "EXPLICIT", 32, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x32x112x112xf32, 32xf32, 32xf32, 32xf32, 32xf32, -1xui8) <- (128x32x112x112xf32, 32xf32, 32xf32, 32xf32, 32xf32)
        (
            batch_norm__270,
            batch_norm__5,
            batch_norm__6,
            batch_norm__7,
            batch_norm__8,
            batch_norm__9,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_0,
                parameter_264,
                parameter_263,
                parameter_262,
                parameter_261,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x32x112x112xf32) <- (128x32x112x112xf32)
        relu_1 = paddle._C_ops.relu(batch_norm__270)

        # pd_op.conv2d: (128x64x112x112xf32) <- (128x32x112x112xf32, 64x32x1x1xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            relu_1, parameter_260, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x64x112x112xf32, 64xf32, 64xf32, 64xf32, 64xf32, -1xui8) <- (128x64x112x112xf32, 64xf32, 64xf32, 64xf32, 64xf32)
        (
            batch_norm__271,
            batch_norm__10,
            batch_norm__11,
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_1,
                parameter_259,
                parameter_258,
                parameter_257,
                parameter_256,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x64x112x112xf32) <- (128x64x112x112xf32)
        relu_2 = paddle._C_ops.relu(batch_norm__271)

        # pd_op.depthwise_conv2d: (128x64x56x56xf32) <- (128x64x112x112xf32, 64x1x3x3xf32)
        depthwise_conv2d_1 = paddle._C_ops.depthwise_conv2d(
            relu_2, parameter_255, [2, 2], [1, 1], "EXPLICIT", 64, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x64x56x56xf32, 64xf32, 64xf32, 64xf32, 64xf32, -1xui8) <- (128x64x56x56xf32, 64xf32, 64xf32, 64xf32, 64xf32)
        (
            batch_norm__272,
            batch_norm__15,
            batch_norm__16,
            batch_norm__17,
            batch_norm__18,
            batch_norm__19,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_1,
                parameter_254,
                parameter_253,
                parameter_252,
                parameter_251,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x64x56x56xf32) <- (128x64x56x56xf32)
        relu_3 = paddle._C_ops.relu(batch_norm__272)

        # pd_op.conv2d: (128x128x56x56xf32) <- (128x64x56x56xf32, 128x64x1x1xf32)
        conv2d_2 = paddle._C_ops.conv2d(
            relu_3, parameter_250, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x128x56x56xf32, 128xf32, 128xf32, 128xf32, 128xf32, -1xui8) <- (128x128x56x56xf32, 128xf32, 128xf32, 128xf32, 128xf32)
        (
            batch_norm__273,
            batch_norm__20,
            batch_norm__21,
            batch_norm__22,
            batch_norm__23,
            batch_norm__24,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_2,
                parameter_249,
                parameter_248,
                parameter_247,
                parameter_246,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x128x56x56xf32) <- (128x128x56x56xf32)
        relu_4 = paddle._C_ops.relu(batch_norm__273)

        # pd_op.depthwise_conv2d: (128x128x56x56xf32) <- (128x128x56x56xf32, 128x1x3x3xf32)
        depthwise_conv2d_2 = paddle._C_ops.depthwise_conv2d(
            relu_4, parameter_245, [1, 1], [1, 1], "EXPLICIT", 128, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x128x56x56xf32, 128xf32, 128xf32, 128xf32, 128xf32, -1xui8) <- (128x128x56x56xf32, 128xf32, 128xf32, 128xf32, 128xf32)
        (
            batch_norm__274,
            batch_norm__25,
            batch_norm__26,
            batch_norm__27,
            batch_norm__28,
            batch_norm__29,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_2,
                parameter_244,
                parameter_243,
                parameter_242,
                parameter_241,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x128x56x56xf32) <- (128x128x56x56xf32)
        relu_5 = paddle._C_ops.relu(batch_norm__274)

        # pd_op.conv2d: (128x128x56x56xf32) <- (128x128x56x56xf32, 128x128x1x1xf32)
        conv2d_3 = paddle._C_ops.conv2d(
            relu_5, parameter_240, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x128x56x56xf32, 128xf32, 128xf32, 128xf32, 128xf32, -1xui8) <- (128x128x56x56xf32, 128xf32, 128xf32, 128xf32, 128xf32)
        (
            batch_norm__275,
            batch_norm__30,
            batch_norm__31,
            batch_norm__32,
            batch_norm__33,
            batch_norm__34,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_3,
                parameter_239,
                parameter_238,
                parameter_237,
                parameter_236,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x128x56x56xf32) <- (128x128x56x56xf32)
        relu_6 = paddle._C_ops.relu(batch_norm__275)

        # pd_op.depthwise_conv2d: (128x128x28x28xf32) <- (128x128x56x56xf32, 128x1x3x3xf32)
        depthwise_conv2d_3 = paddle._C_ops.depthwise_conv2d(
            relu_6, parameter_235, [2, 2], [1, 1], "EXPLICIT", 128, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x128x28x28xf32, 128xf32, 128xf32, 128xf32, 128xf32, -1xui8) <- (128x128x28x28xf32, 128xf32, 128xf32, 128xf32, 128xf32)
        (
            batch_norm__276,
            batch_norm__35,
            batch_norm__36,
            batch_norm__37,
            batch_norm__38,
            batch_norm__39,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_3,
                parameter_234,
                parameter_233,
                parameter_232,
                parameter_231,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x128x28x28xf32) <- (128x128x28x28xf32)
        relu_7 = paddle._C_ops.relu(batch_norm__276)

        # pd_op.conv2d: (128x256x28x28xf32) <- (128x128x28x28xf32, 256x128x1x1xf32)
        conv2d_4 = paddle._C_ops.conv2d(
            relu_7, parameter_230, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x28x28xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x28x28xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__277,
            batch_norm__40,
            batch_norm__41,
            batch_norm__42,
            batch_norm__43,
            batch_norm__44,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_4,
                parameter_229,
                parameter_228,
                parameter_227,
                parameter_226,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x28x28xf32) <- (128x256x28x28xf32)
        relu_8 = paddle._C_ops.relu(batch_norm__277)

        # pd_op.depthwise_conv2d: (128x256x28x28xf32) <- (128x256x28x28xf32, 256x1x3x3xf32)
        depthwise_conv2d_4 = paddle._C_ops.depthwise_conv2d(
            relu_8, parameter_225, [1, 1], [1, 1], "EXPLICIT", 256, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x256x28x28xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x28x28xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__278,
            batch_norm__45,
            batch_norm__46,
            batch_norm__47,
            batch_norm__48,
            batch_norm__49,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_4,
                parameter_224,
                parameter_223,
                parameter_222,
                parameter_221,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x28x28xf32) <- (128x256x28x28xf32)
        relu_9 = paddle._C_ops.relu(batch_norm__278)

        # pd_op.conv2d: (128x256x28x28xf32) <- (128x256x28x28xf32, 256x256x1x1xf32)
        conv2d_5 = paddle._C_ops.conv2d(
            relu_9, parameter_220, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x28x28xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x28x28xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__279,
            batch_norm__50,
            batch_norm__51,
            batch_norm__52,
            batch_norm__53,
            batch_norm__54,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_5,
                parameter_219,
                parameter_218,
                parameter_217,
                parameter_216,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x28x28xf32) <- (128x256x28x28xf32)
        relu_10 = paddle._C_ops.relu(batch_norm__279)

        # pd_op.depthwise_conv2d: (128x256x14x14xf32) <- (128x256x28x28xf32, 256x1x5x5xf32)
        depthwise_conv2d_5 = paddle._C_ops.depthwise_conv2d(
            relu_10, parameter_215, [2, 2], [2, 2], "EXPLICIT", 256, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__55,
            batch_norm__56,
            batch_norm__57,
            batch_norm__58,
            batch_norm__59,
            batch_norm__60,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_5,
                parameter_214,
                parameter_213,
                parameter_212,
                parameter_211,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x256x14x14xf32) <- (128x256x28x28xf32, 256x1x3x3xf32)
        depthwise_conv2d_6 = paddle._C_ops.depthwise_conv2d(
            relu_10, parameter_210, [2, 2], [1, 1], "EXPLICIT", 256, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__61,
            batch_norm__62,
            batch_norm__63,
            batch_norm__64,
            batch_norm__65,
            batch_norm__66,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_6,
                parameter_209,
                parameter_208,
                parameter_207,
                parameter_206,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x256x14x14xf32) <- (128x256x14x14xf32, 128x256x14x14xf32)
        add_8 = paddle._C_ops.add(batch_norm__55, batch_norm__61)

        # pd_op.relu: (128x256x14x14xf32) <- (128x256x14x14xf32)
        relu_11 = paddle._C_ops.relu(add_8)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_0 = [1, 1]

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_5 = full_int_array_0

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_4 = full_int_array_0

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_3 = full_int_array_0

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_2 = full_int_array_0

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_1 = full_int_array_0

        # pd_op.assign: (2xi64) <- (2xi64)
        assign_0 = full_int_array_0

        # pd_op.pool2d: (128x256x1x1xf32) <- (128x256x14x14xf32, 2xi64)
        pool2d_0 = paddle._C_ops.pool2d(
            relu_11,
            full_int_array_0,
            [1, 1],
            [0, 0],
            False,
            True,
            "NCHW",
            "avg",
            False,
            True,
            "EXPLICIT",
        )

        # pd_op.conv2d: (128x64x1x1xf32) <- (128x256x1x1xf32, 64x256x1x1xf32)
        conv2d_6 = paddle._C_ops.conv2d(
            pool2d_0, parameter_205, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_1 = [1, -1, 1, 1]

        # pd_op.reshape: (1x64x1x1xf32) <- (64xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(parameter_204, full_int_array_1)

        # pd_op.add: (128x64x1x1xf32) <- (128x64x1x1xf32, 1x64x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_6, reshape_0)

        # pd_op.relu: (128x64x1x1xf32) <- (128x64x1x1xf32)
        relu_12 = paddle._C_ops.relu(add_9)

        # pd_op.conv2d: (128x256x1x1xf32) <- (128x64x1x1xf32, 256x64x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            relu_12, parameter_203, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x256x1x1xf32) <- (256xf32, 4xi64)
        reshape_1 = paddle._C_ops.reshape(parameter_202, full_int_array_1)

        # pd_op.add: (128x256x1x1xf32) <- (128x256x1x1xf32, 1x256x1x1xf32)
        add_10 = paddle._C_ops.add(conv2d_7, reshape_1)

        # pd_op.sigmoid: (128x256x1x1xf32) <- (128x256x1x1xf32)
        sigmoid_0 = paddle._C_ops.sigmoid(add_10)

        # pd_op.multiply: (128x256x14x14xf32) <- (128x256x14x14xf32, 128x256x1x1xf32)
        multiply_0 = paddle._C_ops.multiply(relu_11, sigmoid_0)

        # pd_op.conv2d: (128x256x14x14xf32) <- (128x256x14x14xf32, 256x256x1x1xf32)
        conv2d_8 = paddle._C_ops.conv2d(
            multiply_0, parameter_201, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__280,
            batch_norm__67,
            batch_norm__68,
            batch_norm__69,
            batch_norm__70,
            batch_norm__71,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_8,
                parameter_200,
                parameter_199,
                parameter_198,
                parameter_197,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x14x14xf32) <- (128x256x14x14xf32)
        relu_13 = paddle._C_ops.relu(batch_norm__280)

        # pd_op.conv2d: (128x512x14x14xf32) <- (128x256x14x14xf32, 512x256x1x1xf32)
        conv2d_9 = paddle._C_ops.conv2d(
            relu_13, parameter_196, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__281,
            batch_norm__72,
            batch_norm__73,
            batch_norm__74,
            batch_norm__75,
            batch_norm__76,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_9,
                parameter_195,
                parameter_194,
                parameter_193,
                parameter_192,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_14 = paddle._C_ops.relu(batch_norm__281)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x5x5xf32)
        depthwise_conv2d_7 = paddle._C_ops.depthwise_conv2d(
            relu_14, parameter_191, [1, 1], [2, 2], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__77,
            batch_norm__78,
            batch_norm__79,
            batch_norm__80,
            batch_norm__81,
            batch_norm__82,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_7,
                parameter_190,
                parameter_189,
                parameter_188,
                parameter_187,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x3x3xf32)
        depthwise_conv2d_8 = paddle._C_ops.depthwise_conv2d(
            relu_14, parameter_186, [1, 1], [1, 1], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__83,
            batch_norm__84,
            batch_norm__85,
            batch_norm__86,
            batch_norm__87,
            batch_norm__88,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_8,
                parameter_185,
                parameter_184,
                parameter_183,
                parameter_182,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_0 = paddle._C_ops.add(batch_norm__77, batch_norm__83)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x1x1xf32)
        depthwise_conv2d_9 = paddle._C_ops.depthwise_conv2d(
            relu_14, parameter_181, [1, 1], [0, 0], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__89,
            batch_norm__90,
            batch_norm__91,
            batch_norm__92,
            batch_norm__93,
            batch_norm__94,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_9,
                parameter_180,
                parameter_179,
                parameter_178,
                parameter_177,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_11 = paddle._C_ops.add(add_0, batch_norm__89)

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_15 = paddle._C_ops.relu(add_11)

        # pd_op.pool2d: (128x512x1x1xf32) <- (128x512x14x14xf32, 2xi64)
        pool2d_1 = paddle._C_ops.pool2d(
            relu_15,
            full_int_array_0,
            [1, 1],
            [0, 0],
            False,
            True,
            "NCHW",
            "avg",
            False,
            True,
            "EXPLICIT",
        )

        # pd_op.conv2d: (128x128x1x1xf32) <- (128x512x1x1xf32, 128x512x1x1xf32)
        conv2d_10 = paddle._C_ops.conv2d(
            pool2d_1, parameter_176, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x128x1x1xf32) <- (128xf32, 4xi64)
        reshape_2 = paddle._C_ops.reshape(parameter_175, full_int_array_1)

        # pd_op.add: (128x128x1x1xf32) <- (128x128x1x1xf32, 1x128x1x1xf32)
        add_12 = paddle._C_ops.add(conv2d_10, reshape_2)

        # pd_op.relu: (128x128x1x1xf32) <- (128x128x1x1xf32)
        relu_16 = paddle._C_ops.relu(add_12)

        # pd_op.conv2d: (128x512x1x1xf32) <- (128x128x1x1xf32, 512x128x1x1xf32)
        conv2d_11 = paddle._C_ops.conv2d(
            relu_16, parameter_174, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x512x1x1xf32) <- (512xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(parameter_173, full_int_array_1)

        # pd_op.add: (128x512x1x1xf32) <- (128x512x1x1xf32, 1x512x1x1xf32)
        add_13 = paddle._C_ops.add(conv2d_11, reshape_3)

        # pd_op.sigmoid: (128x512x1x1xf32) <- (128x512x1x1xf32)
        sigmoid_1 = paddle._C_ops.sigmoid(add_13)

        # pd_op.multiply: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x1x1xf32)
        multiply_1 = paddle._C_ops.multiply(relu_15, sigmoid_1)

        # pd_op.conv2d: (128x256x14x14xf32) <- (128x512x14x14xf32, 256x512x1x1xf32)
        conv2d_12 = paddle._C_ops.conv2d(
            multiply_1, parameter_172, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__282,
            batch_norm__95,
            batch_norm__96,
            batch_norm__97,
            batch_norm__98,
            batch_norm__99,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_12,
                parameter_171,
                parameter_170,
                parameter_169,
                parameter_168,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x14x14xf32) <- (128x256x14x14xf32)
        relu_17 = paddle._C_ops.relu(batch_norm__282)

        # pd_op.conv2d: (128x512x14x14xf32) <- (128x256x14x14xf32, 512x256x1x1xf32)
        conv2d_13 = paddle._C_ops.conv2d(
            relu_17, parameter_167, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__283,
            batch_norm__100,
            batch_norm__101,
            batch_norm__102,
            batch_norm__103,
            batch_norm__104,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_13,
                parameter_166,
                parameter_165,
                parameter_164,
                parameter_163,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_18 = paddle._C_ops.relu(batch_norm__283)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x5x5xf32)
        depthwise_conv2d_10 = paddle._C_ops.depthwise_conv2d(
            relu_18, parameter_162, [1, 1], [2, 2], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__105,
            batch_norm__106,
            batch_norm__107,
            batch_norm__108,
            batch_norm__109,
            batch_norm__110,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_10,
                parameter_161,
                parameter_160,
                parameter_159,
                parameter_158,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x3x3xf32)
        depthwise_conv2d_11 = paddle._C_ops.depthwise_conv2d(
            relu_18, parameter_157, [1, 1], [1, 1], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__111,
            batch_norm__112,
            batch_norm__113,
            batch_norm__114,
            batch_norm__115,
            batch_norm__116,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_11,
                parameter_156,
                parameter_155,
                parameter_154,
                parameter_153,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_1 = paddle._C_ops.add(batch_norm__105, batch_norm__111)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x1x1xf32)
        depthwise_conv2d_12 = paddle._C_ops.depthwise_conv2d(
            relu_18, parameter_152, [1, 1], [0, 0], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__117,
            batch_norm__118,
            batch_norm__119,
            batch_norm__120,
            batch_norm__121,
            batch_norm__122,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_12,
                parameter_151,
                parameter_150,
                parameter_149,
                parameter_148,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_14 = paddle._C_ops.add(add_1, batch_norm__117)

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_19 = paddle._C_ops.relu(add_14)

        # pd_op.pool2d: (128x512x1x1xf32) <- (128x512x14x14xf32, 2xi64)
        pool2d_2 = paddle._C_ops.pool2d(
            relu_19,
            full_int_array_0,
            [1, 1],
            [0, 0],
            False,
            True,
            "NCHW",
            "avg",
            False,
            True,
            "EXPLICIT",
        )

        # pd_op.conv2d: (128x128x1x1xf32) <- (128x512x1x1xf32, 128x512x1x1xf32)
        conv2d_14 = paddle._C_ops.conv2d(
            pool2d_2, parameter_147, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x128x1x1xf32) <- (128xf32, 4xi64)
        reshape_4 = paddle._C_ops.reshape(parameter_146, full_int_array_1)

        # pd_op.add: (128x128x1x1xf32) <- (128x128x1x1xf32, 1x128x1x1xf32)
        add_15 = paddle._C_ops.add(conv2d_14, reshape_4)

        # pd_op.relu: (128x128x1x1xf32) <- (128x128x1x1xf32)
        relu_20 = paddle._C_ops.relu(add_15)

        # pd_op.conv2d: (128x512x1x1xf32) <- (128x128x1x1xf32, 512x128x1x1xf32)
        conv2d_15 = paddle._C_ops.conv2d(
            relu_20, parameter_145, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x512x1x1xf32) <- (512xf32, 4xi64)
        reshape_5 = paddle._C_ops.reshape(parameter_144, full_int_array_1)

        # pd_op.add: (128x512x1x1xf32) <- (128x512x1x1xf32, 1x512x1x1xf32)
        add_16 = paddle._C_ops.add(conv2d_15, reshape_5)

        # pd_op.sigmoid: (128x512x1x1xf32) <- (128x512x1x1xf32)
        sigmoid_2 = paddle._C_ops.sigmoid(add_16)

        # pd_op.multiply: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x1x1xf32)
        multiply_2 = paddle._C_ops.multiply(relu_19, sigmoid_2)

        # pd_op.conv2d: (128x256x14x14xf32) <- (128x512x14x14xf32, 256x512x1x1xf32)
        conv2d_16 = paddle._C_ops.conv2d(
            multiply_2, parameter_143, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__284,
            batch_norm__123,
            batch_norm__124,
            batch_norm__125,
            batch_norm__126,
            batch_norm__127,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_16,
                parameter_142,
                parameter_141,
                parameter_140,
                parameter_139,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x14x14xf32) <- (128x256x14x14xf32)
        relu_21 = paddle._C_ops.relu(batch_norm__284)

        # pd_op.conv2d: (128x512x14x14xf32) <- (128x256x14x14xf32, 512x256x1x1xf32)
        conv2d_17 = paddle._C_ops.conv2d(
            relu_21, parameter_138, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__285,
            batch_norm__128,
            batch_norm__129,
            batch_norm__130,
            batch_norm__131,
            batch_norm__132,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_17,
                parameter_137,
                parameter_136,
                parameter_135,
                parameter_134,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_22 = paddle._C_ops.relu(batch_norm__285)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x5x5xf32)
        depthwise_conv2d_13 = paddle._C_ops.depthwise_conv2d(
            relu_22, parameter_133, [1, 1], [2, 2], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__133,
            batch_norm__134,
            batch_norm__135,
            batch_norm__136,
            batch_norm__137,
            batch_norm__138,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_13,
                parameter_132,
                parameter_131,
                parameter_130,
                parameter_129,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x3x3xf32)
        depthwise_conv2d_14 = paddle._C_ops.depthwise_conv2d(
            relu_22, parameter_128, [1, 1], [1, 1], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__139,
            batch_norm__140,
            batch_norm__141,
            batch_norm__142,
            batch_norm__143,
            batch_norm__144,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_14,
                parameter_127,
                parameter_126,
                parameter_125,
                parameter_124,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_2 = paddle._C_ops.add(batch_norm__133, batch_norm__139)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x1x1xf32)
        depthwise_conv2d_15 = paddle._C_ops.depthwise_conv2d(
            relu_22, parameter_123, [1, 1], [0, 0], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__145,
            batch_norm__146,
            batch_norm__147,
            batch_norm__148,
            batch_norm__149,
            batch_norm__150,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_15,
                parameter_122,
                parameter_121,
                parameter_120,
                parameter_119,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_17 = paddle._C_ops.add(add_2, batch_norm__145)

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_23 = paddle._C_ops.relu(add_17)

        # pd_op.pool2d: (128x512x1x1xf32) <- (128x512x14x14xf32, 2xi64)
        pool2d_3 = paddle._C_ops.pool2d(
            relu_23,
            full_int_array_0,
            [1, 1],
            [0, 0],
            False,
            True,
            "NCHW",
            "avg",
            False,
            True,
            "EXPLICIT",
        )

        # pd_op.conv2d: (128x128x1x1xf32) <- (128x512x1x1xf32, 128x512x1x1xf32)
        conv2d_18 = paddle._C_ops.conv2d(
            pool2d_3, parameter_118, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x128x1x1xf32) <- (128xf32, 4xi64)
        reshape_6 = paddle._C_ops.reshape(parameter_117, full_int_array_1)

        # pd_op.add: (128x128x1x1xf32) <- (128x128x1x1xf32, 1x128x1x1xf32)
        add_18 = paddle._C_ops.add(conv2d_18, reshape_6)

        # pd_op.relu: (128x128x1x1xf32) <- (128x128x1x1xf32)
        relu_24 = paddle._C_ops.relu(add_18)

        # pd_op.conv2d: (128x512x1x1xf32) <- (128x128x1x1xf32, 512x128x1x1xf32)
        conv2d_19 = paddle._C_ops.conv2d(
            relu_24, parameter_116, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x512x1x1xf32) <- (512xf32, 4xi64)
        reshape_7 = paddle._C_ops.reshape(parameter_115, full_int_array_1)

        # pd_op.add: (128x512x1x1xf32) <- (128x512x1x1xf32, 1x512x1x1xf32)
        add_19 = paddle._C_ops.add(conv2d_19, reshape_7)

        # pd_op.sigmoid: (128x512x1x1xf32) <- (128x512x1x1xf32)
        sigmoid_3 = paddle._C_ops.sigmoid(add_19)

        # pd_op.multiply: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x1x1xf32)
        multiply_3 = paddle._C_ops.multiply(relu_23, sigmoid_3)

        # pd_op.conv2d: (128x256x14x14xf32) <- (128x512x14x14xf32, 256x512x1x1xf32)
        conv2d_20 = paddle._C_ops.conv2d(
            multiply_3, parameter_114, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__286,
            batch_norm__151,
            batch_norm__152,
            batch_norm__153,
            batch_norm__154,
            batch_norm__155,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_20,
                parameter_113,
                parameter_112,
                parameter_111,
                parameter_110,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x14x14xf32) <- (128x256x14x14xf32)
        relu_25 = paddle._C_ops.relu(batch_norm__286)

        # pd_op.conv2d: (128x512x14x14xf32) <- (128x256x14x14xf32, 512x256x1x1xf32)
        conv2d_21 = paddle._C_ops.conv2d(
            relu_25, parameter_109, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__287,
            batch_norm__156,
            batch_norm__157,
            batch_norm__158,
            batch_norm__159,
            batch_norm__160,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_21,
                parameter_108,
                parameter_107,
                parameter_106,
                parameter_105,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_26 = paddle._C_ops.relu(batch_norm__287)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x5x5xf32)
        depthwise_conv2d_16 = paddle._C_ops.depthwise_conv2d(
            relu_26, parameter_104, [1, 1], [2, 2], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__161,
            batch_norm__162,
            batch_norm__163,
            batch_norm__164,
            batch_norm__165,
            batch_norm__166,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_16,
                parameter_103,
                parameter_102,
                parameter_101,
                parameter_100,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x3x3xf32)
        depthwise_conv2d_17 = paddle._C_ops.depthwise_conv2d(
            relu_26, parameter_99, [1, 1], [1, 1], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__167,
            batch_norm__168,
            batch_norm__169,
            batch_norm__170,
            batch_norm__171,
            batch_norm__172,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_17,
                parameter_98,
                parameter_97,
                parameter_96,
                parameter_95,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_3 = paddle._C_ops.add(batch_norm__161, batch_norm__167)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x1x1xf32)
        depthwise_conv2d_18 = paddle._C_ops.depthwise_conv2d(
            relu_26, parameter_94, [1, 1], [0, 0], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__173,
            batch_norm__174,
            batch_norm__175,
            batch_norm__176,
            batch_norm__177,
            batch_norm__178,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_18,
                parameter_93,
                parameter_92,
                parameter_91,
                parameter_90,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_20 = paddle._C_ops.add(add_3, batch_norm__173)

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_27 = paddle._C_ops.relu(add_20)

        # pd_op.pool2d: (128x512x1x1xf32) <- (128x512x14x14xf32, 2xi64)
        pool2d_4 = paddle._C_ops.pool2d(
            relu_27,
            full_int_array_0,
            [1, 1],
            [0, 0],
            False,
            True,
            "NCHW",
            "avg",
            False,
            True,
            "EXPLICIT",
        )

        # pd_op.conv2d: (128x128x1x1xf32) <- (128x512x1x1xf32, 128x512x1x1xf32)
        conv2d_22 = paddle._C_ops.conv2d(
            pool2d_4, parameter_89, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x128x1x1xf32) <- (128xf32, 4xi64)
        reshape_8 = paddle._C_ops.reshape(parameter_88, full_int_array_1)

        # pd_op.add: (128x128x1x1xf32) <- (128x128x1x1xf32, 1x128x1x1xf32)
        add_21 = paddle._C_ops.add(conv2d_22, reshape_8)

        # pd_op.relu: (128x128x1x1xf32) <- (128x128x1x1xf32)
        relu_28 = paddle._C_ops.relu(add_21)

        # pd_op.conv2d: (128x512x1x1xf32) <- (128x128x1x1xf32, 512x128x1x1xf32)
        conv2d_23 = paddle._C_ops.conv2d(
            relu_28, parameter_87, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x512x1x1xf32) <- (512xf32, 4xi64)
        reshape_9 = paddle._C_ops.reshape(parameter_86, full_int_array_1)

        # pd_op.add: (128x512x1x1xf32) <- (128x512x1x1xf32, 1x512x1x1xf32)
        add_22 = paddle._C_ops.add(conv2d_23, reshape_9)

        # pd_op.sigmoid: (128x512x1x1xf32) <- (128x512x1x1xf32)
        sigmoid_4 = paddle._C_ops.sigmoid(add_22)

        # pd_op.multiply: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x1x1xf32)
        multiply_4 = paddle._C_ops.multiply(relu_27, sigmoid_4)

        # pd_op.conv2d: (128x256x14x14xf32) <- (128x512x14x14xf32, 256x512x1x1xf32)
        conv2d_24 = paddle._C_ops.conv2d(
            multiply_4, parameter_85, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__288,
            batch_norm__179,
            batch_norm__180,
            batch_norm__181,
            batch_norm__182,
            batch_norm__183,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_24,
                parameter_84,
                parameter_83,
                parameter_82,
                parameter_81,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x14x14xf32) <- (128x256x14x14xf32)
        relu_29 = paddle._C_ops.relu(batch_norm__288)

        # pd_op.conv2d: (128x512x14x14xf32) <- (128x256x14x14xf32, 512x256x1x1xf32)
        conv2d_25 = paddle._C_ops.conv2d(
            relu_29, parameter_80, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__289,
            batch_norm__184,
            batch_norm__185,
            batch_norm__186,
            batch_norm__187,
            batch_norm__188,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_25,
                parameter_79,
                parameter_78,
                parameter_77,
                parameter_76,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_30 = paddle._C_ops.relu(batch_norm__289)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x5x5xf32)
        depthwise_conv2d_19 = paddle._C_ops.depthwise_conv2d(
            relu_30, parameter_75, [1, 1], [2, 2], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__189,
            batch_norm__190,
            batch_norm__191,
            batch_norm__192,
            batch_norm__193,
            batch_norm__194,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_19,
                parameter_74,
                parameter_73,
                parameter_72,
                parameter_71,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x3x3xf32)
        depthwise_conv2d_20 = paddle._C_ops.depthwise_conv2d(
            relu_30, parameter_70, [1, 1], [1, 1], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__195,
            batch_norm__196,
            batch_norm__197,
            batch_norm__198,
            batch_norm__199,
            batch_norm__200,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_20,
                parameter_69,
                parameter_68,
                parameter_67,
                parameter_66,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_4 = paddle._C_ops.add(batch_norm__189, batch_norm__195)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x1x1xf32)
        depthwise_conv2d_21 = paddle._C_ops.depthwise_conv2d(
            relu_30, parameter_65, [1, 1], [0, 0], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__201,
            batch_norm__202,
            batch_norm__203,
            batch_norm__204,
            batch_norm__205,
            batch_norm__206,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_21,
                parameter_64,
                parameter_63,
                parameter_62,
                parameter_61,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_23 = paddle._C_ops.add(add_4, batch_norm__201)

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_31 = paddle._C_ops.relu(add_23)

        # pd_op.pool2d: (128x512x1x1xf32) <- (128x512x14x14xf32, 2xi64)
        pool2d_5 = paddle._C_ops.pool2d(
            relu_31,
            full_int_array_0,
            [1, 1],
            [0, 0],
            False,
            True,
            "NCHW",
            "avg",
            False,
            True,
            "EXPLICIT",
        )

        # pd_op.conv2d: (128x128x1x1xf32) <- (128x512x1x1xf32, 128x512x1x1xf32)
        conv2d_26 = paddle._C_ops.conv2d(
            pool2d_5, parameter_60, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x128x1x1xf32) <- (128xf32, 4xi64)
        reshape_10 = paddle._C_ops.reshape(parameter_59, full_int_array_1)

        # pd_op.add: (128x128x1x1xf32) <- (128x128x1x1xf32, 1x128x1x1xf32)
        add_24 = paddle._C_ops.add(conv2d_26, reshape_10)

        # pd_op.relu: (128x128x1x1xf32) <- (128x128x1x1xf32)
        relu_32 = paddle._C_ops.relu(add_24)

        # pd_op.conv2d: (128x512x1x1xf32) <- (128x128x1x1xf32, 512x128x1x1xf32)
        conv2d_27 = paddle._C_ops.conv2d(
            relu_32, parameter_58, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x512x1x1xf32) <- (512xf32, 4xi64)
        reshape_11 = paddle._C_ops.reshape(parameter_57, full_int_array_1)

        # pd_op.add: (128x512x1x1xf32) <- (128x512x1x1xf32, 1x512x1x1xf32)
        add_25 = paddle._C_ops.add(conv2d_27, reshape_11)

        # pd_op.sigmoid: (128x512x1x1xf32) <- (128x512x1x1xf32)
        sigmoid_5 = paddle._C_ops.sigmoid(add_25)

        # pd_op.multiply: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x1x1xf32)
        multiply_5 = paddle._C_ops.multiply(relu_31, sigmoid_5)

        # pd_op.conv2d: (128x256x14x14xf32) <- (128x512x14x14xf32, 256x512x1x1xf32)
        conv2d_28 = paddle._C_ops.conv2d(
            multiply_5, parameter_56, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32, -1xui8) <- (128x256x14x14xf32, 256xf32, 256xf32, 256xf32, 256xf32)
        (
            batch_norm__290,
            batch_norm__207,
            batch_norm__208,
            batch_norm__209,
            batch_norm__210,
            batch_norm__211,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_28,
                parameter_55,
                parameter_54,
                parameter_53,
                parameter_52,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x256x14x14xf32) <- (128x256x14x14xf32)
        relu_33 = paddle._C_ops.relu(batch_norm__290)

        # pd_op.conv2d: (128x512x14x14xf32) <- (128x256x14x14xf32, 512x256x1x1xf32)
        conv2d_29 = paddle._C_ops.conv2d(
            relu_33, parameter_51, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__291,
            batch_norm__212,
            batch_norm__213,
            batch_norm__214,
            batch_norm__215,
            batch_norm__216,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_29,
                parameter_50,
                parameter_49,
                parameter_48,
                parameter_47,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_34 = paddle._C_ops.relu(batch_norm__291)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x5x5xf32)
        depthwise_conv2d_22 = paddle._C_ops.depthwise_conv2d(
            relu_34, parameter_46, [1, 1], [2, 2], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__217,
            batch_norm__218,
            batch_norm__219,
            batch_norm__220,
            batch_norm__221,
            batch_norm__222,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_22,
                parameter_45,
                parameter_44,
                parameter_43,
                parameter_42,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x3x3xf32)
        depthwise_conv2d_23 = paddle._C_ops.depthwise_conv2d(
            relu_34, parameter_41, [1, 1], [1, 1], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__223,
            batch_norm__224,
            batch_norm__225,
            batch_norm__226,
            batch_norm__227,
            batch_norm__228,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_23,
                parameter_40,
                parameter_39,
                parameter_38,
                parameter_37,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_5 = paddle._C_ops.add(batch_norm__217, batch_norm__223)

        # pd_op.depthwise_conv2d: (128x512x14x14xf32) <- (128x512x14x14xf32, 512x1x1x1xf32)
        depthwise_conv2d_24 = paddle._C_ops.depthwise_conv2d(
            relu_34, parameter_36, [1, 1], [0, 0], "EXPLICIT", 512, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512x14x14xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__229,
            batch_norm__230,
            batch_norm__231,
            batch_norm__232,
            batch_norm__233,
            batch_norm__234,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_24,
                parameter_35,
                parameter_34,
                parameter_33,
                parameter_32,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x512x14x14xf32) <- (128x512x14x14xf32, 128x512x14x14xf32)
        add_26 = paddle._C_ops.add(add_5, batch_norm__229)

        # pd_op.relu: (128x512x14x14xf32) <- (128x512x14x14xf32)
        relu_35 = paddle._C_ops.relu(add_26)

        # pd_op.conv2d: (128x1024x14x14xf32) <- (128x512x14x14xf32, 1024x512x1x1xf32)
        conv2d_30 = paddle._C_ops.conv2d(
            relu_35, parameter_31, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32, -1xui8) <- (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32)
        (
            batch_norm__292,
            batch_norm__235,
            batch_norm__236,
            batch_norm__237,
            batch_norm__238,
            batch_norm__239,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_30,
                parameter_30,
                parameter_29,
                parameter_28,
                parameter_27,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x1024x14x14xf32) <- (128x1024x14x14xf32)
        relu_36 = paddle._C_ops.relu(batch_norm__292)

        # pd_op.depthwise_conv2d: (128x1024x14x14xf32) <- (128x1024x14x14xf32, 1024x1x5x5xf32)
        depthwise_conv2d_25 = paddle._C_ops.depthwise_conv2d(
            relu_36, parameter_26, [1, 1], [2, 2], "EXPLICIT", 1024, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32, -1xui8) <- (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32)
        (
            batch_norm__240,
            batch_norm__241,
            batch_norm__242,
            batch_norm__243,
            batch_norm__244,
            batch_norm__245,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_25,
                parameter_25,
                parameter_24,
                parameter_23,
                parameter_22,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.depthwise_conv2d: (128x1024x14x14xf32) <- (128x1024x14x14xf32, 1024x1x3x3xf32)
        depthwise_conv2d_26 = paddle._C_ops.depthwise_conv2d(
            relu_36, parameter_21, [1, 1], [1, 1], "EXPLICIT", 1024, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32, -1xui8) <- (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32)
        (
            batch_norm__246,
            batch_norm__247,
            batch_norm__248,
            batch_norm__249,
            batch_norm__250,
            batch_norm__251,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_26,
                parameter_20,
                parameter_19,
                parameter_18,
                parameter_17,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x1024x14x14xf32) <- (128x1024x14x14xf32, 128x1024x14x14xf32)
        add_6 = paddle._C_ops.add(batch_norm__240, batch_norm__246)

        # pd_op.depthwise_conv2d: (128x1024x14x14xf32) <- (128x1024x14x14xf32, 1024x1x1x1xf32)
        depthwise_conv2d_27 = paddle._C_ops.depthwise_conv2d(
            relu_36, parameter_16, [1, 1], [0, 0], "EXPLICIT", 1024, [1, 1], "NCHW"
        )

        # pd_op.batch_norm_: (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32, -1xui8) <- (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32)
        (
            batch_norm__252,
            batch_norm__253,
            batch_norm__254,
            batch_norm__255,
            batch_norm__256,
            batch_norm__257,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                depthwise_conv2d_27,
                parameter_15,
                parameter_14,
                parameter_13,
                parameter_12,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.add: (128x1024x14x14xf32) <- (128x1024x14x14xf32, 128x1024x14x14xf32)
        add_27 = paddle._C_ops.add(add_6, batch_norm__252)

        # pd_op.relu: (128x1024x14x14xf32) <- (128x1024x14x14xf32)
        relu_37 = paddle._C_ops.relu(add_27)

        # pd_op.conv2d: (128x1024x14x14xf32) <- (128x1024x14x14xf32, 1024x1024x1x1xf32)
        conv2d_31 = paddle._C_ops.conv2d(
            relu_37, parameter_11, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32, -1xui8) <- (128x1024x14x14xf32, 1024xf32, 1024xf32, 1024xf32, 1024xf32)
        (
            batch_norm__293,
            batch_norm__258,
            batch_norm__259,
            batch_norm__260,
            batch_norm__261,
            batch_norm__262,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                conv2d_31,
                parameter_10,
                parameter_9,
                parameter_8,
                parameter_7,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.relu: (128x1024x14x14xf32) <- (128x1024x14x14xf32)
        relu_38 = paddle._C_ops.relu(batch_norm__293)

        # pd_op.add: (128x1024x14x14xf32) <- (128x1024x14x14xf32, 128x1024x14x14xf32)
        add_7 = paddle._C_ops.add(relu_38, relu_36)

        # pd_op.pool2d: (128x1024x1x1xf32) <- (128x1024x14x14xf32, 2xi64)
        pool2d_6 = paddle._C_ops.pool2d(
            add_7,
            full_int_array_0,
            [1, 1],
            [0, 0],
            False,
            True,
            "NCHW",
            "avg",
            False,
            True,
            "EXPLICIT",
        )

        # pd_op.conv2d: (128x512x1x1xf32) <- (128x1024x1x1xf32, 512x1024x1x1xf32)
        conv2d_32 = paddle._C_ops.conv2d(
            pool2d_6, parameter_6, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x512x1x1xf32) <- (512xf32, 4xi64)
        reshape_12 = paddle._C_ops.reshape(parameter_5, full_int_array_1)

        # pd_op.add: (128x512x1x1xf32) <- (128x512x1x1xf32, 1x512x1x1xf32)
        add_28 = paddle._C_ops.add(conv2d_32, reshape_12)

        # pd_op.full: (1xf32) <- ()
        full_0 = paddle._C_ops.full(
            [1], float("0.2"), paddle.float32, paddle.core.CPUPlace()
        )

        # pd_op.dropout: (128x512x1x1xf32, 128x512x1x1xui8) <- (128x512x1x1xf32, None, 1xf32)
        dropout_0, dropout_1 = (lambda x, f: f(x))(
            paddle._C_ops.dropout(
                add_28, None, full_0, False, "downgrade_in_infer", 0, False
            ),
            lambda out: out if isinstance(out, (list, tuple)) else (out, None),
        )

        # pd_op.flatten: (128x512xf32) <- (128x512x1x1xf32)
        flatten_1 = paddle._C_ops.flatten(dropout_0, 1, 3)

        # pd_op.flatten: (128x512xf32) <- (128x512xf32)
        flatten_0 = paddle._C_ops.flatten(flatten_1, 1, 1)

        # pd_op.batch_norm_: (128x512xf32, 512xf32, 512xf32, 512xf32, 512xf32, -1xui8) <- (128x512xf32, 512xf32, 512xf32, 512xf32, 512xf32)
        (
            batch_norm__268,
            batch_norm__263,
            batch_norm__264,
            batch_norm__265,
            batch_norm__266,
            batch_norm__267,
        ) = (lambda x, f: f(x))(
            paddle._C_ops.batch_norm(
                flatten_0,
                parameter_4,
                parameter_3,
                parameter_2,
                parameter_1,
                False,
                float("0.9"),
                float("1e-05"),
                "NCHW",
                False,
                False,
            ),
            lambda out: out
            if isinstance(out, (list, tuple))
            else (out, None, None, None, None, None),
        )

        # pd_op.matmul: (128x159xf32) <- (128x512xf32, 512x159xf32)
        matmul_0 = paddle._C_ops.matmul(batch_norm__268, parameter_0, False, False)
        return (
            conv2d_0,
            batch_norm__0,
            batch_norm__1,
            batch_norm__2,
            batch_norm__3,
            batch_norm__4,
            relu_0,
            depthwise_conv2d_0,
            batch_norm__5,
            batch_norm__6,
            batch_norm__7,
            batch_norm__8,
            batch_norm__9,
            relu_1,
            conv2d_1,
            batch_norm__10,
            batch_norm__11,
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
            relu_2,
            depthwise_conv2d_1,
            batch_norm__15,
            batch_norm__16,
            batch_norm__17,
            batch_norm__18,
            batch_norm__19,
            relu_3,
            conv2d_2,
            batch_norm__20,
            batch_norm__21,
            batch_norm__22,
            batch_norm__23,
            batch_norm__24,
            relu_4,
            depthwise_conv2d_2,
            batch_norm__25,
            batch_norm__26,
            batch_norm__27,
            batch_norm__28,
            batch_norm__29,
            relu_5,
            conv2d_3,
            batch_norm__30,
            batch_norm__31,
            batch_norm__32,
            batch_norm__33,
            batch_norm__34,
            relu_6,
            depthwise_conv2d_3,
            batch_norm__35,
            batch_norm__36,
            batch_norm__37,
            batch_norm__38,
            batch_norm__39,
            relu_7,
            conv2d_4,
            batch_norm__40,
            batch_norm__41,
            batch_norm__42,
            batch_norm__43,
            batch_norm__44,
            relu_8,
            depthwise_conv2d_4,
            batch_norm__45,
            batch_norm__46,
            batch_norm__47,
            batch_norm__48,
            batch_norm__49,
            relu_9,
            conv2d_5,
            batch_norm__50,
            batch_norm__51,
            batch_norm__52,
            batch_norm__53,
            batch_norm__54,
            relu_10,
            depthwise_conv2d_5,
            batch_norm__55,
            batch_norm__56,
            batch_norm__57,
            batch_norm__58,
            batch_norm__59,
            batch_norm__60,
            depthwise_conv2d_6,
            batch_norm__61,
            batch_norm__62,
            batch_norm__63,
            batch_norm__64,
            batch_norm__65,
            batch_norm__66,
            relu_11,
            full_int_array_0,
            pool2d_0,
            conv2d_6,
            reshape_0,
            relu_12,
            conv2d_7,
            reshape_1,
            sigmoid_0,
            multiply_0,
            conv2d_8,
            batch_norm__67,
            batch_norm__68,
            batch_norm__69,
            batch_norm__70,
            batch_norm__71,
            relu_13,
            conv2d_9,
            batch_norm__72,
            batch_norm__73,
            batch_norm__74,
            batch_norm__75,
            batch_norm__76,
            relu_14,
            depthwise_conv2d_7,
            batch_norm__77,
            batch_norm__78,
            batch_norm__79,
            batch_norm__80,
            batch_norm__81,
            batch_norm__82,
            depthwise_conv2d_8,
            batch_norm__83,
            batch_norm__84,
            batch_norm__85,
            batch_norm__86,
            batch_norm__87,
            batch_norm__88,
            add_0,
            depthwise_conv2d_9,
            batch_norm__89,
            batch_norm__90,
            batch_norm__91,
            batch_norm__92,
            batch_norm__93,
            batch_norm__94,
            relu_15,
            assign_0,
            pool2d_1,
            conv2d_10,
            reshape_2,
            relu_16,
            conv2d_11,
            reshape_3,
            sigmoid_1,
            multiply_1,
            conv2d_12,
            batch_norm__95,
            batch_norm__96,
            batch_norm__97,
            batch_norm__98,
            batch_norm__99,
            relu_17,
            conv2d_13,
            batch_norm__100,
            batch_norm__101,
            batch_norm__102,
            batch_norm__103,
            batch_norm__104,
            relu_18,
            depthwise_conv2d_10,
            batch_norm__105,
            batch_norm__106,
            batch_norm__107,
            batch_norm__108,
            batch_norm__109,
            batch_norm__110,
            depthwise_conv2d_11,
            batch_norm__111,
            batch_norm__112,
            batch_norm__113,
            batch_norm__114,
            batch_norm__115,
            batch_norm__116,
            add_1,
            depthwise_conv2d_12,
            batch_norm__117,
            batch_norm__118,
            batch_norm__119,
            batch_norm__120,
            batch_norm__121,
            batch_norm__122,
            relu_19,
            assign_1,
            pool2d_2,
            conv2d_14,
            reshape_4,
            relu_20,
            conv2d_15,
            reshape_5,
            sigmoid_2,
            multiply_2,
            conv2d_16,
            batch_norm__123,
            batch_norm__124,
            batch_norm__125,
            batch_norm__126,
            batch_norm__127,
            relu_21,
            conv2d_17,
            batch_norm__128,
            batch_norm__129,
            batch_norm__130,
            batch_norm__131,
            batch_norm__132,
            relu_22,
            depthwise_conv2d_13,
            batch_norm__133,
            batch_norm__134,
            batch_norm__135,
            batch_norm__136,
            batch_norm__137,
            batch_norm__138,
            depthwise_conv2d_14,
            batch_norm__139,
            batch_norm__140,
            batch_norm__141,
            batch_norm__142,
            batch_norm__143,
            batch_norm__144,
            add_2,
            depthwise_conv2d_15,
            batch_norm__145,
            batch_norm__146,
            batch_norm__147,
            batch_norm__148,
            batch_norm__149,
            batch_norm__150,
            relu_23,
            assign_2,
            pool2d_3,
            conv2d_18,
            reshape_6,
            relu_24,
            conv2d_19,
            reshape_7,
            sigmoid_3,
            multiply_3,
            conv2d_20,
            batch_norm__151,
            batch_norm__152,
            batch_norm__153,
            batch_norm__154,
            batch_norm__155,
            relu_25,
            conv2d_21,
            batch_norm__156,
            batch_norm__157,
            batch_norm__158,
            batch_norm__159,
            batch_norm__160,
            relu_26,
            depthwise_conv2d_16,
            batch_norm__161,
            batch_norm__162,
            batch_norm__163,
            batch_norm__164,
            batch_norm__165,
            batch_norm__166,
            depthwise_conv2d_17,
            batch_norm__167,
            batch_norm__168,
            batch_norm__169,
            batch_norm__170,
            batch_norm__171,
            batch_norm__172,
            add_3,
            depthwise_conv2d_18,
            batch_norm__173,
            batch_norm__174,
            batch_norm__175,
            batch_norm__176,
            batch_norm__177,
            batch_norm__178,
            relu_27,
            assign_3,
            pool2d_4,
            conv2d_22,
            reshape_8,
            relu_28,
            conv2d_23,
            reshape_9,
            sigmoid_4,
            multiply_4,
            conv2d_24,
            batch_norm__179,
            batch_norm__180,
            batch_norm__181,
            batch_norm__182,
            batch_norm__183,
            relu_29,
            conv2d_25,
            batch_norm__184,
            batch_norm__185,
            batch_norm__186,
            batch_norm__187,
            batch_norm__188,
            relu_30,
            depthwise_conv2d_19,
            batch_norm__189,
            batch_norm__190,
            batch_norm__191,
            batch_norm__192,
            batch_norm__193,
            batch_norm__194,
            depthwise_conv2d_20,
            batch_norm__195,
            batch_norm__196,
            batch_norm__197,
            batch_norm__198,
            batch_norm__199,
            batch_norm__200,
            add_4,
            depthwise_conv2d_21,
            batch_norm__201,
            batch_norm__202,
            batch_norm__203,
            batch_norm__204,
            batch_norm__205,
            batch_norm__206,
            relu_31,
            assign_4,
            pool2d_5,
            conv2d_26,
            reshape_10,
            relu_32,
            conv2d_27,
            reshape_11,
            sigmoid_5,
            multiply_5,
            conv2d_28,
            batch_norm__207,
            batch_norm__208,
            batch_norm__209,
            batch_norm__210,
            batch_norm__211,
            relu_33,
            conv2d_29,
            batch_norm__212,
            batch_norm__213,
            batch_norm__214,
            batch_norm__215,
            batch_norm__216,
            relu_34,
            depthwise_conv2d_22,
            batch_norm__217,
            batch_norm__218,
            batch_norm__219,
            batch_norm__220,
            batch_norm__221,
            batch_norm__222,
            depthwise_conv2d_23,
            batch_norm__223,
            batch_norm__224,
            batch_norm__225,
            batch_norm__226,
            batch_norm__227,
            batch_norm__228,
            add_5,
            depthwise_conv2d_24,
            batch_norm__229,
            batch_norm__230,
            batch_norm__231,
            batch_norm__232,
            batch_norm__233,
            batch_norm__234,
            relu_35,
            conv2d_30,
            batch_norm__235,
            batch_norm__236,
            batch_norm__237,
            batch_norm__238,
            batch_norm__239,
            relu_36,
            depthwise_conv2d_25,
            batch_norm__240,
            batch_norm__241,
            batch_norm__242,
            batch_norm__243,
            batch_norm__244,
            batch_norm__245,
            depthwise_conv2d_26,
            batch_norm__246,
            batch_norm__247,
            batch_norm__248,
            batch_norm__249,
            batch_norm__250,
            batch_norm__251,
            add_6,
            depthwise_conv2d_27,
            batch_norm__252,
            batch_norm__253,
            batch_norm__254,
            batch_norm__255,
            batch_norm__256,
            batch_norm__257,
            relu_37,
            conv2d_31,
            batch_norm__258,
            batch_norm__259,
            batch_norm__260,
            batch_norm__261,
            batch_norm__262,
            relu_38,
            add_7,
            assign_5,
            pool2d_6,
            conv2d_32,
            reshape_12,
            full_0,
            dropout_0,
            dropout_1,
            flatten_0,
            batch_norm__263,
            batch_norm__264,
            batch_norm__265,
            batch_norm__266,
            batch_norm__267,
            flatten_1,
            batch_norm__268,
            matmul_0,
        )
