...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.conv2d: (-1x40x56x56xf32) <- (-1x3x224x224xf32, 40x3x4x4xf32)
        ...
        # pd_op.split: ([-1x10x56x56xf32, -1x30x56x56xf32]) <- (-1x40x56x56xf32, 2xi64, 1xi32)
        split_0 = paddle._C_ops.split(batch_norm__0, full_int_array_0, full_0)

        # builtin.split: (-1x10x56x56xf32, -1x30x56x56xf32) <- ([-1x10x56x56xf32, -1x30x56x56xf32])
        (
            split_1,
            split_2,
        ) = split_0

        # pd_op.conv2d: (-1x10x56x56xf32) <- (-1x10x56x56xf32, 10x10x3x3xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            split_1, parameter_108, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # builtin.combine: ([-1x10x56x56xf32, -1x30x56x56xf32]) <- (-1x10x56x56xf32, -1x30x56x56xf32)
        combine_0 = [conv2d_1, split_2]

        # pd_op.concat: (-1x40x56x56xf32) <- ([-1x10x56x56xf32, -1x30x56x56xf32], 1xi32)
        concat_0 = paddle._C_ops.concat(combine_0, full_0)

        # pd_op.conv2d: (-1x80x56x56xf32) <- (-1x40x56x56xf32, 80x40x1x1xf32)
        ...
        # pd_op.gelu: (-1x80x56x56xf32) <- (-1x80x56x56xf32)
        gelu_0 = paddle._C_ops.gelu(batch_norm__6, False)

        # pd_op.conv2d: (-1x40x56x56xf32) <- (-1x80x56x56xf32, 40x80x1x1xf32)
        conv2d_3 = paddle._C_ops.conv2d(
            gelu_0, parameter_102, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (-1x40x56x56xf32) <- (-1x40x56x56xf32, -1x40x56x56xf32)
        add_1 = paddle._C_ops.add(batch_norm__0, conv2d_3)

        # pd_op.conv2d: (-1x80x28x28xf32) <- (-1x40x56x56xf32, 80x40x2x2xf32)
        conv2d_4 = paddle._C_ops.conv2d(
            add_1, parameter_101, [2, 2], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.batch_norm_: (-1x80x28x28xf32, 80xf32, 80xf32, 80xf32, 80xf32, -1xui8) <- (-1x80x28x28xf32, 80xf32, 80xf32, 80xf32, 80xf32)
        (
            batch_norm__12,
            batch_norm__13,
            batch_norm__14,
        ...
        # pd_op.split: ([-1x20x28x28xf32, -1x60x28x28xf32]) <- (-1x80x28x28xf32, 2xi64, 1xi32)
        split_3 = paddle._C_ops.split(batch_norm__12, full_int_array_1, full_0)

        # builtin.split: (-1x20x28x28xf32, -1x60x28x28xf32) <- ([-1x20x28x28xf32, -1x60x28x28xf32])
        (
            split_4,
            split_5,
        ) = split_3

        # pd_op.conv2d: (-1x20x28x28xf32) <- (-1x20x28x28xf32, 20x20x3x3xf32)
        conv2d_5 = paddle._C_ops.conv2d(
            split_4, parameter_96, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # builtin.combine: ([-1x20x28x28xf32, -1x60x28x28xf32]) <- (-1x20x28x28xf32, -1x60x28x28xf32)
        combine_1 = [conv2d_5, split_5]

        # pd_op.concat: (-1x80x28x28xf32) <- ([-1x20x28x28xf32, -1x60x28x28xf32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_1, full_0)

        # pd_op.conv2d: (-1x160x28x28xf32) <- (-1x80x28x28xf32, 160x80x1x1xf32)
        ...
        # pd_op.gelu: (-1x160x28x28xf32) <- (-1x160x28x28xf32)
        gelu_1 = paddle._C_ops.gelu(batch_norm__18, False)

        # pd_op.conv2d: (-1x80x28x28xf32) <- (-1x160x28x28xf32, 80x160x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            gelu_1, parameter_90, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (-1x80x28x28xf32) <- (-1x80x28x28xf32, -1x80x28x28xf32)
        ...
        # pd_op.conv2d: (-1x160x14x14xf32) <- (-1x80x28x28xf32, 160x80x2x2xf32)
        ...
        # pd_op.split: ([-1x40x14x14xf32, -1x120x14x14xf32]) <- (-1x160x14x14xf32, 2xi64, 1xi32)
        split_9 = paddle._C_ops.split(batch_norm__30, full_int_array_2, full_0)

        # builtin.split: (-1x40x14x14xf32, -1x120x14x14xf32) <- ([-1x40x14x14xf32, -1x120x14x14xf32])
        (
            split_10,
            split_11,
        ) = split_9

        # pd_op.conv2d: (-1x40x14x14xf32) <- (-1x40x14x14xf32, 40x40x3x3xf32)
        conv2d_12 = paddle._C_ops.conv2d(
            split_10, parameter_77, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # builtin.combine: ([-1x40x14x14xf32, -1x120x14x14xf32]) <- (-1x40x14x14xf32, -1x120x14x14xf32)
        combine_3 = [conv2d_12, split_11]

        # pd_op.concat: (-1x160x14x14xf32) <- ([-1x40x14x14xf32, -1x120x14x14xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_3, full_0)

        # pd_op.conv2d: (-1x320x14x14xf32) <- (-1x160x14x14xf32, 320x160x1x1xf32)
        ...
        # pd_op.gelu: (-1x320x14x14xf32) <- (-1x320x14x14xf32)
        gelu_3 = paddle._C_ops.gelu(batch_norm__36, False)

        # pd_op.conv2d: (-1x160x14x14xf32) <- (-1x320x14x14xf32, 160x320x1x1xf32)
        conv2d_14 = paddle._C_ops.conv2d(
            gelu_3, parameter_71, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (-1x160x14x14xf32) <- (-1x160x14x14xf32, -1x160x14x14xf32)
        ...
        # pd_op.conv2d: (-1x320x7x7xf32) <- (-1x160x14x14xf32, 320x160x2x2xf32)
        ...
        # pd_op.split: ([-1x80x7x7xf32, -1x240x7x7xf32]) <- (-1x320x7x7xf32, 2xi64, 1xi32)
        split_33 = paddle._C_ops.split(batch_norm__84, full_int_array_3, full_0)

        # builtin.split: (-1x80x7x7xf32, -1x240x7x7xf32) <- ([-1x80x7x7xf32, -1x240x7x7xf32])
        (
            split_34,
            split_35,
        ) = split_33

        # pd_op.conv2d: (-1x80x7x7xf32) <- (-1x80x7x7xf32, 80x80x3x3xf32)
        conv2d_37 = paddle._C_ops.conv2d(
            split_34, parameter_16, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # builtin.combine: ([-1x80x7x7xf32, -1x240x7x7xf32]) <- (-1x80x7x7xf32, -1x240x7x7xf32)
        combine_11 = [conv2d_37, split_35]

        # pd_op.concat: (-1x320x7x7xf32) <- ([-1x80x7x7xf32, -1x240x7x7xf32], 1xi32)
        concat_11 = paddle._C_ops.concat(combine_11, full_0)

        # pd_op.conv2d: (-1x640x7x7xf32) <- (-1x320x7x7xf32, 640x320x1x1xf32)
        ...
        # pd_op.gelu: (-1x640x7x7xf32) <- (-1x640x7x7xf32)
        gelu_11 = paddle._C_ops.gelu(batch_norm__90, False)

        # pd_op.conv2d: (-1x320x7x7xf32) <- (-1x640x7x7xf32, 320x640x1x1xf32)
        ...
        # pd_op.pool2d: (-1x320x1x1xf32) <- (-1x320x7x7xf32, 2xi64)
