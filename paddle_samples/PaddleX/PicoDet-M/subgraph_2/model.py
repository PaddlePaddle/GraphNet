...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.unsqueeze: (4x1x3598xf32) <- (4x3598xf32, 1xi64)
        ...
        # pd_op.greater_than: (4x1x3598xb) <- (4x1x3598xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(unsqueeze_0, full_0)
        del full_0, unsqueeze_0

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_1 = [1, 3, 1]

        # pd_op.tile: (4x3x3598xb) <- (4x1x3598xb, 3xi64)
        ...
        # pd_op.argmax: (4x3598xi64) <- (4x3x3598xf32, 1xi64)
        ...
        # pd_op.one_hot: (4x3598x3xf32) <- (4x3598xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            argmax_0 % paddle.cast(full_2, argmax_0.dtype), full_2
        )
        del argmax_0, full_2

        # pd_op.transpose: (4x3x3598xf32) <- (4x3598x3xf32)
        transpose_0 = paddle._C_ops.transpose(one_hot_0, [0, 2, 1])
        del one_hot_0

        # pd_op.where: (4x3x3598xf32) <- (4x3x3598xb, 4x3x3598xf32, 4x3x3598xf32)
        ...
        # pd_op.sum: (4x3598xf32) <- (4x3x3598xf32, 1xi64)
        ...
        # pd_op.add: (4x3598xi64) <- (4x3598xi64, 4x1xi64)
        add_0 = paddle._C_ops.add(argmax_1, cast_0)
        del argmax_1, cast_0

        # pd_op.flatten: (12xi32) <- (4x3x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_4, 0, 2)
        del data_4

        # pd_op.flatten: (14392xi64) <- (4x3598xi64)
        ...
        # pd_op.gather: (14392xi32) <- (12xi32, 14392xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_3 = [4, 3598]

        # pd_op.reshape: (4x3598xi32) <- (14392xi32, 2xi64)
        ...
        # pd_op.greater_than: (4x3598xb) <- (4x3598xf32, xf32)
        ...
        # pd_op.full_like: (4x3598xi32) <- (4x3598xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_6

        # pd_op.where: (4x3598xi32) <- (4x3598xb, 4x3598xi32, 4x3598xi32)
        ...
        # pd_op.one_hot: (4x3598x5xf32) <- (4x3598xi32, 1xi32)
        ...
        # pd_op.index_select: (4x3598x4xf32) <- (4x3598x5xf32, 4xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_1, assign_value__0, -1)
        del assign_value__0, one_hot_1

        # pd_op.multiply: (4x3x3598xf32) <- (4x3x3598xf32, 4x3x3598xf32)
        ...
        # pd_op.max: (4x3x1xf32) <- (4x3x3598xf32, 1xi64)
