...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.unsqueeze: (1x9382x4xf32) <- (9382x4xf32, 1xi64)
        ...
        # pd_op.slice: (1x9382x2xf32) <- (1x9382x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (6x9382x2xf32) <- (6x1x2xf32, 1x9382x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (6x9382x2xf32) <- (6x1x2xf32, 1x9382x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (6x9382x2xf32) <- (6x9382x2xf32, 6x9382x2xf32)
        ...
        # pd_op.clip: (6x9382x2xf32) <- (6x9382x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_0, full_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_5 = [-1]

        # pd_op.prod: (6x9382xf32) <- (6x9382x2xf32, 1xi64)
        ...
        # pd_op.subtract: (1x9382x2xf32) <- (1x9382x2xf32, 1x9382x2xf32)
        subtract_2 = paddle._C_ops.subtract(slice_3, slice_2)

        # pd_op.clip: (1x9382x2xf32) <- (1x9382x2xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(subtract_2, full_0, full_1)

        # pd_op.prod: (1x9382xf32) <- (1x9382x2xf32, 1xi64)
        prod_2 = paddle._C_ops.prod(clip_2, full_int_array_5, False, False)

        # pd_op.add: (6x9382xf32) <- (6x1xf32, 1x9382xf32)
        add_0 = paddle._C_ops.add(prod_1, prod_2)

        # pd_op.subtract: (6x9382xf32) <- (6x9382xf32, 6x9382xf32)
        ...
        # pd_op.scale: (6x9382xf32) <- (6x9382xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(subtract_3, full_2, float("1e-10"), True)

        # pd_op.divide: (6x9382xf32) <- (6x9382xf32, 6x9382xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_0)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_6 = [2, -1, 9382]

        # pd_op.reshape: (2x3x9382xf32) <- (6x9382xf32, 3xi64)
        ...
        # pd_op.unsqueeze: (1x9382x2xf32) <- (9382x2xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(stack_0, full_int_array_2)

        # pd_op.subtract: (6x9382x2xf32) <- (6x1x2xf32, 1x9382x2xf32)
        subtract_4 = paddle._C_ops.subtract(unsqueeze_2, unsqueeze_3)

        # pd_op.p_norm: (6x9382xf32) <- (6x9382x2xf32)
