import paddle


class GraphModule(paddle.nn.Layer):
    def __init__(self):
        super().__init__()

    def forward(
        self,
        parameter_0,
        parameter_1,
        parameter_2,
        parameter_3,
        parameter_4,
        parameter_5,
        data_0,
        data_1,
        data_2,
        data_3,
        data_4,
        data_5,
        data_6,
        data_7,
        data_8,
        data_9,
    ):
        # pd_op.conv2d: (4x256x256x192xf32) <- (4x256x256x192xf32, 256x256x3x3xf32)
        conv2d_0 = paddle._C_ops.conv2d(
            data_5, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_0 = [1, -1, 1, 1]

        # pd_op.reshape: (1x256x1x1xf32) <- (256xf32, 4xi64)
        reshape_0 = paddle._C_ops.reshape(parameter_4, full_int_array_0)

        # pd_op.assign: (1x256x1x1xf32) <- (1x256x1x1xf32)
        assign_3 = reshape_0

        # pd_op.assign: (1x256x1x1xf32) <- (1x256x1x1xf32)
        assign_2 = reshape_0

        # pd_op.assign: (1x256x1x1xf32) <- (1x256x1x1xf32)
        assign_1 = reshape_0

        # pd_op.assign: (1x256x1x1xf32) <- (1x256x1x1xf32)
        assign_0 = reshape_0

        # pd_op.add: (4x256x256x192xf32) <- (4x256x256x192xf32, 1x256x1x1xf32)
        add_10 = paddle._C_ops.add(conv2d_0, reshape_0)

        # pd_op.relu: (4x256x256x192xf32) <- (4x256x256x192xf32)
        relu_0 = paddle._C_ops.relu(add_10)

        # pd_op.conv2d: (4x256x128x96xf32) <- (4x256x128x96xf32, 256x256x3x3xf32)
        conv2d_1 = paddle._C_ops.conv2d(
            data_6, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x256x128x96xf32) <- (4x256x128x96xf32, 1x256x1x1xf32)
        add_11 = paddle._C_ops.add(conv2d_1, reshape_0)

        # pd_op.relu: (4x256x128x96xf32) <- (4x256x128x96xf32)
        relu_1 = paddle._C_ops.relu(add_11)

        # pd_op.conv2d: (4x256x64x48xf32) <- (4x256x64x48xf32, 256x256x3x3xf32)
        conv2d_2 = paddle._C_ops.conv2d(
            data_7, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x256x64x48xf32) <- (4x256x64x48xf32, 1x256x1x1xf32)
        add_12 = paddle._C_ops.add(conv2d_2, reshape_0)

        # pd_op.relu: (4x256x64x48xf32) <- (4x256x64x48xf32)
        relu_2 = paddle._C_ops.relu(add_12)

        # pd_op.conv2d: (4x256x32x24xf32) <- (4x256x32x24xf32, 256x256x3x3xf32)
        conv2d_3 = paddle._C_ops.conv2d(
            data_8, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x256x32x24xf32) <- (4x256x32x24xf32, 1x256x1x1xf32)
        add_13 = paddle._C_ops.add(conv2d_3, reshape_0)

        # pd_op.relu: (4x256x32x24xf32) <- (4x256x32x24xf32)
        relu_3 = paddle._C_ops.relu(add_13)

        # pd_op.conv2d: (4x256x16x12xf32) <- (4x256x16x12xf32, 256x256x3x3xf32)
        conv2d_4 = paddle._C_ops.conv2d(
            data_9, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x256x16x12xf32) <- (4x256x16x12xf32, 1x256x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_4, reshape_0)

        # pd_op.relu: (4x256x16x12xf32) <- (4x256x16x12xf32)
        relu_4 = paddle._C_ops.relu(add_14)

        # pd_op.conv2d: (4x3x256x192xf32) <- (4x256x256x192xf32, 3x256x1x1xf32)
        conv2d_5 = paddle._C_ops.conv2d(
            relu_0, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x3x1x1xf32) <- (3xf32, 4xi64)
        reshape_1 = paddle._C_ops.reshape(parameter_2, full_int_array_0)

        # pd_op.assign: (1x3x1x1xf32) <- (1x3x1x1xf32)
        assign_10 = reshape_1

        # pd_op.assign: (1x3x1x1xf32) <- (1x3x1x1xf32)
        assign_8 = reshape_1

        # pd_op.assign: (1x3x1x1xf32) <- (1x3x1x1xf32)
        assign_6 = reshape_1

        # pd_op.assign: (1x3x1x1xf32) <- (1x3x1x1xf32)
        assign_4 = reshape_1

        # pd_op.add: (4x3x256x192xf32) <- (4x3x256x192xf32, 1x3x1x1xf32)
        add_0 = paddle._C_ops.add(conv2d_5, reshape_1)

        # pd_op.conv2d: (4x12x256x192xf32) <- (4x256x256x192xf32, 12x256x1x1xf32)
        conv2d_6 = paddle._C_ops.conv2d(
            relu_0, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.reshape: (1x12x1x1xf32) <- (12xf32, 4xi64)
        reshape_2 = paddle._C_ops.reshape(parameter_0, full_int_array_0)

        # pd_op.assign: (1x12x1x1xf32) <- (1x12x1x1xf32)
        assign_11 = reshape_2

        # pd_op.assign: (1x12x1x1xf32) <- (1x12x1x1xf32)
        assign_9 = reshape_2

        # pd_op.assign: (1x12x1x1xf32) <- (1x12x1x1xf32)
        assign_7 = reshape_2

        # pd_op.assign: (1x12x1x1xf32) <- (1x12x1x1xf32)
        assign_5 = reshape_2

        # pd_op.add: (4x12x256x192xf32) <- (4x12x256x192xf32, 1x12x1x1xf32)
        add_5 = paddle._C_ops.add(conv2d_6, reshape_2)

        # pd_op.conv2d: (4x3x128x96xf32) <- (4x256x128x96xf32, 3x256x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            relu_1, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x3x128x96xf32) <- (4x3x128x96xf32, 1x3x1x1xf32)
        add_1 = paddle._C_ops.add(conv2d_7, reshape_1)

        # pd_op.conv2d: (4x12x128x96xf32) <- (4x256x128x96xf32, 12x256x1x1xf32)
        conv2d_8 = paddle._C_ops.conv2d(
            relu_1, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x12x128x96xf32) <- (4x12x128x96xf32, 1x12x1x1xf32)
        add_6 = paddle._C_ops.add(conv2d_8, reshape_2)

        # pd_op.conv2d: (4x3x64x48xf32) <- (4x256x64x48xf32, 3x256x1x1xf32)
        conv2d_9 = paddle._C_ops.conv2d(
            relu_2, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x3x64x48xf32) <- (4x3x64x48xf32, 1x3x1x1xf32)
        add_2 = paddle._C_ops.add(conv2d_9, reshape_1)

        # pd_op.conv2d: (4x12x64x48xf32) <- (4x256x64x48xf32, 12x256x1x1xf32)
        conv2d_10 = paddle._C_ops.conv2d(
            relu_2, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x12x64x48xf32) <- (4x12x64x48xf32, 1x12x1x1xf32)
        add_7 = paddle._C_ops.add(conv2d_10, reshape_2)

        # pd_op.conv2d: (4x3x32x24xf32) <- (4x256x32x24xf32, 3x256x1x1xf32)
        conv2d_11 = paddle._C_ops.conv2d(
            relu_3, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x3x32x24xf32) <- (4x3x32x24xf32, 1x3x1x1xf32)
        add_3 = paddle._C_ops.add(conv2d_11, reshape_1)

        # pd_op.conv2d: (4x12x32x24xf32) <- (4x256x32x24xf32, 12x256x1x1xf32)
        conv2d_12 = paddle._C_ops.conv2d(
            relu_3, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x12x32x24xf32) <- (4x12x32x24xf32, 1x12x1x1xf32)
        add_8 = paddle._C_ops.add(conv2d_12, reshape_2)

        # pd_op.conv2d: (4x3x16x12xf32) <- (4x256x16x12xf32, 3x256x1x1xf32)
        conv2d_13 = paddle._C_ops.conv2d(
            relu_4, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x3x16x12xf32) <- (4x3x16x12xf32, 1x3x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_13, reshape_1)

        # pd_op.conv2d: (4x12x16x12xf32) <- (4x256x16x12xf32, 12x256x1x1xf32)
        conv2d_14 = paddle._C_ops.conv2d(
            relu_4, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (4x12x16x12xf32) <- (4x12x16x12xf32, 1x12x1x1xf32)
        add_9 = paddle._C_ops.add(conv2d_14, reshape_2)

        # pd_op.full: (1xf64) <- ()
        full_0 = paddle._C_ops.full(
            [1], float("0"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.full: (1xf64) <- ()
        full_1 = paddle._C_ops.full(
            [1], float("768"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.full: (1xf64) <- ()
        full_2 = paddle._C_ops.full(
            [1], float("4"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (192xf32) <- (1xf64, 1xf64, 1xf64)
        arange_0 = paddle.arange(full_0, full_1, full_2, dtype="float32")

        # pd_op.full: (1xf64) <- ()
        full_3 = paddle._C_ops.full(
            [1], float("1024"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (256xf32) <- (1xf64, 1xf64, 1xf64)
        arange_1 = paddle.arange(full_0, full_3, full_2, dtype="float32")

        # builtin.combine: ([256xf32, 192xf32]) <- (256xf32, 192xf32)
        combine_0 = [arange_1, arange_0]

        # pd_op.meshgrid: ([256x192xf32, 256x192xf32]) <- ([256xf32, 192xf32])
        meshgrid_0 = paddle._C_ops.meshgrid(combine_0)

        # builtin.split: (256x192xf32, 256x192xf32) <- ([256x192xf32, 256x192xf32])
        (
            split_0,
            split_1,
        ) = meshgrid_0

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_1 = [-1]

        # pd_op.reshape: (49152xf32) <- (256x192xf32, 1xi64)
        reshape_8 = paddle._C_ops.reshape(split_1, full_int_array_1)

        # pd_op.reshape: (49152xf32) <- (256x192xf32, 1xi64)
        reshape_9 = paddle._C_ops.reshape(split_0, full_int_array_1)

        # builtin.combine: ([49152xf32, 49152xf32, 49152xf32, 49152xf32]) <- (49152xf32, 49152xf32, 49152xf32, 49152xf32)
        combine_1 = [reshape_8, reshape_9, reshape_8, reshape_9]

        # pd_op.stack: (49152x4xf32) <- ([49152xf32, 49152xf32, 49152xf32, 49152xf32])
        stack_0 = paddle._C_ops.stack(combine_1, 1)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [-1, 1, 4]

        # pd_op.reshape: (49152x1x4xf32) <- (49152x4xf32, 3xi64)
        reshape_10 = paddle._C_ops.reshape(stack_0, full_int_array_2)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [1, -1, 4]

        # pd_op.reshape: (1x3x4xf32) <- (3x4xf32, 3xi64)
        reshape_11 = paddle._C_ops.reshape(data_0, full_int_array_3)

        # pd_op.add: (49152x3x4xf32) <- (49152x1x4xf32, 1x3x4xf32)
        add_15 = paddle._C_ops.add(reshape_10, reshape_11)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_4 = [-1, 4]

        # pd_op.reshape: (147456x4xf32) <- (49152x3x4xf32, 2xi64)
        reshape_3 = paddle._C_ops.reshape(add_15, full_int_array_4)

        # pd_op.full: (1xf64) <- ()
        full_4 = paddle._C_ops.full(
            [1], float("8"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (96xf32) <- (1xf64, 1xf64, 1xf64)
        arange_2 = paddle.arange(full_0, full_1, full_4, dtype="float32")

        # pd_op.arange: (128xf32) <- (1xf64, 1xf64, 1xf64)
        arange_3 = paddle.arange(full_0, full_3, full_4, dtype="float32")

        # builtin.combine: ([128xf32, 96xf32]) <- (128xf32, 96xf32)
        combine_2 = [arange_3, arange_2]

        # pd_op.meshgrid: ([128x96xf32, 128x96xf32]) <- ([128xf32, 96xf32])
        meshgrid_1 = paddle._C_ops.meshgrid(combine_2)

        # builtin.split: (128x96xf32, 128x96xf32) <- ([128x96xf32, 128x96xf32])
        (
            split_2,
            split_3,
        ) = meshgrid_1

        # pd_op.reshape: (12288xf32) <- (128x96xf32, 1xi64)
        reshape_12 = paddle._C_ops.reshape(split_3, full_int_array_1)

        # pd_op.reshape: (12288xf32) <- (128x96xf32, 1xi64)
        reshape_13 = paddle._C_ops.reshape(split_2, full_int_array_1)

        # builtin.combine: ([12288xf32, 12288xf32, 12288xf32, 12288xf32]) <- (12288xf32, 12288xf32, 12288xf32, 12288xf32)
        combine_3 = [reshape_12, reshape_13, reshape_12, reshape_13]

        # pd_op.stack: (12288x4xf32) <- ([12288xf32, 12288xf32, 12288xf32, 12288xf32])
        stack_1 = paddle._C_ops.stack(combine_3, 1)

        # pd_op.reshape: (12288x1x4xf32) <- (12288x4xf32, 3xi64)
        reshape_14 = paddle._C_ops.reshape(stack_1, full_int_array_2)

        # pd_op.reshape: (1x3x4xf32) <- (3x4xf32, 3xi64)
        reshape_15 = paddle._C_ops.reshape(data_1, full_int_array_3)

        # pd_op.add: (12288x3x4xf32) <- (12288x1x4xf32, 1x3x4xf32)
        add_16 = paddle._C_ops.add(reshape_14, reshape_15)

        # pd_op.reshape: (36864x4xf32) <- (12288x3x4xf32, 2xi64)
        reshape_4 = paddle._C_ops.reshape(add_16, full_int_array_4)

        # pd_op.full: (1xf64) <- ()
        full_5 = paddle._C_ops.full(
            [1], float("16"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (48xf32) <- (1xf64, 1xf64, 1xf64)
        arange_4 = paddle.arange(full_0, full_1, full_5, dtype="float32")

        # pd_op.arange: (64xf32) <- (1xf64, 1xf64, 1xf64)
        arange_5 = paddle.arange(full_0, full_3, full_5, dtype="float32")

        # builtin.combine: ([64xf32, 48xf32]) <- (64xf32, 48xf32)
        combine_4 = [arange_5, arange_4]

        # pd_op.meshgrid: ([64x48xf32, 64x48xf32]) <- ([64xf32, 48xf32])
        meshgrid_2 = paddle._C_ops.meshgrid(combine_4)

        # builtin.split: (64x48xf32, 64x48xf32) <- ([64x48xf32, 64x48xf32])
        (
            split_4,
            split_5,
        ) = meshgrid_2

        # pd_op.reshape: (3072xf32) <- (64x48xf32, 1xi64)
        reshape_16 = paddle._C_ops.reshape(split_5, full_int_array_1)

        # pd_op.reshape: (3072xf32) <- (64x48xf32, 1xi64)
        reshape_17 = paddle._C_ops.reshape(split_4, full_int_array_1)

        # builtin.combine: ([3072xf32, 3072xf32, 3072xf32, 3072xf32]) <- (3072xf32, 3072xf32, 3072xf32, 3072xf32)
        combine_5 = [reshape_16, reshape_17, reshape_16, reshape_17]

        # pd_op.stack: (3072x4xf32) <- ([3072xf32, 3072xf32, 3072xf32, 3072xf32])
        stack_2 = paddle._C_ops.stack(combine_5, 1)

        # pd_op.reshape: (3072x1x4xf32) <- (3072x4xf32, 3xi64)
        reshape_18 = paddle._C_ops.reshape(stack_2, full_int_array_2)

        # pd_op.reshape: (1x3x4xf32) <- (3x4xf32, 3xi64)
        reshape_19 = paddle._C_ops.reshape(data_2, full_int_array_3)

        # pd_op.add: (3072x3x4xf32) <- (3072x1x4xf32, 1x3x4xf32)
        add_17 = paddle._C_ops.add(reshape_18, reshape_19)

        # pd_op.reshape: (9216x4xf32) <- (3072x3x4xf32, 2xi64)
        reshape_5 = paddle._C_ops.reshape(add_17, full_int_array_4)

        # pd_op.full: (1xf64) <- ()
        full_6 = paddle._C_ops.full(
            [1], float("32"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (24xf32) <- (1xf64, 1xf64, 1xf64)
        arange_6 = paddle.arange(full_0, full_1, full_6, dtype="float32")

        # pd_op.arange: (32xf32) <- (1xf64, 1xf64, 1xf64)
        arange_7 = paddle.arange(full_0, full_3, full_6, dtype="float32")

        # builtin.combine: ([32xf32, 24xf32]) <- (32xf32, 24xf32)
        combine_6 = [arange_7, arange_6]

        # pd_op.meshgrid: ([32x24xf32, 32x24xf32]) <- ([32xf32, 24xf32])
        meshgrid_3 = paddle._C_ops.meshgrid(combine_6)

        # builtin.split: (32x24xf32, 32x24xf32) <- ([32x24xf32, 32x24xf32])
        (
            split_6,
            split_7,
        ) = meshgrid_3

        # pd_op.reshape: (768xf32) <- (32x24xf32, 1xi64)
        reshape_20 = paddle._C_ops.reshape(split_7, full_int_array_1)

        # pd_op.reshape: (768xf32) <- (32x24xf32, 1xi64)
        reshape_21 = paddle._C_ops.reshape(split_6, full_int_array_1)

        # builtin.combine: ([768xf32, 768xf32, 768xf32, 768xf32]) <- (768xf32, 768xf32, 768xf32, 768xf32)
        combine_7 = [reshape_20, reshape_21, reshape_20, reshape_21]

        # pd_op.stack: (768x4xf32) <- ([768xf32, 768xf32, 768xf32, 768xf32])
        stack_3 = paddle._C_ops.stack(combine_7, 1)

        # pd_op.reshape: (768x1x4xf32) <- (768x4xf32, 3xi64)
        reshape_22 = paddle._C_ops.reshape(stack_3, full_int_array_2)

        # pd_op.reshape: (1x3x4xf32) <- (3x4xf32, 3xi64)
        reshape_23 = paddle._C_ops.reshape(data_3, full_int_array_3)

        # pd_op.add: (768x3x4xf32) <- (768x1x4xf32, 1x3x4xf32)
        add_18 = paddle._C_ops.add(reshape_22, reshape_23)

        # pd_op.reshape: (2304x4xf32) <- (768x3x4xf32, 2xi64)
        reshape_6 = paddle._C_ops.reshape(add_18, full_int_array_4)

        # pd_op.full: (1xf64) <- ()
        full_7 = paddle._C_ops.full(
            [1], float("64"), paddle.float64, paddle.core.CPUPlace()
        )

        # pd_op.arange: (12xf32) <- (1xf64, 1xf64, 1xf64)
        arange_8 = paddle.arange(full_0, full_1, full_7, dtype="float32")

        # pd_op.arange: (16xf32) <- (1xf64, 1xf64, 1xf64)
        arange_9 = paddle.arange(full_0, full_3, full_7, dtype="float32")

        # builtin.combine: ([16xf32, 12xf32]) <- (16xf32, 12xf32)
        combine_8 = [arange_9, arange_8]

        # pd_op.meshgrid: ([16x12xf32, 16x12xf32]) <- ([16xf32, 12xf32])
        meshgrid_4 = paddle._C_ops.meshgrid(combine_8)

        # builtin.split: (16x12xf32, 16x12xf32) <- ([16x12xf32, 16x12xf32])
        (
            split_8,
            split_9,
        ) = meshgrid_4

        # pd_op.reshape: (192xf32) <- (16x12xf32, 1xi64)
        reshape_24 = paddle._C_ops.reshape(split_9, full_int_array_1)

        # pd_op.reshape: (192xf32) <- (16x12xf32, 1xi64)
        reshape_25 = paddle._C_ops.reshape(split_8, full_int_array_1)

        # builtin.combine: ([192xf32, 192xf32, 192xf32, 192xf32]) <- (192xf32, 192xf32, 192xf32, 192xf32)
        combine_9 = [reshape_24, reshape_25, reshape_24, reshape_25]

        # pd_op.stack: (192x4xf32) <- ([192xf32, 192xf32, 192xf32, 192xf32])
        stack_4 = paddle._C_ops.stack(combine_9, 1)

        # pd_op.reshape: (192x1x4xf32) <- (192x4xf32, 3xi64)
        reshape_26 = paddle._C_ops.reshape(stack_4, full_int_array_2)

        # pd_op.reshape: (1x3x4xf32) <- (3x4xf32, 3xi64)
        reshape_27 = paddle._C_ops.reshape(data_4, full_int_array_3)

        # pd_op.add: (192x3x4xf32) <- (192x1x4xf32, 1x3x4xf32)
        add_19 = paddle._C_ops.add(reshape_26, reshape_27)

        # pd_op.reshape: (576x4xf32) <- (192x3x4xf32, 2xi64)
        reshape_7 = paddle._C_ops.reshape(add_19, full_int_array_4)
        return (
            conv2d_0,
            reshape_0,
            relu_0,
            conv2d_1,
            assign_0,
            relu_1,
            conv2d_2,
            assign_1,
            relu_2,
            conv2d_3,
            assign_2,
            relu_3,
            conv2d_4,
            assign_3,
            relu_4,
            conv2d_5,
            reshape_1,
            conv2d_6,
            reshape_2,
            conv2d_7,
            assign_4,
            conv2d_8,
            assign_5,
            conv2d_9,
            assign_6,
            conv2d_10,
            assign_7,
            conv2d_11,
            assign_8,
            conv2d_12,
            assign_9,
            conv2d_13,
            assign_10,
            conv2d_14,
            assign_11,
            add_0,
            add_1,
            add_2,
            add_3,
            add_4,
            add_5,
            add_6,
            add_7,
            add_8,
            add_9,
            reshape_3,
            reshape_4,
            reshape_5,
            reshape_6,
            reshape_7,
        )
