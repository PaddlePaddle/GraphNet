...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3):
        ...
        # pd_op.split: ([6x1x198x4xf32, 6x1x300x4xf32]) <- (6x1x498x4xf32, 2xi64, 1xi32)
        split_4 = paddle._C_ops.split(data_1, full_int_array_1, full_0)

        # builtin.split: (6x1x198x4xf32, 6x1x300x4xf32) <- ([6x1x198x4xf32, 6x1x300x4xf32])
        (
            split_2,
            split_0,
        ) = split_4

        # pd_op.split: ([6x1x198x11xf32, 6x1x300x11xf32]) <- (6x1x498x11xf32, 2xi64, 1xi32)
        split_5 = paddle._C_ops.split(data_3, full_int_array_1, full_0)

        # builtin.split: (6x1x198x11xf32, 6x1x300x11xf32) <- ([6x1x198x11xf32, 6x1x300x11xf32])
        (
            split_3,
            split_1,
        ) = split_5
        ...
        # pd_op.unsqueeze: (1x1x300x11xf32) <- (1x300x11xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(data_2, full_int_array_0)

        # builtin.combine: ([1x1x300x11xf32, 6x1x300x11xf32]) <- (1x1x300x11xf32, 6x1x300x11xf32)
        combine_1 = [unsqueeze_1, split_1]

        # pd_op.concat: (7x1x300x11xf32) <- ([1x1x300x11xf32, 6x1x300x11xf32], 1xi32)
