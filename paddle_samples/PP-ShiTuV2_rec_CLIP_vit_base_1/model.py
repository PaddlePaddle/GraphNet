...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        transpose_2 = paddle._C_ops.transpose(slice_25, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_5 = paddle._C_ops.transpose(slice_26, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_8 = paddle._C_ops.transpose(slice_27, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_11 = paddle._C_ops.transpose(slice_28, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_14 = paddle._C_ops.transpose(slice_29, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_17 = paddle._C_ops.transpose(slice_30, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_20 = paddle._C_ops.transpose(slice_31, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_26 = paddle._C_ops.transpose(slice_33, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_29 = paddle._C_ops.transpose(slice_34, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_32 = paddle._C_ops.transpose(slice_35, [0, 1, 3, 2])
        ...
        # pd_op.transpose: (32x12x64x197xf32) <- (32x12x197x64xf32)
        transpose_35 = paddle._C_ops.transpose(slice_36, [0, 1, 3, 2])
        ...
        matmul_48 = paddle._C_ops.matmul(layer_norm_75, data_2, False, False)

        # pd_op.mean: (32x512xf32) <- (32x196x512xf32)
        mean_0 = paddle._C_ops.mean(matmul_48, [1], False)

        # pd_op.matmul: (32x512xf32) <- (32x512xf32, 512x512xf32)
        matmul_49 = paddle._C_ops.matmul(mean_0, parameter_1, False, False)

        # pd_op.add: (32x512xf32) <- (32x512xf32, 512xf32)
        add_74 = paddle._C_ops.add(matmul_49, parameter_0)

        # pd_op.square: (32x512xf32) <- (32x512xf32)
        square_0 = paddle._C_ops.square(add_74)

        # pd_op.sum: (32x1xf32) <- (32x512xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(square_0, full_int_array_2, None, True)

        # pd_op.sqrt: (32x1xf32) <- (32x1xf32)
        sqrt_0 = paddle._C_ops.sqrt(sum_0)

        # pd_op.divide: (32x512xf32) <- (32x512xf32, 32x1xf32)
        ...
        # pd_op.matmul: (32x159xf32) <- (32x512xf32, 512x159xf32)
        matmul_50 = paddle._C_ops.matmul(divide_0, divide_1, False, False)

        # pd_op.square: (32x159xf32) <- (32x159xf32)
        ...
        # pd_op.scale: (32x159xf32) <- (32x159xf32, 1xf32)
        ...
        # pd_op.sqrt: (32x159xf32) <- (32x159xf32)
        ...
        # pd_op.subtract: (32x159xf32) <- (32x159xf32, 32x159xf32)
        ...
        # pd_op.greater_than: (32x159xb) <- (32x159xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(matmul_50, full_7)

        # pd_op.cast: (32x159xf32) <- (32x159xb)
        cast_0 = paddle._C_ops.cast(greater_than_0, paddle.float32)

        # pd_op.multiply: (32x159xf32) <- (32x159xf32, 32x159xf32)
        ...
        # pd_op.add: (32x159xf32) <- (32x159xf32, 32x159xf32)
        ...
        # pd_op.one_hot: (32x1x159xf32) <- (32x1xi64, 1xi32)
        one_hot_0 = paddle._C_ops.one_hot(
            data_1 % paddle.cast(full_8, data_1.dtype), full_8
        )

        # pd_op.squeeze: (32x159xf32) <- (32x1x159xf32, 1xi64)
