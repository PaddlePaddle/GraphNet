...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.relu: (1x256x25x38xf32) <- (1x256x25x38xf32)
        relu_3 = paddle._C_ops.relu(add_13)

        # pd_op.conv2d: (1x256x13x19xf32) <- (1x256x13x19xf32, 256x256x3x3xf32)
        conv2d_4 = paddle._C_ops.conv2d(
            data_4, parameter_5, [1, 1], [1, 1], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x256x13x19xf32) <- (1x256x13x19xf32, 1x256x1x1xf32)
        add_14 = paddle._C_ops.add(conv2d_4, reshape_0)

        # pd_op.relu: (1x256x13x19xf32) <- (1x256x13x19xf32)
        relu_4 = paddle._C_ops.relu(add_14)

        # pd_op.conv2d: (1x3x200x304xf32) <- (1x256x200x304xf32, 3x256x1x1xf32)
        ...
        # pd_op.add: (1x3x200x304xf32) <- (1x3x200x304xf32, 1x3x1x1xf32)
        add_0 = paddle._C_ops.add(conv2d_5, reshape_1)

        # pd_op.conv2d: (1x12x200x304xf32) <- (1x256x200x304xf32, 12x256x1x1xf32)
        ...
        # pd_op.add: (1x12x200x304xf32) <- (1x12x200x304xf32, 1x12x1x1xf32)
        add_5 = paddle._C_ops.add(conv2d_6, reshape_2)

        # pd_op.conv2d: (1x3x100x152xf32) <- (1x256x100x152xf32, 3x256x1x1xf32)
        conv2d_7 = paddle._C_ops.conv2d(
            relu_1, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x3x100x152xf32) <- (1x3x100x152xf32, 1x3x1x1xf32)
        add_1 = paddle._C_ops.add(conv2d_7, reshape_1)

        # pd_op.conv2d: (1x12x100x152xf32) <- (1x256x100x152xf32, 12x256x1x1xf32)
        conv2d_8 = paddle._C_ops.conv2d(
            relu_1, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x12x100x152xf32) <- (1x12x100x152xf32, 1x12x1x1xf32)
        add_6 = paddle._C_ops.add(conv2d_8, reshape_2)

        # pd_op.conv2d: (1x3x50x76xf32) <- (1x256x50x76xf32, 3x256x1x1xf32)
        conv2d_9 = paddle._C_ops.conv2d(
            relu_2, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x3x50x76xf32) <- (1x3x50x76xf32, 1x3x1x1xf32)
        add_2 = paddle._C_ops.add(conv2d_9, reshape_1)

        # pd_op.conv2d: (1x12x50x76xf32) <- (1x256x50x76xf32, 12x256x1x1xf32)
        conv2d_10 = paddle._C_ops.conv2d(
            relu_2, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x12x50x76xf32) <- (1x12x50x76xf32, 1x12x1x1xf32)
        add_7 = paddle._C_ops.add(conv2d_10, reshape_2)

        # pd_op.conv2d: (1x3x25x38xf32) <- (1x256x25x38xf32, 3x256x1x1xf32)
        conv2d_11 = paddle._C_ops.conv2d(
            relu_3, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x3x25x38xf32) <- (1x3x25x38xf32, 1x3x1x1xf32)
        add_3 = paddle._C_ops.add(conv2d_11, reshape_1)

        # pd_op.conv2d: (1x12x25x38xf32) <- (1x256x25x38xf32, 12x256x1x1xf32)
        conv2d_12 = paddle._C_ops.conv2d(
            relu_3, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x12x25x38xf32) <- (1x12x25x38xf32, 1x12x1x1xf32)
        add_8 = paddle._C_ops.add(conv2d_12, reshape_2)

        # pd_op.conv2d: (1x3x13x19xf32) <- (1x256x13x19xf32, 3x256x1x1xf32)
        conv2d_13 = paddle._C_ops.conv2d(
            relu_4, parameter_3, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x3x13x19xf32) <- (1x3x13x19xf32, 1x3x1x1xf32)
        add_4 = paddle._C_ops.add(conv2d_13, reshape_1)

        # pd_op.conv2d: (1x12x13x19xf32) <- (1x256x13x19xf32, 12x256x1x1xf32)
        conv2d_14 = paddle._C_ops.conv2d(
            relu_4, parameter_1, [1, 1], [0, 0], "EXPLICIT", [1, 1], 1, "NCHW"
        )

        # pd_op.add: (1x12x13x19xf32) <- (1x12x13x19xf32, 1x12x1x1xf32)
        ...
        full_1 = paddle._C_ops.full(
            [1], float("1216"), paddle.float64, paddle.core.CPUPlace()
        )
        ...
        # pd_op.arange: (304xf32) <- (1xf64, 1xf64, 1xf64)
        ...
        # builtin.combine: ([200xf32, 304xf32]) <- (200xf32, 304xf32)
        combine_0 = [arange_1, arange_0]

        # pd_op.meshgrid: ([200x304xf32, 200x304xf32]) <- ([200xf32, 304xf32])
        meshgrid_0 = paddle._C_ops.meshgrid(combine_0)

        # builtin.split: (200x304xf32, 200x304xf32) <- ([200x304xf32, 200x304xf32])
        (
            split_0,
            split_1,
        ) = meshgrid_0
        ...
        # pd_op.reshape: (60800xf32) <- (200x304xf32, 1xi64)
        reshape_8 = paddle._C_ops.reshape(split_1, full_int_array_1)

        # pd_op.reshape: (60800xf32) <- (200x304xf32, 1xi64)
        reshape_9 = paddle._C_ops.reshape(split_0, full_int_array_1)

        # builtin.combine: ([60800xf32, 60800xf32, 60800xf32, 60800xf32]) <- (60800xf32, 60800xf32, 60800xf32, 60800xf32)
        combine_1 = [reshape_8, reshape_9, reshape_8, reshape_9]

        # pd_op.stack: (60800x4xf32) <- ([60800xf32, 60800xf32, 60800xf32, 60800xf32])
        stack_0 = paddle._C_ops.stack(combine_1, 1)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [-1, 1, 4]

        # pd_op.reshape: (60800x1x4xf32) <- (60800x4xf32, 3xi64)
        ...
        # pd_op.add: (60800x3x4xf32) <- (60800x1x4xf32, 1x3x4xf32)
        add_15 = paddle._C_ops.add(reshape_10, reshape_11)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_4 = [-1, 4]

        # pd_op.reshape: (182400x4xf32) <- (60800x3x4xf32, 2xi64)
        ...
        # pd_op.arange: (152xf32) <- (1xf64, 1xf64, 1xf64)
        ...
        # pd_op.arange: (38xf32) <- (1xf64, 1xf64, 1xf64)
