...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.unsqueeze: (1x76x76x2xf32) <- (76x76x2xf32, 1xi64)
        ...
        # pd_op.scale: (1x76x76x2xf32) <- (1x76x76x2xf32, 1xf32)
        scale_0 = paddle._C_ops.scale(unsqueeze_0, full_4, float("0.5"), True)

        # pd_op.divide: (1x76x76x2xf32) <- (1x76x76x2xf32, 2xf32)
        divide_0 = paddle._C_ops.divide(scale_0, cast_1)

        # pd_op.full_like: (1x76x76x2xf32) <- (1x76x76x2xf32, 1xf32)
        ...
        # builtin.combine: ([1x76x76x2xf32, 1x76x76x2xf32]) <- (1x76x76x2xf32, 1x76x76x2xf32)
        combine_2 = [divide_0, scale_2]

        # pd_op.concat: (1x76x76x4xf32) <- ([1x76x76x2xf32, 1x76x76x2xf32], 1xi32)
        concat_1 = paddle._C_ops.concat(combine_2, full_6)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_1 = [-1, 5776, 4]

        # pd_op.reshape: (1x5776x4xf32) <- (1x76x76x4xf32, 3xi64)
        ...
        # pd_op.unsqueeze: (1x38x38x2xf32) <- (38x38x2xf32, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(stack_2, full_int_array_0)

        # pd_op.scale: (1x38x38x2xf32) <- (1x38x38x2xf32, 1xf32)
        scale_3 = paddle._C_ops.scale(unsqueeze_1, full_4, float("0.5"), True)

        # pd_op.divide: (1x38x38x2xf32) <- (1x38x38x2xf32, 2xf32)
        divide_1 = paddle._C_ops.divide(scale_3, cast_2)

        # pd_op.full_like: (1x38x38x2xf32) <- (1x38x38x2xf32, 1xf32)
        ...
        # builtin.combine: ([1x38x38x2xf32, 1x38x38x2xf32]) <- (1x38x38x2xf32, 1x38x38x2xf32)
        combine_5 = [divide_1, scale_5]

        # pd_op.concat: (1x38x38x4xf32) <- ([1x38x38x2xf32, 1x38x38x2xf32], 1xi32)
        concat_2 = paddle._C_ops.concat(combine_5, full_6)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [-1, 1444, 4]

        # pd_op.reshape: (1x1444x4xf32) <- (1x38x38x4xf32, 3xi64)
        ...
        # pd_op.unsqueeze: (1x19x19x2xf32) <- (19x19x2xf32, 1xi64)
        unsqueeze_2 = paddle._C_ops.unsqueeze(stack_3, full_int_array_0)

        # pd_op.scale: (1x19x19x2xf32) <- (1x19x19x2xf32, 1xf32)
        scale_6 = paddle._C_ops.scale(unsqueeze_2, full_4, float("0.5"), True)

        # pd_op.divide: (1x19x19x2xf32) <- (1x19x19x2xf32, 2xf32)
        divide_2 = paddle._C_ops.divide(scale_6, cast_3)

        # pd_op.full_like: (1x19x19x2xf32) <- (1x19x19x2xf32, 1xf32)
        ...
        # builtin.combine: ([1x19x19x2xf32, 1x19x19x2xf32]) <- (1x19x19x2xf32, 1x19x19x2xf32)
        combine_8 = [divide_2, scale_8]

        # pd_op.concat: (1x19x19x4xf32) <- ([1x19x19x2xf32, 1x19x19x2xf32], 1xi32)
        concat_3 = paddle._C_ops.concat(combine_8, full_6)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [-1, 361, 4]

        # pd_op.reshape: (1x361x4xf32) <- (1x19x19x4xf32, 3xi64)
        ...
        # builtin.combine: ([1x5776x4xf32, 1x1444x4xf32, 1x361x4xf32]) <- (1x5776x4xf32, 1x1444x4xf32, 1x361x4xf32)
        combine_9 = [reshape_0, reshape_1, reshape_2]

        # pd_op.concat: (1x7581x4xf32) <- ([1x5776x4xf32, 1x1444x4xf32, 1x361x4xf32], 1xi32)
        ...
        # pd_op.greater_than: (1x7581x4xb) <- (1x7581x4xf32, xf32)
        ...
        # pd_op.less_than: (1x7581x4xb) <- (1x7581x4xf32, xf32)
        less_than_0 = paddle._C_ops.less_than(concat_4, full_15)

        # pd_op.multiply: (1x7581x4xb) <- (1x7581x4xb, 1x7581x4xb)
        multiply_0 = paddle._C_ops.multiply(greater_than_0, less_than_0)

        # pd_op.all: (1x7581x1xb) <- (1x7581x4xb)
        ...
        # pd_op.scale: (1x7581x4xf32) <- (1x7581x4xf32, 1xf32)
        scale_9 = paddle._C_ops.scale(concat_4, full_16, float("1"), True)

        # pd_op.divide: (1x7581x4xf32) <- (1x7581x4xf32, 1x7581x4xf32)
        divide_3 = paddle._C_ops.divide(concat_4, scale_9)

        # pd_op.log: (1x7581x4xf32) <- (1x7581x4xf32)
        ...
        # pd_op.full_like: (1x7581x4xf32) <- (1x7581x4xf32, 1xf32)
        ...
        # pd_op.full_like: (1x7581x256xf32) <- (1x7581x256xf32, 1xf32)
        ...
        # pd_op.add: (1x7581x256xf32) <- (1x7581x256xf32, xf32)
        add_12 = paddle._C_ops.add(full_like_6, full_like_7)

        # pd_op.add: (1x7581x256xf32) <- (1x7581x256xf32, 1x7581x1xf32)
        add_0 = paddle._C_ops.add(add_12, cast_7)

        # pd_op.add: (1x7581x256xf32) <- (1x7581x256xf32, 1x7581x256xf32)
        add_1 = paddle._C_ops.add(data_0, add_0)

        # pd_op.add: (1x7581x256xf32) <- (xf32, 1x7581x256xf32)
        add_2 = paddle._C_ops.add(assign_value__0, add_0)

        # pd_op.add: (1x7581x256xf32) <- (1x7581x1xf32, 1x7581x256xf32)
        add_13 = paddle._C_ops.add(cast_8, add_0)

        # pd_op.cast: (1x7581x256xb) <- (1x7581x256xf32)
        cast_0 = paddle._C_ops.cast(add_13, paddle.bool)

        # pd_op.where: (1x7581x256xf32) <- (1x7581x256xb, 1x7581x256xf32, 1x7581x256xf32)
        ...
        # pd_op.layer_norm: (1x7581x256xf32, 1x7581xf32, 1x7581xf32) <- (1x7581x256xf32, 256xf32, 256xf32)
        ...
        # pd_op.matmul: (1x7581x11xf32) <- (1x7581x256xf32, 256x11xf32)
        matmul_1 = paddle._C_ops.matmul(layer_norm_0, parameter_7, False, False)

        # pd_op.add: (1x7581x11xf32) <- (1x7581x11xf32, 11xf32)
        ...
        # pd_op.relu: (1x7581x256xf32) <- (1x7581x256xf32)
        ...
        # pd_op.matmul: (1x7581x4xf32) <- (1x7581x256xf32, 256x4xf32)
        matmul_4 = paddle._C_ops.matmul(relu_1, parameter_1, False, False)

        # pd_op.add: (1x7581x4xf32) <- (1x7581x4xf32, 4xf32)
        ...
        # pd_op.max: (1x7581xf32) <- (1x7581x11xf32, 1xi64)
        ...
        # pd_op.topk: (1x300xf32, 1x300xi64) <- (1x7581xf32, 1xi32)
        ...
        # pd_op.gather_nd: (1x300x4xf32) <- (1x7581x4xf32, 1x300x2xi64)
        ...
        # builtin.combine: ([1x192x4xf32, 1x300x4xf32]) <- (1x192x4xf32, 1x300x4xf32)
        combine_11 = [data_2, gather_nd_1]

        # pd_op.concat: (1x492x4xf32) <- ([1x192x4xf32, 1x300x4xf32], 1xi32)
        concat_5 = paddle._C_ops.concat(combine_11, full_13)

        # pd_op.share_data_: (1x492x4xf32) <- (1x492x4xf32)
        share_data__1 = concat_5.detach()

        # pd_op.gather_nd: (1x300x11xf32) <- (1x7581x11xf32, 1x300x2xi64)
        gather_nd_0 = paddle._C_ops.gather_nd(add_4, stack_0)

        # pd_op.gather_nd: (1x300x256xf32) <- (1x7581x256xf32, 1x300x2xi64)
        ...
        # builtin.combine: ([1x192x256xf32, 1x300x256xf32]) <- (1x192x256xf32, 1x300x256xf32)
        combine_12 = [data_1, share_data__0]

        # pd_op.concat: (1x492x256xf32) <- ([1x192x256xf32, 1x300x256xf32], 1xi32)
