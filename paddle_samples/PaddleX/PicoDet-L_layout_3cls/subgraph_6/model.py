...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (2x7681xi64) <- (2x3x7681xf32, 1xi64)
        ...
        # pd_op.add: (2x7681xi64) <- (2x7681xi64, 2x1xi64)
        add_0 = paddle._C_ops.add(argmax_0, cast_0)
        del argmax_0, cast_0

        # pd_op.flatten: (6xi32) <- (2x3x1xi32)
        flatten_0 = paddle._C_ops.flatten(data_1, 0, 2)
        del data_1

        # pd_op.flatten: (15362xi64) <- (2x7681xi64)
        ...
        # pd_op.gather: (15362xi32) <- (6xi32, 15362xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_5)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [2, 7681]

        # pd_op.reshape: (2x7681xi32) <- (15362xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x7681xb) <- (2x7681xf32, xf32)
        ...
        # pd_op.full_like: (2x7681xi32) <- (2x7681xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_7, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_7

        # pd_op.where: (2x7681xi32) <- (2x7681xb, 2x7681xi32, 2x7681xi32)
        ...
        # pd_op.gather: (15362x4xf32) <- (6x4xf32, 15362xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_5)
        del flatten_1, full_5, reshape_2

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [2, 7681, 4]

        # pd_op.reshape: (2x7681x4xf32) <- (15362x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (2x7681x12xf32) <- (2x7681xi32, 1xi32)
        ...
        # pd_op.index_select: (2x7681x11xf32) <- (2x7681x12xf32, 11xi64)
        ...
        # pd_op.unsqueeze: (2x1x7681x4xf32) <- (2x7681x4xf32, 1xi64)
        ...
        # pd_op.slice: (2x3x1x2xf32) <- (2x3x1x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.slice: (2x1x7681x2xf32) <- (2x1x7681x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (2x3x7681x2xf32) <- (2x3x1x2xf32, 2x1x7681x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (2x3x7681x2xf32) <- (2x3x1x2xf32, 2x1x7681x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (2x3x7681x2xf32) <- (2x3x7681x2xf32, 2x3x7681x2xf32)
        ...
        # pd_op.scale: (2x3x7681xf32) <- (2x3x7681xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_12, float("1e-09"), True)
        del full_12, subtract_3

        # pd_op.divide: (2x3x7681xf32) <- (2x3x7681xf32, 2x3x7681xf32)
