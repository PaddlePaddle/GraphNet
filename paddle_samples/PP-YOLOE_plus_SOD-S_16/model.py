...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        # pd_op.cast: (2x6804xi32) <- (2x6804xb)
        ...
        # pd_op.sum: (2x6804xf32) <- (2x6804x10xf32, 1xi64)
        sum_2 = paddle._C_ops.sum(data_3, full_int_array_1, None, False)

        # pd_op.masked_select: (-1xf32) <- (2x6804xf32, 2x6804xb)
        ...
        # pd_op.unsqueeze: (2x6804x1xb) <- (2x6804xb, 1xi64)
        unsqueeze_4 = paddle._C_ops.unsqueeze(data_0, full_int_array_1)

        # pd_op.cast: (2x6804x1xi32) <- (2x6804x1xb)
        cast_3 = paddle._C_ops.cast(unsqueeze_4, paddle.int32)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_4 = [1, 1, 40]

        # pd_op.tile: (2x6804x40xi32) <- (2x6804x1xi32, 3xi64)
        tile_1 = paddle._C_ops.tile(cast_3, full_int_array_4)

        # pd_op.cast: (2x6804x40xb) <- (2x6804x40xi32)
        cast_1 = paddle._C_ops.cast(tile_1, paddle.bool)

        # pd_op.masked_select: (-1xf32) <- (2x6804x40xf32, 2x6804x40xb)
        masked_select_1 = paddle._C_ops.masked_select(data_5, cast_1)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [-1, 4, 10]

        # pd_op.reshape: (-1x4x10xf32) <- (-1xf32, 3xi64)
        ...
        # pd_op.split_with_num: ([2x6804x2xf32, 2x6804x2xf32]) <- (2x6804x4xf32, 1xi32)
        split_with_num_2 = paddle._C_ops.split_with_num(data_2, 2, full_5)

        # builtin.split: (2x6804x2xf32, 2x6804x2xf32) <- ([2x6804x2xf32, 2x6804x2xf32])
        (
            split_8,
            split_9,
        ) = split_with_num_2

        # pd_op.subtract: (2x6804x2xf32) <- (6804x2xf32, 2x6804x2xf32)
        subtract_13 = paddle._C_ops.subtract(data_6, split_8)

        # pd_op.subtract: (2x6804x2xf32) <- (2x6804x2xf32, 6804x2xf32)
        ...
        # builtin.combine: ([2x6804x2xf32, 2x6804x2xf32]) <- (2x6804x2xf32, 2x6804x2xf32)
        combine_0 = [subtract_13, subtract_14]

        # pd_op.concat: (2x6804x4xf32) <- ([2x6804x2xf32, 2x6804x2xf32], 1xi32)
        ...
        full_8 = paddle._C_ops.full(
            [1], float("6.99"), paddle.float32, paddle.core.CPUPlace()
        )

        # pd_op.clip: (2x6804x4xf32) <- (2x6804x4xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(concat_0, full_7, full_8)
        ...
        # pd_op.scale: (-1x4xi64) <- (-1x4xi64, 1xf32)
        scale_6 = paddle._C_ops.scale(cast_4, full_3, float("2"), True)

        # pd_op.unsqueeze: (-1x4x1xi64) <- (-1x4xi64, 1xi64)
        unsqueeze_1 = paddle._C_ops.unsqueeze(scale_6, full_int_array_1)

        # pd_op.cross_entropy_with_softmax: (-1x4x10xf32, -1x4x1xf32) <- (-1x4x10xf32, -1x4x1xi64)
        ...
        # pd_op.scale: (-1x4xi64) <- (-1x4xi64, 1xf32)
        scale_7 = paddle._C_ops.scale(scale_5, full_3, float("2"), True)
