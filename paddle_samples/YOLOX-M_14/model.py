...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1):
        ...
        # pd_op.slice: (524xf32) <- (524x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.subtract: (524xf32) <- (524xf32, 524xf32)
        ...
        # pd_op.multiply: (524xf32) <- (524xf32, 524xf32)
        ...
        # pd_op.subtract: (6xf32) <- (6xf32, 6xf32)
        ...
        # pd_op.slice: (524x2xf32) <- (524x4xf32, 1xi64, 1xi64)
        slice_8 = paddle._C_ops.slice(
            data_0, [1], full_int_array_2, full_int_array_0, [1], []
        )

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_5 = [524, 1, 2]

        # pd_op.reshape: (524x1x2xf32) <- (524x2xf32, 3xi64)
        reshape_0 = paddle._C_ops.reshape(slice_8, full_int_array_5)

        # pd_op.slice: (6x2xf32) <- (6x4xf32, 1xi64, 1xi64)
        slice_9 = paddle._C_ops.slice(
            data_1, [1], full_int_array_2, full_int_array_0, [1], []
        )

        # pd_op.maximum: (524x6x2xf32) <- (524x1x2xf32, 6x2xf32)
        ...
        # pd_op.minimum: (524x6x2xf32) <- (524x1x2xf32, 6x2xf32)
        minimum_0 = paddle._C_ops.minimum(reshape_1, slice_11)

        # pd_op.subtract: (524x6x2xf32) <- (524x6x2xf32, 524x6x2xf32)
        ...
        # pd_op.clip: (524x6x2xf32) <- (524x6x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_4, full_0, full_1)

        # pd_op.slice: (524x6xf32) <- (524x6x2xf32, 1xi64, 1xi64)
        ...
        # pd_op.multiply: (524x6xf32) <- (524x6xf32, 524x6xf32)
        multiply_2 = paddle._C_ops.multiply(slice_12, slice_13)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_7 = [524, 1]

        # pd_op.reshape: (524x1xf32) <- (524xf32, 2xi64)
        reshape_2 = paddle._C_ops.reshape(multiply_0, full_int_array_7)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_8 = [1, 6]

        # pd_op.reshape: (1x6xf32) <- (6xf32, 2xi64)
        reshape_3 = paddle._C_ops.reshape(multiply_1, full_int_array_8)

        # pd_op.add: (524x6xf32) <- (524x1xf32, 1x6xf32)
        add_0 = paddle._C_ops.add(reshape_2, reshape_3)

        # pd_op.subtract: (524x6xf32) <- (524x6xf32, 524x6xf32)
        ...
        # pd_op.maximum: (524x6xf32) <- (524x6xf32, 1xf32)
        maximum_1 = paddle._C_ops.maximum(subtract_5, assign_value__0)

        # pd_op.divide: (524x6xf32) <- (524x6xf32, 524x6xf32)
