...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (2x9382xi64) <- (2x3x9382xf32, 1xi64)
        ...
        # pd_op.gather: (18764xi32) <- (6xi32, 18764xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_5)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [2, 9382]

        # pd_op.reshape: (2x9382xi32) <- (18764xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x9382xb) <- (2x9382xf32, xf32)
        ...
        # pd_op.full_like: (2x9382xi32) <- (2x9382xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_7, paddle.int32, paddle.framework._current_expected_place()
        )

        # pd_op.where: (2x9382xi32) <- (2x9382xb, 2x9382xi32, 2x9382xi32)
        ...
        # pd_op.gather: (18764x4xf32) <- (6x4xf32, 18764xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_5)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [2, 9382, 4]

        # pd_op.reshape: (2x9382x4xf32) <- (18764x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (2x9382x12xf32) <- (2x9382xi32, 1xi32)
        ...
        # pd_op.index_select: (2x9382x11xf32) <- (2x9382x12xf32, 11xi64)
        ...
        # pd_op.unsqueeze: (2x1x9382x4xf32) <- (2x9382x4xf32, 1xi64)
        ...
        # pd_op.slice: (2x1x9382x2xf32) <- (2x1x9382x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (2x3x9382x2xf32) <- (2x3x1x2xf32, 2x1x9382x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (2x3x9382x2xf32) <- (2x3x1x2xf32, 2x1x9382x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (2x3x9382x2xf32) <- (2x3x9382x2xf32, 2x3x9382x2xf32)
        ...
        # pd_op.clip: (2x3x9382x2xf32) <- (2x3x9382x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_10, full_11)

        # pd_op.prod: (2x3x9382xf32) <- (2x3x9382x2xf32, 1xi64)
        ...
        # pd_op.subtract: (2x1x9382x2xf32) <- (2x1x9382x2xf32, 2x1x9382x2xf32)
        subtract_2 = paddle._C_ops.subtract(slice_3, slice_2)

        # pd_op.clip: (2x1x9382x2xf32) <- (2x1x9382x2xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(subtract_2, full_10, full_11)

        # pd_op.prod: (2x1x9382xf32) <- (2x1x9382x2xf32, 1xi64)
        prod_2 = paddle._C_ops.prod(clip_2, full_int_array_0, False, False)

        # pd_op.add: (2x3x9382xf32) <- (2x3x1xf32, 2x1x9382xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)

        # pd_op.subtract: (2x3x9382xf32) <- (2x3x9382xf32, 2x3x9382xf32)
        ...
        # pd_op.scale: (2x3x9382xf32) <- (2x3x9382xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_12, float("1e-09"), True)

        # pd_op.divide: (2x3x9382xf32) <- (2x3x9382xf32, 2x3x9382xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_1)

        # pd_op.multiply: (2x3x9382xf32) <- (2x3x9382xf32, 2x3x9382xf32)
        multiply_1 = paddle._C_ops.multiply(divide_0, data_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_8 = [-2]

        # pd_op.max: (2x9382xf32) <- (2x3x9382xf32, 1xi64)
        max_0 = paddle._C_ops.max(multiply_1, full_int_array_8, False)

        # pd_op.unsqueeze: (2x9382x1xf32) <- (2x9382xf32, 1xi64)
        unsqueeze_3 = paddle._C_ops.unsqueeze(max_0, full_int_array_0)

        # pd_op.multiply: (2x9382x11xf32) <- (2x9382x11xf32, 2x9382x1xf32)
