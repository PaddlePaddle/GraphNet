...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4):
        ...
        # pd_op.argmax: (1x4165xi64) <- (1x2x4165xf32, 1xi64)
        ...
        # pd_op.add: (1x4165xi64) <- (1x4165xi64, 1x1xi64)
        ...
        # pd_op.flatten: (4165xi64) <- (1x4165xi64)
        ...
        # pd_op.gather: (4165xi32) <- (2xi32, 4165xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_4)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_1 = [1, 4165]

        # pd_op.reshape: (1x4165xi32) <- (4165xi32, 2xi64)
        ...
        # pd_op.greater_than: (1x4165xb) <- (1x4165xf32, xf32)
        ...
        # pd_op.full_like: (1x4165xi32) <- (1x4165xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_6, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_6

        # pd_op.where: (1x4165xi32) <- (1x4165xb, 1x4165xi32, 1x4165xi32)
        ...
        # pd_op.gather: (4165x4xf32) <- (2x4xf32, 4165xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_4)
        del flatten_1, full_4, reshape_2

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_3 = [1, 4165, 4]

        # pd_op.reshape: (1x4165x4xf32) <- (4165x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (1x4165x12xf32) <- (1x4165xi32, 1xi32)
        ...
        # pd_op.index_select: (1x4165x11xf32) <- (1x4165x12xf32, 11xi64)
        ...
        # pd_op.unsqueeze: (1x1x4165x4xf32) <- (1x4165x4xf32, 1xi64)
        ...
        # pd_op.slice: (1x1x4165x2xf32) <- (1x1x4165x4xf32, 1xi64, 1xi64)
        ...
        # pd_op.maximum: (1x2x4165x2xf32) <- (1x2x1x2xf32, 1x1x4165x2xf32)
        maximum_0 = paddle._C_ops.maximum(slice_0, slice_2)

        # pd_op.minimum: (1x2x4165x2xf32) <- (1x2x1x2xf32, 1x1x4165x2xf32)
        minimum_0 = paddle._C_ops.minimum(slice_1, slice_3)

        # pd_op.subtract: (1x2x4165x2xf32) <- (1x2x4165x2xf32, 1x2x4165x2xf32)
        ...
        # pd_op.clip: (1x2x4165x2xf32) <- (1x2x4165x2xf32, 1xf32, 1xf32)
        clip_0 = paddle._C_ops.clip(subtract_0, full_9, full_10)
        del subtract_0

        # pd_op.prod: (1x2x4165xf32) <- (1x2x4165x2xf32, 1xi64)
        ...
        # pd_op.subtract: (1x1x4165x2xf32) <- (1x1x4165x2xf32, 1x1x4165x2xf32)
        subtract_2 = paddle._C_ops.subtract(slice_3, slice_2)
        del slice_2, slice_3

        # pd_op.clip: (1x1x4165x2xf32) <- (1x1x4165x2xf32, 1xf32, 1xf32)
        clip_2 = paddle._C_ops.clip(subtract_2, full_9, full_10)
        del full_10, full_9, subtract_2

        # pd_op.prod: (1x1x4165xf32) <- (1x1x4165x2xf32, 1xi64)
        prod_2 = paddle._C_ops.prod(clip_2, full_int_array_0, False, False)
        del clip_2

        # pd_op.add: (1x2x4165xf32) <- (1x2x1xf32, 1x1x4165xf32)
        add_1 = paddle._C_ops.add(prod_1, prod_2)
        del prod_1, prod_2

        # pd_op.subtract: (1x2x4165xf32) <- (1x2x4165xf32, 1x2x4165xf32)
        ...
        # pd_op.scale: (1x2x4165xf32) <- (1x2x4165xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(subtract_3, full_11, float("1e-09"), True)
        del full_11, subtract_3

        # pd_op.divide: (1x2x4165xf32) <- (1x2x4165xf32, 1x2x4165xf32)
        divide_0 = paddle._C_ops.divide(prod_0, scale_1)
        del prod_0, scale_1

        # pd_op.multiply: (1x2x4165xf32) <- (1x2x4165xf32, 1x2x4165xf32)
