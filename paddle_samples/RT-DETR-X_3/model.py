...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(
        ...
        # pd_op.add: (2x498x256xf32) <- (2x498x256xf32, 2x498x256xf32)
        add_0 = paddle._C_ops.add(data_1, data_0)

        # pd_op.layer_norm: (2x498x256xf32, 2x498xf32, 2x498xf32) <- (2x498x256xf32, 256xf32, 256xf32)
        ...
        # pd_op.matmul: (2x4725x256xf32) <- (2x4725x256xf32, 256x256xf32)
        matmul_0 = paddle._C_ops.matmul(data_4, parameter_15, False, False)

        # pd_op.add: (2x4725x256xf32) <- (2x4725x256xf32, 256xf32)
        add_2 = paddle._C_ops.add(matmul_0, parameter_14)

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_1 = [2, 4725, 8, 32]
        ...
        full_int_array_2 = [2, 498, 8, 3, 4, 2]
        ...
        # pd_op.split: ([-1x-1x-1x-1xf32, -1x-1x-1x-1xf32, -1x-1x-1x-1xf32]) <- (2x4725x8x32xf32, 3xi64, 1xi32)
        ...
        # pd_op.slice: (2x498x8x4x2xf32) <- (2x498x8x3x4x2xf32, 1xi64, 1xi64)
        slice_14 = paddle._C_ops.slice(
            scale_2, [3], full_int_array_5, full_int_array_9, [1], [3]
        )

        # pd_op.transpose: (2x8x498x4x2xf32) <- (2x498x8x4x2xf32)
        transpose_1 = paddle._C_ops.transpose(slice_14, [0, 2, 1, 3, 4])

        # pd_op.flatten: (16x498x4x2xf32) <- (2x8x498x4x2xf32)
        flatten_0 = paddle._C_ops.flatten(transpose_1, 0, 1)

        # pd_op.grid_sample: (16x32x498x4xf32) <- (16x32x-1x-1xf32, 16x498x4x2xf32)
        ...
        # pd_op.transpose: (2x8x498x3x4xf32) <- (2x498x8x3x4xf32)
        transpose_6 = paddle._C_ops.transpose(reshape_7, [0, 2, 1, 3, 4])

        # pd_op.full_int_array: (4xi64) <- ()
        full_int_array_11 = [16, 1, 498, 12]

        # pd_op.reshape: (16x1x498x12xf32) <- (2x8x498x3x4xf32, 4xi64)
        reshape_3 = paddle._C_ops.reshape(transpose_6, full_int_array_11)

        # builtin.combine: ([16x32x498x4xf32, 16x32x498x4xf32, 16x32x498x4xf32]) <- (16x32x498x4xf32, 16x32x498x4xf32, 16x32x498x4xf32)
        combine_4 = [grid_sample_0, grid_sample_1, grid_sample_2]

        # pd_op.stack: (16x32x498x3x4xf32) <- ([16x32x498x4xf32, 16x32x498x4xf32, 16x32x498x4xf32])
        stack_0 = paddle._C_ops.stack(combine_4, -2)

        # pd_op.flatten: (16x32x498x12xf32) <- (16x32x498x3x4xf32)
        flatten_3 = paddle._C_ops.flatten(stack_0, 3, 4)

        # pd_op.multiply: (16x32x498x12xf32) <- (16x32x498x12xf32, 16x1x498x12xf32)
        multiply_0 = paddle._C_ops.multiply(flatten_3, reshape_3)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_0 = [-1]

        # pd_op.sum: (16x32x498xf32) <- (16x32x498x12xf32, 1xi64)
        sum_0 = paddle._C_ops.sum(multiply_0, full_int_array_0, None, False)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_12 = [2, 256, 498]

        # pd_op.reshape: (2x256x498xf32) <- (16x32x498xf32, 3xi64)
        reshape_8 = paddle._C_ops.reshape(sum_0, full_int_array_12)

        # pd_op.transpose: (2x498x256xf32) <- (2x256x498xf32)
        transpose_7 = paddle._C_ops.transpose(reshape_8, [0, 2, 1])

        # pd_op.matmul: (2x498x256xf32) <- (2x498x256xf32, 256x256xf32)
