...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3):
        ...
        # pd_op.slice: (10105xf32) <- (10105x4xf32, 1xi64, 1xi64)
        slice_1 = paddle._C_ops.slice(
            data_1, [1], full_int_array_0, full_int_array_1, [1], [1]
        )

        # pd_op.unsqueeze: (10105x1xf32) <- (10105xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(slice_1, full_int_array_1)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_2 = [1, 7]

        # pd_op.tile: (10105x7xf32) <- (10105x1xf32, 2xi64)
        ...
        # pd_op.slice: (7xf32) <- (7x4xf32, 1xi64, 1xi64)
        slice_5 = paddle._C_ops.slice(
            data_3, [1], full_int_array_0, full_int_array_1, [1], [1]
        )

        # pd_op.subtract: (10105x7xf32) <- (10105x7xf32, 7xf32)
        ...
        # pd_op.subtract: (10105x7xf32) <- (7xf32, 10105x7xf32)
        ...
        # builtin.combine: ([10105x7xf32, 10105x7xf32, 10105x7xf32, 10105x7xf32]) <- (10105x7xf32, 10105x7xf32, 10105x7xf32, 10105x7xf32)
        combine_0 = [subtract_0, subtract_1, subtract_2, subtract_3]

        # pd_op.stack: (10105x4x7xf32) <- ([10105x7xf32, 10105x7xf32, 10105x7xf32, 10105x7xf32])
        stack_0 = paddle._C_ops.stack(combine_0, 1)

        # pd_op.min: (10105x7xf32) <- (10105x4x7xf32, 1xi64)
        ...
        # pd_op.greater_than: (10105x7xb) <- (10105x7xf32, xf32)
        greater_than_0 = paddle._C_ops.greater_than(min_0, full_0)

        # pd_op.sum: (10105xi64) <- (10105x7xb, 1xi64)
        ...
        # pd_op.greater_than: (10105xb) <- (10105xi64, xi64)
        greater_than_1 = paddle._C_ops.greater_than(sum_0, full_1)

        # pd_op.add: (7xf32) <- (7xf32, 7xf32)
        ...
        # pd_op.scale: (10105x7xf32) <- (10105x7xf32, 1xf32)
        ...
        # pd_op.add: (10105x7xf32) <- (7xf32, 10105x7xf32)
        add_2 = paddle._C_ops.add(scale_0, scale_2)

        # pd_op.add: (10105x7xf32) <- (7xf32, 10105x7xf32)
        add_3 = paddle._C_ops.add(scale_1, scale_3)

        # pd_op.subtract: (10105x7xf32) <- (10105x7xf32, 10105x7xf32)
        ...
        # pd_op.logical_or: (10105xb) <- (10105xb, 10105xb)
        logical_or_0 = paddle._C_ops.logical_or(greater_than_1, greater_than_3)

        # pd_op.nonzero: (-1x1xi64) <- (10105xb)
        ...
        # pd_op.cast: (10105x7xi64) <- (10105x7xb)
        ...
        # pd_op.gather: (-1x7xi64) <- (10105x7xi64, -1xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(cast_0, squeeze_0, full_4)

        # pd_op.cast: (-1x7xb) <- (-1x7xi64)
        ...
        # pd_op.logical_and: (-1x7xb) <- (-1x7xb, -1x7xb)
        ...
        # pd_op.gather_nd: (-1x4xf32) <- (10105x4xf32, -1x1xi64)
        gather_nd_0 = paddle._C_ops.gather_nd(data_2, unsqueeze_4)

        # pd_op.gather_nd: (-1x11xf32) <- (10105x11xf32, -1x1xi64)
