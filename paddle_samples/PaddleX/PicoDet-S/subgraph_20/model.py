...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.argmax: (16x2577xi64) <- (16x5x2577xf32, 1xi64)
        ...
        # pd_op.add: (16x2577xi64) <- (16x2577xi64, 16x1xi64)
        ...
        # pd_op.flatten: (41232xi64) <- (16x2577xi64)
        ...
        # pd_op.gather: (41232xi32) <- (80xi32, 41232xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_2)
        del flatten_0

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_0 = [16, 2577]

        # pd_op.reshape: (16x2577xi32) <- (41232xi32, 2xi64)
        ...
        # pd_op.greater_than: (16x2577xb) <- (16x2577xf32, xf32)
        ...
        # pd_op.full_like: (16x2577xi32) <- (16x2577xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_4, paddle.int32, paddle.framework._current_expected_place()
        )
        del full_4

        # pd_op.where: (16x2577xi32) <- (16x2577xb, 16x2577xi32, 16x2577xi32)
        ...
        # pd_op.gather: (41232x4xf32) <- (80x4xf32, 41232xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_2)
        del flatten_1, full_2, reshape_2

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [16, 2577, 4]

        # pd_op.reshape: (16x2577x4xf32) <- (41232x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (16x2577x5xf32) <- (16x2577xi32, 1xi32)
        ...
        # pd_op.index_select: (16x2577x4xf32) <- (16x2577x5xf32, 4xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_0, assign_value__0, -1)
        del assign_value__0, one_hot_0

        # pd_op.multiply: (16x5x2577xf32) <- (16x5x2577xf32, 16x5x2577xf32)
        ...
        # pd_op.max: (16x5x1xf32) <- (16x5x2577xf32, 1xi64)
        ...
        # pd_op.scale: (16x5x1xf32) <- (16x5x1xf32, 1xf32)
        scale_1 = paddle._C_ops.scale(max_0, full_7, float("1e-09"), True)
        del full_7, max_0

        # pd_op.divide: (16x5x2577xf32) <- (16x5x2577xf32, 16x5x1xf32)
        divide_0 = paddle._C_ops.divide(multiply_1, scale_1)
        del multiply_1, scale_1

        # pd_op.multiply: (16x5x2577xf32) <- (16x5x2577xf32, 16x5x1xf32)
