...
class GraphModule(paddle.nn.Layer):
    ...
    def forward(self, data_0, data_1, data_2, data_3, data_4, data_5, data_6):
        ...
        # pd_op.argmax: (2x10164xi64) <- (2x12x10164xf32, 1xi64)
        ...
        # pd_op.gather: (20328xi32) <- (24xi32, 20328xi64, 1xi32)
        gather_0 = paddle._C_ops.gather(flatten_0, flatten_1, full_2)

        # pd_op.full_int_array: (2xi64) <- ()
        full_int_array_0 = [2, 10164]

        # pd_op.reshape: (2x10164xi32) <- (20328xi32, 2xi64)
        ...
        # pd_op.greater_than: (2x10164xb) <- (2x10164xf32, xf32)
        ...
        # pd_op.full_like: (2x10164xi32) <- (2x10164xi32, 1xf32)
        full_like_0 = paddle._C_ops.full_like(
            reshape_1, full_4, paddle.int32, paddle.framework._current_expected_place()
        )

        # pd_op.where: (2x10164xi32) <- (2x10164xb, 2x10164xi32, 2x10164xi32)
        ...
        # pd_op.gather: (20328x4xf32) <- (24x4xf32, 20328xi64, 1xi32)
        gather_1 = paddle._C_ops.gather(reshape_2, flatten_1, full_2)

        # pd_op.full_int_array: (3xi64) <- ()
        full_int_array_2 = [2, 10164, 4]

        # pd_op.reshape: (2x10164x4xf32) <- (20328x4xf32, 3xi64)
        ...
        # pd_op.one_hot: (2x10164x5xf32) <- (2x10164xi32, 1xi32)
        ...
        # pd_op.index_select: (2x10164x4xf32) <- (2x10164x5xf32, 4xi64)
        index_select_0 = paddle._C_ops.index_select(one_hot_0, assign_value__0, -1)

        # pd_op.multiply: (2x12x10164xf32) <- (2x12x10164xf32, 2x12x10164xf32)
        multiply_1 = paddle._C_ops.multiply(data_5, data_0)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_3 = [-1]

        # pd_op.max: (2x12x1xf32) <- (2x12x10164xf32, 1xi64)
        ...
        # pd_op.divide: (2x12x10164xf32) <- (2x12x10164xf32, 2x12x1xf32)
        divide_0 = paddle._C_ops.divide(multiply_1, scale_1)

        # pd_op.multiply: (2x12x10164xf32) <- (2x12x10164xf32, 2x12x1xf32)
        multiply_3 = paddle._C_ops.multiply(divide_0, max_1)

        # pd_op.full_int_array: (1xi64) <- ()
        full_int_array_4 = [-2]

        # pd_op.max: (2x10164xf32) <- (2x12x10164xf32, 1xi64)
        max_2 = paddle._C_ops.max(multiply_3, full_int_array_4, False)

        # pd_op.unsqueeze: (2x10164x1xf32) <- (2x10164xf32, 1xi64)
        unsqueeze_0 = paddle._C_ops.unsqueeze(max_2, full_int_array_3)

        # pd_op.multiply: (2x10164x4xf32) <- (2x10164x4xf32, 2x10164x1xf32)
