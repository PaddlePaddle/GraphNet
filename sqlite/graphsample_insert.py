...
def insert_graph_sample(session, data):
...
def insert_sample_op_name_list(
    ...
    session.execute(
        sql_delete(SampleOpNameList).where(SampleOpNameList.sample_uuid == sample_uuid)
    )
    session.execute(
    ...
    print(f"Inserted {len(selected_op_names)} op_names for sample_uuid={sample_uuid}")
        ...
        if data["is_subgraph"]:
            ...
            if sample_type in ["fusible_graph", "typical_graph"]:
                insert_dimension_generalization_source(
                    session=session,
                    generalized_graph_uuid=subgraph_source_data["subgraph_uuid"],
                    original_graph_uuid=subgraph_source_data["full_graph_uuid"],
                    model_path_prefix=model_path_prefix,
                    relative_model_path=relative_model_path,
                )
